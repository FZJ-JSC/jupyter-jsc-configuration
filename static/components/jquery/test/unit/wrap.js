(function () {
	...
	function testWrap(val, assert) {
		...
		result = jQuery("#first")
			.wrap(val("<div class='red'><span></span></div>"))
			.text();
		...
		result = jQuery("#first")
			.wrap(val(document.getElementById("empty")))
			.parent();
		...
		assert.equal(
			j[0].parentNode.nodeName.toUpperCase(),
			"LI",
		...
		assert.equal(
			j[0].previousSibling.nodeType,
			3,
			"Make sure the previous node is a text element"
		);
		assert.equal(
			j[0].parentNode.nodeName.toUpperCase(),
			"DIV",
			"And that we're in the div element."
		);

		// Try to wrap an element with multiple elements (should fail)
		j = jQuery("<div><span></span></div>")
			.children()
			.wrap("<p></p><div></div>");
		assert.equal(
		...
		assert.equal(
			j[0].parentNode.nodeName.toUpperCase(),
			"P",
			"The span should be in the paragraph."
		);

		// Wrap an element with a jQuery set
		j = jQuery("<span></span>").wrap(jQuery("<div></div>"));
		assert.equal(
			j[0].parentNode.nodeName.toLowerCase(),
			"div",
			"Wrapping works."
		);
	...
	QUnit.test("wrap(String) consecutive elements (#10177)", function (assert) {
		...
		targets.each(function () {
			...
			assert.equal(
				...
				0,
				"Each element should be wrapped individually"
			);
		});
	});
	...
	QUnit.test("wrapAll(String)", function (assert) {
		...
		assert.equal(
			jQuery("#first").parent().parent()[0].previousSibling,
			prev,
			"Correct Previous Sibling"
		);
		assert.equal(
			jQuery("#first").parent().parent()[0].parentNode,
			p,
			"Correct Parent"
		);
	});
	...
	QUnit.test("wrapAll(Function)", function (assert) {
		...
		assert.ok(
			jQuery("#first").parent().parent().parent().is(p),
			"Correct Parent"
		);
		assert.strictEqual(
			jQuery("#first").parent().parent()[0].previousSibling,
	...
	QUnit.test(
		"wrapAll(Function) check execution characteristics",
		function (assert) {
			assert.expect(3);
			...
			assert.ok(
				!i,
				"should not execute function argument if target element does not exist"
			);
			...
			jQuery("#firstp").wrapAll(function (index) {
				assert.strictEqual(
					this,
					jQuery("#firstp")[0],
					"context must be the first found element"
				);
				assert.strictEqual(
					index,
					undefined,
					"index argument should not be included in function execution"
				);
			});
		}
	);
	...
	QUnit.test("wrapAll(Element)", function (assert) {
		...
		assert.equal(
			jQuery("#first").parent()[0],
			jQuery("#firstp").parent()[0],
			"Same Parent"
		);
		assert.equal(
			jQuery("#first").parent()[0].previousSibling,
			prev,
			"Correct Previous Sibling"
		);
	...
	QUnit.test("wrapInner(String)", function (assert) {
		...
		assert.equal(
			jQuery("#first").children().children().children().length,
			num,
			"Verify Elements Intact"
		);

		num = jQuery("#first").html("foo<div>test</div><div>test2</div>").children()
			.length;
		jQuery("#first").wrapInner("<div class='red'><div id='tmp'></div></div>");
		assert.equal(jQuery("#first").children().length, 1, "Only one child");
		assert.ok(jQuery("#first").children().is(".red"), "Verify Right Element");
	...
	QUnit.test("wrapInner(Element)", function (assert) {
		...
		assert.equal(
			jQuery("#first").children().children().length,
			num,
			"Verify Elements Intact"
		);
		...
		assert.equal(
			div.children()[0].nodeName.toLowerCase(),
			"span",
			"A span was inserted."
		);
	});
	...
	QUnit.test("wrapInner(Function) returns String", function (assert) {
		...
		jQuery("#first").wrapInner(
			val("<div class='red'><div id='tmp'></div></div>")
		);
	...
	QUnit.test(
		"jQuery(<tag>) & wrap[Inner/All]() handle unknown elems (#10667)",
		function (assert) {
			assert.expect(2);

			var $wraptarget = jQuery("<div id='wrap-target'>Target</div>").appendTo(
					"#qunit-fixture"
				),
