...
QUnit.test("jQuery.propFix integrity test", function (assert) {
	...
	assert.deepEqual(
		props,
		jQuery.propFix,
		"jQuery.propFix passes integrity check"
	);
});
...
QUnit.test("attr(String)", function (assert) {
	...
	assert.equal(
		jQuery("#text1").attr("type"),
		"text",
		"Check for type attribute"
	);
	assert.equal(
		jQuery("#radio1").attr("type"),
		"radio",
		"Check for type attribute"
	);
	assert.equal(
		jQuery("#check1").attr("type"),
		"checkbox",
		"Check for type attribute"
	);
	assert.equal(
		jQuery("#simon1").attr("rel"),
		"bookmark",
		"Check for rel attribute"
	);
	assert.equal(
		jQuery("#google").attr("title"),
		"Google!",
		"Check for title attribute"
	);
	assert.equal(
		jQuery("#mark").attr("hreflang"),
		"en",
		"Check for hreflang attribute"
	);
	assert.equal(jQuery("#en").attr("lang"), "en", "Check for lang attribute");
	assert.equal(
		jQuery("#simon").attr("class"),
		"blog link",
		"Check for class attribute"
	);
	assert.equal(
		jQuery("#name").attr("name"),
		"name",
		"Check for name attribute"
	);
	assert.equal(
		jQuery("#text1").attr("name"),
		"action",
		"Check for name attribute"
	);
	assert.ok(
		jQuery("#form").attr("action").indexOf("formaction") >= 0,
		"Check for action attribute"
	);
	assert.equal(
		jQuery("#text1").attr("value", "t").attr("value"),
		"t",
		"Check setting the value attribute"
	);
	assert.equal(
		jQuery("#text1").attr("value", "").attr("value"),
		"",
		"Check setting the value attribute to empty string"
	);
	assert.equal(
		jQuery("<div value='t'></div>").attr("value"),
		"t",
		"Check setting custom attr named 'value' on a div"
	);
	assert.equal(
		jQuery("#form").attr("blah", "blah").attr("blah"),
		"blah",
		"Set non-existent attribute on a form"
	);
	assert.equal(
		jQuery("#foo").attr("height"),
		undefined,
	...
	assert.equal(
		jQuery("#form").attr("action", "newformaction").attr("action"),
		"newformaction",
		"Check that action attribute was changed"
	);
	assert.equal(
		jQuery("#testForm").attr("target"),
		undefined,
		"Retrieving target does not equal the input with name=target"
	);
	assert.equal(
		jQuery("#testForm").attr("target", "newTarget").attr("target"),
		"newTarget",
		"Set target successfully on a form"
	);
	assert.equal(
		jQuery("#testForm").removeAttr("id").attr("id"),
		undefined,
	...
	assert.equal(
		jQuery("#tAnchor5").attr("href"),
		"#5",
		"Check for non-absolute href (an anchor)"
	);
	jQuery("<a id='tAnchor6' href='#5'></a>").appendTo("#qunit-fixture");
	assert.equal(
		jQuery("#tAnchor5").prop("href"),
		jQuery("#tAnchor6").prop("href"),
		"Check for absolute href prop on an anchor"
	);

	jQuery(
		"<script type='jquery/test' src='#5' id='scriptSrc'></script>"
	).appendTo("#qunit-fixture");
	assert.equal(
		jQuery("#tAnchor5").prop("href"),
		jQuery("#scriptSrc").prop("src"),
	...
	assert.equal(
		jQuery("#list-test").attr("list"),
		"datalist",
		"Check setting list attribute"
	);
	...
	assert.equal(
		$body.attr("foo"),
		"baz",
		"Make sure the dom attribute is retrieved when no expando is found"
	);
	...
	assert.equal(
		jQuery(option).prop("selected"),
		true,
		"Make sure that a single option is selected, even when in an optgroup."
	);

	$img = jQuery(
		"<img style='display:none' width='215' height='53' src='" +
			baseURL +
			"1x1.jpg'/>"
	).appendTo("body");
	assert.equal(
		$img.attr("width"),
		"215",
		"Retrieve width attribute on an element with display:none."
	);
	assert.equal(
		$img.attr("height"),
		"53",
		"Retrieve height attribute on an element with display:none."
	);
	...
	assert.ok(
		!!~styleElem.attr("style").indexOf("UPPERlower.gif"),
		"Check style attribute getter"
	);
	assert.ok(
		!!~styleElem
	...
	assert.strictEqual(
		$button.attr("value"),
		undefined,
		"Absence of value attribute on a button"
	);
	assert.equal(
		$button.attr("value", "foobar").attr("value"),
		"foobar",
		"Value attribute on a button does not return innerHTML"
	);
	assert.equal(
		$button.attr("value", "baz").html(),
		"text",
	...
	assert.equal(
		$form.attr("class"),
		"something",
		"Retrieve the class attribute on a form."
	);

	$a = jQuery("<a href='#' onclick='something()'>Click</a>").appendTo(
		"#qunit-fixture"
	);
	assert.equal(
		$a.attr("onclick"),
		"something()",
		"Retrieve ^on attribute without anonymous function wrapper."
	);

	assert.ok(
		jQuery("<div></div>").attr("doesntexist") === undefined,
		"Make sure undefined is returned when no attribute is found."
	);
	assert.ok(
		jQuery("<div></div>").attr("title") === undefined,
		"Make sure undefined is returned when no attribute is found."
	);
	assert.equal(
		jQuery("<div></div>").attr("title", "something").attr("title"),
		"something",
		"Set the title attribute."
	);
	assert.ok(
		jQuery().attr("doesntexist") === undefined,
		"Make sure undefined is returned when no element is there."
	);
	assert.equal(
		jQuery("<div></div>").attr("value"),
		undefined,
		"An unset value on a div returns undefined."
	);
	assert.strictEqual(
		jQuery("<select><option value='property'></option></select>").attr("value"),
		undefined,
...
QUnit.test("attr(String) on cloned elements, #9646", function (assert) {
	...
	assert.strictEqual(
		input.clone(true).attr("name", "test")[0].name,
		"test",
	...
	assert.strictEqual(
		input.clone(true).attr("value", "test")[0].value,
		"test",
		"Value attribute should be changed on cloned element"
	);

	assert.strictEqual(
		input.clone(true).attr("value", 42)[0].value,
		"42",
		"Value attribute should be changed on cloned element"
	);
});
...
QUnit.test("attr(String) in XML Files", function (assert) {
	...
	assert.equal(
		jQuery("locations", xml).attr("class"),
		"foo",
		"Check class attribute in XML document"
	);
	assert.equal(
		jQuery("location", xml).attr("for"),
		"bar",
		"Check for attribute in XML document"
	);
	assert.equal(
		jQuery("location", xml).attr("checked"),
		"different",
...
QUnit.test("attr(Hash)", function (assert) {
	...
	jQuery("#qunit-fixture div")
		...
		.each(function () {
			if (
				this.getAttribute("foo") !== "baz" &&
				this.getAttribute("zoo") !== "ping"
			) {
				pass = false;
			}
		});
...
QUnit.test("attr(String, Object)", function (assert) {
	...
	assert.equal(
		fail,
		false,
		"Set Attribute, the #" + fail + " element didn't get the attribute 'foo'"
	);
	...
	assert.equal(
		$input.attr("name"),
		"something",
		"Check element creation gets/sets the name attribute."
	);
	assert.equal(
		$input.attr("id"),
		"specified",
		"Check element creation gets/sets the id attribute."
	);
	...
	assert.equal(
		$input.prop("checked"),
		true,
		"Setting checked updates property (verified by .prop)"
	);
	assert.equal(
		$input[0].checked,
		true,
		"Setting checked updates property (verified by native property)"
	);
	$input = jQuery("<option></option>").attr("selected", true);
	assert.equal(
		$input.prop("selected"),
		true,
		"Setting selected updates property (verified by .prop)"
	);
	assert.equal(
		$input[0].selected,
		true,
		"Setting selected updates property (verified by native property)"
	);
	...
	assert.equal(
		$input.attr("checked"),
		"checked",
		"Set checked to 'checked' (verified by .attr)"
	);

	$radios = jQuery("#checkedtest").find("input[type='radio']");
	$radios.eq(1).trigger("click");
	assert.equal(
		$radios.eq(1).prop("checked"),
		true,
		"Second radio was checked when clicked"
	);
	assert.equal(
		$radios.eq(0).attr("checked"),
		"checked",
		"First radio is still [checked]"
	);
	...
	assert.equal(
		$input[0].readOnly,
		true,
		"Set readonly property (verified by native property)"
	);
	assert.equal(
		$input.prop("readOnly"),
		true,
		"Set readonly property (verified by .prop)"
	);
	$input.attr("readonly", true).prop("readOnly", false);
	assert.equal(
		$input[0].readOnly,
		false,
		"Clear readonly property (verified by native property)"
	);
	assert.equal(
		$input.prop("readOnly"),
		false,
		"Clear readonly property (verified by .prop)"
	);
	...
	assert.equal(
		$input[0].maxLength,
		5,
		"Set maxlength (verified by native property)"
	);
	...
	assert.equal(
		$text.attr("autofocus", false).attr("autofocus"),
		undefined,
	...
	assert.equal(
		$text.attr("required", false).attr("required"),
		undefined,
	...
	assert.equal(
		$details.attr("open"),
		"open",
		"open attribute presence indicates true"
	);
	assert.equal(
		$details.attr("open", false).attr("open"),
		undefined,
	...
	assert.equal(
		$text.attr("aria-disabled", false).attr("aria-disabled"),
		"false",
		"Setting aria attributes are not affected by boolean settings"
	);
	$text
		.removeData("something")
		.removeData("another")
		.removeAttr("aria-disabled");

	jQuery("#foo").attr("contenteditable", true);
	assert.equal(
		jQuery("#foo").attr("contenteditable"),
		"true",
		"Enumerated attributes are set properly"
	);
	...
	assert.equal(
		table[0]["cellSpacing"],
		"2",
		"Check cellspacing is correctly set"
	);

	assert.equal(
		jQuery("#area1").attr("value"),
		undefined,
		"Value attribute is distinct from value property."
	);

	// for #1070
	jQuery("#name").attr("someAttr", "0");
	assert.equal(
		jQuery("#name").attr("someAttr"),
		"0",
		"Set attribute to a string of '0'"
	);
	jQuery("#name").attr("someAttr", 0);
	assert.equal(
		jQuery("#name").attr("someAttr"),
		"0",
		"Set attribute to the number 0"
	);
	jQuery("#name").attr("someAttr", 1);
	assert.equal(
		jQuery("#name").attr("someAttr"),
		"1",
		"Set attribute to the number 1"
	);
	...
	assert.equal(
		j.attr("name"),
		"attrvalue",
		"Check node,textnode,comment for attr"
	);
	j.removeAttr("name");
	...
	assert.equal(
		$radio.val(),
		"sup",
		"Value is not reset when type is set after value on a radio"
	);
	...
	assert.equal(
		$svg.attr("cx", 100).attr("cx"),
		"100",
		"Set attribute on svg element"
	);
	$svg.remove();
	...
	assert.equal(
		typeof jQuery("#name").attr("maxlength", undefined),
		"object",
		".attr('attribute', undefined) is chainable (#5571)"
	);
	assert.equal(
		jQuery("#name").attr("maxlength", undefined).attr("maxlength"),
		"5",
		".attr('attribute', undefined) does not change value (#5571)"
	);
	assert.equal(
		jQuery("#name").attr("nonexisting", undefined).attr("nonexisting"),
		undefined,
...
QUnit.test("attr(non-ASCII)", function (assert) {
	...
	var $div = jQuery("<div Ω='omega' aØc='alpha'></div>").appendTo(
		"#qunit-fixture"
	);
...
QUnit.test("attr(String, Object) - Loaded via XML document", function (assert) {
	...
	assert.equal(
		titles[0],
		"Location",
		"attr() in XML context: Check first title"
	);
...
QUnit.test("attr('tabindex')", function (assert) {
	...
	assert.equal(
		jQuery("#listWithTabIndex").attr("tabindex"),
		"5",
		"not natively tabbable, with tabindex set to 0"
	);
	assert.equal(
		jQuery("#divWithNoTabIndex").attr("tabindex"),
		undefined,
	...
	// anchor with href
	assert.equal(
		jQuery("#linkWithNoTabIndex").attr("tabindex"),
		undefined,
		"anchor with href, no tabindex set"
	);
	assert.equal(
		jQuery("#linkWithTabIndex").attr("tabindex"),
		"2",
		"anchor with href, tabindex set to 2"
	);
	assert.equal(
		jQuery("#linkWithNegativeTabIndex").attr("tabindex"),
		"-1",
	...
	// anchor without href
	assert.equal(
		jQuery("#linkWithNoHrefWithNoTabIndex").attr("tabindex"),
		undefined,
		"anchor without href, no tabindex set"
	);
	assert.equal(
		jQuery("#linkWithNoHrefWithTabIndex").attr("tabindex"),
		"1",
		"anchor without href, tabindex set to 2"
	);
	assert.equal(
		jQuery("#linkWithNoHrefWithNegativeTabIndex").attr("tabindex"),
		"-1",
		"anchor without href, no tabindex set"
	);
});
...
QUnit.test("removeAttr(String)", function (assert) {
	...
	assert.equal(
		jQuery("<div class='hello'></div>").removeAttr("class").attr("class"),
		undefined,
		"remove class"
	);
	assert.equal(
		jQuery("#form").removeAttr("id").attr("id"),
		undefined,
		"Remove id"
	);
	assert.equal(
	...
	assert.equal(
		jQuery("#fx-test-group").attr("height", "3px").removeAttr("height").get(0)
			.style.height,
		"1px",
	...
	assert.equal(
		document.getElementById("text1").readOnly,
		false,
	...
	assert.equal(
		$first.attr("Case"),
		undefined,
		"mixed-case attribute was removed"
	);
});
...
QUnit.test("removeAttr(Multi String, variable space width)", function (assert) {
	...
	jQuery.each(tests, function (key, val) {
		assert.equal(
			div.attr(key),
			val,
			"Attribute `" + key + "` exists, and has a value of `" + val + "`"
		);
	});
...
QUnit.test(
	"removeAttr(Multi String, non-HTML whitespace is valid in attribute names (gh-3003)",
	function (assert) {
		assert.expect(8);
...
QUnit.test("prop(String, Object)", function (assert) {
	...
	assert.equal(
		jQuery("#text1").prop("value"),
		"Test",
		"Check for value attribute"
	);
	assert.equal(
		jQuery("#text1").prop("value", "Test2").prop("defaultValue"),
		"Test",
		"Check for defaultValue attribute"
	);
	assert.equal(
		jQuery("#select2").prop("selectedIndex"),
		3,
		"Check for selectedIndex attribute"
	);
	assert.equal(
		jQuery("#foo").prop("nodeName").toUpperCase(),
		"DIV",
		"Check for nodeName attribute"
	);
	assert.equal(
		jQuery("#foo").prop("tagName").toUpperCase(),
		"DIV",
		"Check for tagName attribute"
	);
	assert.equal(
		jQuery("<option></option>").prop("selected"),
		false,
		"Check selected attribute on disconnected element."
	);

	assert.equal(
		jQuery("#listWithTabIndex").prop("tabindex"),
		5,
		"Check retrieving tabindex"
	);
	...
	assert.equal(
		jQuery("#label-for").prop("for"),
		"action",
		"Check retrieving htmlFor"
	);
	jQuery("#text1").prop("class", "test");
	assert.equal(
		document.getElementById("text1").className,
		"test",
		"Check setting className with 'class'"
	);
	assert.equal(
		jQuery("#text1").prop("maxlength"),
		30,
		"Check retrieving maxLength"
	);
	jQuery("#table").prop("cellspacing", 1);
	assert.equal(
		jQuery("#table").prop("cellSpacing"),
		"1",
		"Check setting and retrieving cellSpacing"
	);
	jQuery("#table").prop("cellpadding", 1);
	assert.equal(
		jQuery("#table").prop("cellPadding"),
		"1",
		"Check setting and retrieving cellPadding"
	);
	jQuery("#table").prop("rowspan", 1);
	assert.equal(
		jQuery("#table").prop("rowSpan"),
		1,
		"Check setting and retrieving rowSpan"
	);
	jQuery("#table").prop("colspan", 1);
	assert.equal(
		jQuery("#table").prop("colSpan"),
		1,
		"Check setting and retrieving colSpan"
	);
	jQuery("#table").prop("usemap", 1);
	assert.equal(
		jQuery("#table").prop("useMap"),
		1,
		"Check setting and retrieving useMap"
	);
	jQuery("#table").prop("frameborder", 1);
	assert.equal(
		jQuery("#table").prop("frameBorder"),
		1,
		"Check setting and retrieving frameBorder"
	);
});
...
QUnit.test("prop(String, Object) on null/undefined", function (assert) {
	...
	assert.equal(
		$body.prop("foo"),
		"bar",
		"Make sure the expando is preferred over the dom attribute"
	);
	body["foo"] = undefined;
	assert.ok(
		$body.prop("foo") === undefined,
	...
	obj = {};
	jQuery.each(
		[document, attributeNode, commentNode, textNode, obj, "#firstp"],
		function (i, ele) {
			assert.strictEqual(
				jQuery(ele).prop("nonexisting"),
				undefined,
...
QUnit.test("prop('tabindex')", function (assert) {
	...
	assert.equal(
		jQuery("#inputWithoutTabIndex").prop("tabindex"),
		0,
		"input without tabindex"
	);
	assert.equal(
		jQuery("#buttonWithoutTabIndex").prop("tabindex"),
		0,
		"button without tabindex"
	);
	assert.equal(
		jQuery("#textareaWithoutTabIndex").prop("tabindex"),
		0,
		"textarea without tabindex"
	);
	...
	assert.equal(
		jQuery("#divWithNoTabIndex").prop("tabindex"),
		-1,
		"not natively tabbable, no tabindex set"
	);
	...
	// anchor with href
	assert.equal(
		jQuery("#linkWithNoTabIndex").prop("tabindex"),
		0,
		"anchor with href, no tabindex set"
	);
	assert.equal(
		jQuery("#linkWithTabIndex").prop("tabindex"),
		2,
		"anchor with href, tabindex set to 2"
	);
	assert.equal(
		jQuery("#linkWithNegativeTabIndex").prop("tabindex"),
		-1,
		"anchor with href, tabindex set to -1"
	);
	...
	// anchor without href
	assert.equal(
		jQuery("#linkWithNoHrefWithNoTabIndex").prop("tabindex"),
		-1,
		"anchor without href, no tabindex set"
	);
	assert.equal(
		jQuery("#linkWithNoHrefWithTabIndex").prop("tabindex"),
		1,
		"anchor without href, tabindex set to 2"
	);
	assert.equal(
		jQuery("#linkWithNoHrefWithNegativeTabIndex").prop("tabindex"),
		-1,
		"anchor without href, no tabindex set"
	);
});
...
QUnit.test("image.prop( 'tabIndex' )", function (assert) {
	assert.expect(1);
	var image = jQuery("<img src='" + baseURL + "1x1.jpg' />").appendTo(
		"#qunit-fixture"
	);
...
QUnit.test("prop('tabindex', value)", function (assert) {
	...
	assert.equal(
		clone[0].getAttribute("tabindex"),
		"1",
		"set tabindex on cloned element"
	);
});

QUnit.test(
	"option.prop('selected', true) affects select.selectedIndex (gh-2732)",
	function (assert) {
		assert.expect(2);
		...
		assert.equal(
			...
			1,
			"Setting option selected affects selectedIndex"
		);
		...
		assert.equal(
			$select[0].selectedIndex,
			1,
			"Setting option in optgroup selected affects selectedIndex"
		);
	}
);
...
QUnit.test("removeProp(String)", function (assert) {
	...
	assert.strictEqual(
		jQuery("#firstp").prop("nonexisting", "foo").removeProp("nonexisting")[0][
			"nonexisting"
		],
		undefined,
...
QUnit.test("val()", function (assert) {
	...
	assert.equal(
		jQuery("#select2").val(),
		"3",
		"Call val() on a single='single' select"
	);

	assert.deepEqual(
		jQuery("#select3").val(),
		["1", "2"],
		"Call val() on a multiple='multiple' select"
	);

	assert.equal(
		jQuery("#option3c").val(),
		"2",
		"Call val() on a option element with value"
	);

	assert.equal(
		jQuery("#option3a").val(),
		"",
		"Call val() on a option element with empty value"
	);

	assert.equal(
		jQuery("#option3e").val(),
		"no value",
		"Call val() on a option element with no value attribute"
	);
	...
	jQuery("#select4 optgroup")
		.add("#select4 > [disabled]")
		.attr("disabled", false);
	assert.deepEqual(
		jQuery("#select4").val(),
		["2", "3"],
	...
	assert.deepEqual(
		jQuery("#select4").val(),
		["2", "3"],
		"Call val() on disabled multiple='multiple' select"
	);
	...
	if (jQuery.fn.serialize) {
		...
		assert.equal(
			checks.eq(3).val(),
			"on",
			"Make sure a value of 'on' is provided if none is specified."
		);
	...
	$button = jQuery("<button value='foobar'>text</button>").insertAfter(
		"#button"
	);
	assert.equal(
		$button.val(),
		"foobar",
		"Value retrieval on a button does not return innerHTML"
	);
	assert.equal(
		$button.val("baz").html(),
		"text",
		"Setting the value does not change innerHTML"
	);

	assert.equal(
		jQuery("<option></option>").val("test").attr("value"),
		"test",
...
QUnit.test(
	"val() with non-matching values on dropdown list",
	function (assert) {
		assert.expect(3);
		...
		assert.equal(
			jQuery("#select5").val(),
			null,
			"Non-matching set on select-one"
		);
		...
		assert.deepEqual(
			jQuery(select6).val(),
			[],
			"Non-matching set (single value) on select-multiple"
		);
		...
		assert.deepEqual(
			jQuery(select6).val(),
			[],
			"Non-matching set (array of values) on select-multiple"
		);
...
if (
	...
	QUnit.test(
		"val() respects numbers without exception (Bug #9319)",
		function (assert) {
			assert.expect(4);
			...
			try {
				assert.equal(
					typeof $meter.val(),
					"number",
					"meter, returns a number and does not throw exception"
				);
				assert.equal(
					$meter.val(),
					$meter[0].value,
					"meter, api matches host and does not throw exception"
				);

				assert.equal(
					typeof $progress.val(),
					"number",
					"progress, returns a number and does not throw exception"
				);
				assert.equal(
					$progress.val(),
					$progress[0].value,
...
var testVal = function (valueObj, assert) {
	...
	assert.equal(
		$select1.val(),
		"2",
		"Check for modified (via val(Number)) value of select element"
	);
	...
	assert.equal(
		$select1.val(),
		"4",
		"Should be possible to set the val() to a newly created option"
	);
...
QUnit.test("val(select) after form.reset() (Bug #2551)", function (assert) {
	...
	assert.equal(
		jQuery("#kkk")[0].value,
		"cf",
		"Check value of select after form reset."
	);
	assert.equal(
		jQuery("#kkk").val(),
		"cf",
...
QUnit.test("select.val(space characters) (gh-2978)", function (assert) {
	...
	jQuery.each(spaces, function (key, obj) {
		...
		assert.equal(
			$select.val(),
			"attr" + val,
			"Value ending with space character (" + key + ") selected (attr)"
		);
		...
		assert.equal(
			$select.val(),
			"at" + val + "tr",
			"Value with space character (" + key + ") in the middle selected (attr)"
		);
		...
		assert.equal(
			$select.val(),
			val + "attr",
			"Value starting with space character (" + key + ") selected (attr)"
		);
	});
	...
	jQuery.each(spaces, function (key, obj) {
		...
		if (/^\\u/.test(key)) {
			...
			assert.equal(
				$select.val(),
				val + "text",
				"Value with non-HTML space character at beginning is not stripped (" +
					key +
					") selected (" +
					key +
					"text)"
			...
			assert.equal(
				$select.val(),
				"te" + val + "xt",
				"Value with non-space whitespace character (" +
					key +
			...
			assert.equal(
				$select.val(),
				"text" + val,
				"Value with non-HTML space character at end is not stripped (" +
					key +
					") selected (text" +
					key +
					")"
			...
			assert.equal(
				$select.val(),
				"text",
				"Value with HTML space character at beginning or end is stripped (" +
					key +
					") selected (text)"
			);
			$select.val("te xt");
			assert.equal(
				$select.val(),
				"te xt",
				"Value with space character (" + key + ") in the middle selected (text)"
			);
		}
	});
});
...
var testAddClass = function (valueObj, assert) {
	...
	assert.equal(
		div.attr("class"),
		"foo",
		"Do not add the same class twice in separate calls."
	);
	...
	assert.equal(
		div.attr("class"),
		"foo fo",
		"Adding a similar class does not get interrupted."
	);
	div.removeClass().addClass("wrap2");
	assert.ok(
		div.addClass("wrap").hasClass("wrap"),
		"Can add similarly named classes"
	);
	...
	assert.equal(
		div.attr("class"),
		"bar",
		"Do not add the same class twice in the same call."
	);
};
...
var testRemoveClass = function (valueObj, assert) {
	...
	$set
		.removeClass(valueObj("test"))
		.removeClass(valueObj("bar"))
		.removeClass(valueObj("foo"));
	...
	assert.strictEqual(
		...
		undefined,
		"removeClass doesn't create a class attribute"
	);
	...
	assert.equal(
		div.className,
		"test",
		"Make sure remaining className is trimmed."
	);
	...
	assert.equal(
		div.className,
		"",
		"Make sure there is nothing left after everything is removed."
	);
};
...
QUnit.test("removeClass() removes duplicates", function (assert) {
	...
	assert.ok(
		!$div.hasClass("x"),
		"Element with multiple same classes does not escape the wrath of removeClass()"
	);
});
...
QUnit.test("removeClass(undefined) is a no-op", function (assert) {
	...
	assert.ok(
		$div.hasClass("base") && $div.hasClass("second"),
...
var testToggleClass = function (valueObj, assert) {
	...
	assert.ok(
		jQuery._data(e[0], "__className__") === "testD testE",
...
QUnit.test(
	"toggleClass(String|boolean|undefined[, boolean])",
	function (assert) {
...
QUnit.test(
	"toggleClass(Function[, boolean]) with incoming value",
	function (assert) {
		assert.expect(14);
...
QUnit.test("addClass, removeClass, hasClass", function (assert) {
	...
	assert.ok(
		jq.hasClass("class2") === false,
		"Check the class has been properly removed"
	);
	jq.removeClass("cla");
	assert.ok(
		jq.hasClass("cla.ss3"),
		"Check the dotted class has not been removed"
	);
	jq.removeClass("cla.ss3");
	assert.ok(
		jq.hasClass("cla.ss3") === false,
		"Check the dotted class has been removed"
	);
	jq.removeClass("class4");
	assert.ok(
		jq.hasClass("class4") === false,
		"Check the class has been properly removed"
	);
});

QUnit.test(
	"addClass, removeClass, hasClass on many elements",
	function (assert) {
		assert.expect(19);
		...
		assert.ok(
			jQuery("<p class='hi'>p0</p><p>p1</p><p>p2</p>").hasClass("hi"),
			"Did find a class in the first element"
		);
		assert.ok(
			jQuery("<p>p0</p><p class='hi'>p1</p><p>p2</p>").hasClass("hi"),
			"Did find a class in the second element"
		);
		assert.ok(
			jQuery("<p>p0</p><p>p1</p><p class='hi'>p2</p>").hasClass("hi"),
			"Did find a class in the last element"
		);

		assert.ok(
			jQuery(
				"<p class='hi'>p0</p><p class='hi'>p1</p><p class='hi'>p2</p>"
			).hasClass("hi"),
		...
		assert.ok(
			!jQuery(
				"<p class='hi0'>p0</p><p class='hi1'>p1</p><p class='hi2'>p2</p>"
			).hasClass("hi"),
...
QUnit.test(
	"addClass, removeClass, hasClass on many elements - Array",
	function (assert) {
		assert.expect(16);
...
QUnit.test(
	...
	function (assert) {
		...
		function testMatches() {
			...
			assert.ok(
				$elem.is(".\\A0test"),
				"Element matches with non-breaking space"
			);
			assert.ok(
				$elem.hasClass("\xA0test"),
				"Element has class with non-breaking space"
			);
		}
...
QUnit.test("contents().hasClass() returns correct values", function (assert) {
	...
	assert.ok(
		!$contents.hasClass("undefined"),
		"Did not find 'undefined' in $contents (correctly)"
	);
});

QUnit.test(
	"hasClass correctly interprets non-space separators (#13835)",
	function (assert) {
		assert.expect(4);
...
QUnit.test(
	"coords returns correct values in IE6/IE7, see #10828",
	function (assert) {
		assert.expect(1);
		...
		area = map
			.html("<area shape='rect' coords='0,0,0,0' href='#' alt='a'></area>")
			.find("area");
		assert.equal(
			area.attr("coords"),
			"0,0,0,0",
			"did not retrieve coords correctly"
		);
	}
);
...
QUnit.test(
	"option value not trimmed when setting via parent select",
	function (assert) {
		assert.expect(1);
		assert.equal(
...
QUnit.test(
	"Insignificant white space returned for $(option).val() (#14858, gh-2978)",
	function (assert) {
		assert.expect(16);
		...
		jQuery.each([" ", "\n", "\t", "\f", "\r"], function (i, character) {
			...
			assert.equal(
				val.length,
				0,
				"insignificant white-space returned for value"
			);

			val = jQuery(
				"<option>" + character + "test" + character + "</option>"
			).val();
			assert.equal(
				val.length,
				4,
...
QUnit.test("SVG class manipulation (gh-2199)", function (assert) {
	...
	jQuery.each(["svg", "rect", "g"], function () {
		...
		assert.ok(
			elem.hasClass("awesome"),
			"SVG element (" + this + ") has added class"
		);

		elem.removeClass("awesome");
		assert.ok(
			!elem.hasClass("awesome"),
			"SVG element (" + this + ") removes the class"
		);

		elem.toggleClass("awesome");
		assert.ok(
			elem.hasClass("awesome"),
			"SVG element (" + this + ") toggles the class on"
		);

		elem.toggleClass("awesome");
		assert.ok(
			!elem.hasClass("awesome"),
			"SVG element (" + this + ") toggles the class off"
		);
	});
});

QUnit.test(
	"non-lowercase boolean attribute getters should not crash",
	function (assert) {
		assert.expect(3);
			...
			function (lowercased, original) {
				try {
					assert.strictEqual(
						...
						"The '" +
							this +
							"' attribute getter should return the lowercased name"
					);
				} catch (e) {
