(function () {
	...
	var supportsFixedPosition,
		supportsScroll,
		alwaysScrollable,
		forceScroll = supportjQuery("<div></div>").css({
			width: 2000,
			height: 2000,
		}),
	...
	QUnit.test("empty set", function (assert) {
		...
		assert.strictEqual(
			jQuery().offset(),
			undefined,
			"offset() returns undefined for empty set (#11962)"
		);
		assert.strictEqual(
			jQuery().position(),
			undefined,
			"position() returns undefined for empty set (#11962)"
		);
	});
	...
	QUnit.test("disconnected element", function (assert) {
		...
		assert.equal(
			result.top,
			0,
			"Retrieving offset on disconnected elements returns zeros (gh-2310)"
		);
		assert.equal(
			result.left,
			0,
		...
		assert.equal(
			...
			2,
			"Retrieving offset on disconnected elements returns offset object (gh-3167)"
		);
		assert.equal(
			jQuery.isPlainObject(result),
			true,
			"Retrieving offset on disconnected elements returns plain object (gh-3612)"
		);
	});
	...
	QUnit.test("hidden (display: none) element", function (assert) {
		...
		var node = jQuery("<div style='display: none'></div>").appendTo(
				"#qunit-fixture"
			),
			result = node.offset();
		...
		assert.equal(
			result.top,
			0,
			"Retrieving offset on hidden elements returns zeros (gh-2310)"
		);
		assert.equal(
			result.left,
			0,
		...
		assert.equal(
			Object.keys(result).length,
			2,
			"Retrieving offset on hidden elements returns offset object (gh-3167)"
		);
		assert.equal(
			jQuery.isPlainObject(result),
			true,
			"Retrieving offset on hidden elements returns plain object (gh-3612)"
		);
	});
	...
	QUnit.test("0 sized element", function (assert) {
		...
		var node = jQuery(
				"<div style='margin: 5px; width: 0; height: 0'></div>"
			).appendTo("#qunit-fixture"),
			result = node.offset();
		...
		assert.notEqual(
			result.top,
			0,
			"Retrieving offset on 0 sized elements (gh-3167)"
		);
		assert.notEqual(
			result.left,
			0,
		...
		assert.equal(
			Object.keys(result).length,
			2,
			"Retrieving offset on 0 sized elements returns offset object (gh-3167)"
		);
		assert.equal(
			jQuery.isPlainObject(result),
			true,
			"Retrieving offset on 0 sized elements returns plain object (gh-3612)"
		);
	});
	...
	QUnit.test("hidden (visibility: hidden) element", function (assert) {
		...
		var node = jQuery(
				"<div style='margin: 5px; visibility: hidden'></div>"
			).appendTo("#qunit-fixture"),
			result = node.offset();
		...
		assert.notEqual(
			result.top,
			0,
			"Retrieving offset on visibility:hidden elements (gh-3167)"
		);
		assert.notEqual(
			result.left,
			0,
		...
		assert.equal(
			Object.keys(result).length,
			2,
			"Retrieving offset on visibility:hidden elements returns offset object (gh-3167)"
		);
		assert.equal(
			jQuery.isPlainObject(result),
			true,
			"Retrieving offset on visibility:hidden elements returns plain object (gh-3612)"
		);
	});
	...
	QUnit.test("normal element", function (assert) {
		...
		assert.equal(
			Object.keys(offset).length,
			2,
			"Retrieving offset on normal elements returns offset object (gh-3612)"
		);
		assert.equal(
			jQuery.isPlainObject(offset),
			true,
			"Retrieving offset on normal elements returns plain object (gh-3612)"
		);

		assert.equal(
			Object.keys(position).length,
			2,
			"Retrieving position on normal elements returns offset object (gh-3612)"
		);
		assert.equal(
			jQuery.isPlainObject(position),
			true,
			"Retrieving position on normal elements returns plain object (gh-3612)"
		);
	});
	...
	testIframe("absolute", "offset/absolute.html", function (assert, $, iframe) {
		...
		// get offset
		tests = [{ id: "#absolute-1", top: 1, left: 1 }];
		jQuery.each(tests, function () {
			assert.equal(
				jQuery(this.id, doc).offset().top,
				this.top,
				"jQuery('" + this.id + "').offset().top"
			);
			assert.equal(
				jQuery(this.id, doc).offset().left,
				this.left,
		...
		// get position
		tests = [{ id: "#absolute-1", top: 0, left: 0 }];
		jQuery.each(tests, function () {
			assert.equal(
				jQuery(this.id, doc).position().top,
				this.top,
				"jQuery('" + this.id + "').position().top"
			);
			assert.equal(
				jQuery(this.id, doc).position().left,
				this.left,
	...
	testIframe("absolute", "offset/absolute.html", function (assert, $) {
		...
		jQuery.each(tests, function () {
			assert.equal(
				$(this.id).position().top,
				this.top,
				"jQuery('" + this.id + "').position().top"
			);
			assert.equal(
				$(this.id).position().left,
				this.left,
				"jQuery('" + this.id + "').position().left"
			);
		});
	...
	testIframe("static", "offset/static.html", function (assert, $) {
		...
		jQuery.each(tests, function () {
			assert.equal(
				$(this.id).position().top,
				this.top,
				"jQuery('" + this.top + "').position().top"
			);
			assert.equal(
				$(this.id).position().left,
				this.left,
				"jQuery('" + this.left + "').position().left"
			);
		});
	...
	testIframe("fixed", "offset/fixed.html", function (assert, $, window) {
		...
		jQuery.each(tests, function () {
			if (!window.supportsScroll) {
				...
				assert.equal(
					jQuery.isPlainObject($(this.id).offset()),
					true,
					"jQuery('" + this.id + "').offset() is plain object"
				);
				assert.equal(
					jQuery.isPlainObject($(this.id).position()),
					true,
					"jQuery('" + this.id + "').position() is plain object"
				);
				assert.equal(
					$(this.id).offset().top,
					this.offsetTop,
		...
		if (window.supportsFixedPosition) {
			assert.equal(
				$noTopLeft.offset().top,
				1007,
				"Check offset top for fixed element with no top set"
			);
			assert.equal(
				$noTopLeft.offset().left,
				1007,
				"Check offset left for fixed element with no left set"
			);
		} else {
			// need to have same number of assertions
			assert.ok(true, "Fixed position is not supported");
	...
	testIframe("table", "offset/table.html", function (assert, $) {
		...
		assert.equal(
			$("#table-1").offset().top,
			6,
			"jQuery('#table-1').offset().top"
		);
		assert.equal(
			$("#table-1").offset().left,
			6,
			"jQuery('#table-1').offset().left"
		);
	...
	testIframe("scroll", "offset/scroll.html", function (assert, $, win) {
		...
		assert.equal(
			$("#scroll-1").offset().left,
			7,
			"jQuery('#scroll-1').offset().left"
		);

		assert.equal(
			$("#scroll-1-1").offset().top,
			11,
			"jQuery('#scroll-1-1').offset().top"
		);
		assert.equal(
			$("#scroll-1-1").offset().left,
			11,
		...
		assert.equal(
			$("#hidden").offset().top,
			0,
			"Hidden elements do not subtract scroll"
		);
		assert.equal(
			$("#hidden").offset().left,
			0,
		...
		assert.equal(
			$("#scroll-1-1").scrollTop(),
			0,
			"jQuery('#scroll-1-1').scrollTop()"
		);
		assert.equal(
			$("#scroll-1-1").scrollLeft(),
			0,
		...
		assert.equal(
			$("#scroll-1").scrollTop(undefined).scrollTop(),
			5,
			".scrollTop(undefined) is chainable (#5571)"
		);
		assert.equal(
			$("#scroll-1").scrollLeft(undefined).scrollLeft(),
			5,
		...
		if (!window.supportsScroll) {
			...
			assert.equal(
				$(win.document).scrollTop(),
				1000,
				"jQuery(document).scrollTop()"
			);
			assert.equal(
				$(win.document).scrollLeft(),
				1000,
		...
		assert.strictEqual(
			$().scrollTop(),
			undefined,
			"jQuery().scrollTop() testing getter on empty jquery object"
		);
		assert.strictEqual(
			$().scrollLeft(),
			undefined,
			"jQuery().scrollLeft() testing getter on empty jquery object"
		);
	});
	...
	QUnit.test("chaining", function (assert) {
		...
		assert.equal(
			jQuery("#absolute-1").offset(coords).jquery,
			jQuery.fn.jquery,
			"offset(coords) returns jQuery object"
		);
		assert.equal(
			jQuery("#non-existent").offset(coords).jquery,
			jQuery.fn.jquery,
			"offset(coords) with empty jQuery set returns jQuery object"
		);
		assert.equal(
			jQuery("#absolute-1").offset(undefined).jquery,
			jQuery.fn.jquery,
	...
	(function () {
		var POSITION_VALUES = ["static", "relative", "absolute", "fixed"],
			...
			htmlProps = function (position) {
				return props(
					position === "static" ? 0 : 4096,
					position === "static" ? 0 : 2048,
					64,
					32,
					128,
					64,
		...
		function getExpectations(htmlPos, bodyPos, scrollTop, scrollLeft) {
				...
				for (; parent; parent = expectations[parent.parent]) {
					...
					if (props.style !== "absolute") {
						...
						if (parent.style === "static") {
							...
							pos.left +=
								parent.pos.left + parent.marginLeft + parent.borderLeft;
						}
					}
					break;
				}
			...
			if (/MSIE (?:9|10)\./.test(navigator.userAgent)) {
				expectations.documentElement.pos.top -=
					expectations.documentElement.marginTop - scrollTop;
				expectations.documentElement.offset.top -=
					expectations.documentElement.marginTop - scrollTop;
				expectations.documentElement.pos.left -=
					expectations.documentElement.marginLeft - scrollLeft;
				expectations.documentElement.offset.left -=
					expectations.documentElement.marginLeft - scrollLeft;
				if (htmlPos !== "static") {
		...
		supportjQuery.each(POSITION_VALUES, function (_, htmlPos) {
			supportjQuery.each(POSITION_VALUES, function (_, bodyPos) {
				var label =
					"nonzero box properties - html." + htmlPos + " body." + bodyPos;
				testIframe(label, "offset/boxes.html", function (assert, $, win, doc) {
					...
					supportjQuery.each(expectations, function (id, descriptor) {
						assert.deepEqual(
							...
							"jQuery('#" +
								id +
								"').offset(): top " +
								descriptor.offset.top +
								", left " +
								descriptor.offset.left
						);
					});
					...
					supportjQuery.each(
						Object.keys(expectations).reverse(),
						function (_, id) {
							var $el = $("#" + id),
							...
							assert.deepEqual(
								supportjQuery.extend({}, $el.position()),
								pos,
	...
	QUnit.test("offsetParent", function (assert) {
		...
		assert.equal(
			div[0],
			jQuery("#nothiddendiv")[0],
			"The div is the offsetParent."
		);
		...
		area = jQuery(
			'<map name="imgmap"><area shape="rect" coords="0,0,200,50"></map>'
		)
			.appendTo("body")
			.find("area");
		assert.equal(
	...
	QUnit.test("fractions (see #7730 and #7885)", function (assert) {
		...
		assert.ok(
			Math.abs(result.top - expected.top) < 0.25,
			"Check top within 0.25 of expected"
		);
		assert.ok(
			Math.abs(result.left - expected.left) < 0.25,
			"Check left within 0.25 of expected"
		);
	...
	QUnit.test("iframe scrollTop/Left (see gh-1945)", function (assert) {
		...
		if (
			...
			assert.equal(
				jQuery(ifDoc).scrollTop(),
				200,
				"$($('#iframe')[0].contentDocument).scrollTop()"
			);
			assert.equal(
				jQuery(ifDoc).scrollLeft(),
				500,
				"$($('#iframe')[0].contentDocument).scrollLeft()"
			);
		}
	});
})();
