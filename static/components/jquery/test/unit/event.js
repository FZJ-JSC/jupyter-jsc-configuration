...
QUnit.test("on(), with data", function (assert) {
	...
	handler = function (event) {
		...
		assert.equal(
			event.data.foo,
			"bar",
			"on() with data, Check value of passed data"
		);
	};
	jQuery("#firstp")
		.on("click", { foo: "bar" }, handler)
		.trigger("click")
		.off("click", handler);
...
QUnit.test("on(), five events at once", function (assert) {
	...
	jQuery("#firstp")
		.on("click mouseover foo bar baz", handler)
		.trigger("click")
		.trigger("mouseover")
		.trigger("foo")
		.trigger("bar")
	...
	jQuery.event.special.test = {
		_default: function (e, data) {
			assert.equal(
				e.type,
				"test",
				"Make sure we're dealing with a test event."
			);
...
QUnit.test(
	"on bubbling, isDefaultPrevented, stopImmediatePropagation",
	function (assert) {
		assert.expect(3);
...
QUnit.test(
	"triggered events stopPropagation() for natively-bound events",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"trigger() works with events that were previously stopped",
	function (assert) {
		assert.expect(0);
...
QUnit.test("on(), namespaced events, cloned events", function (assert) {
	...
	jQuery("#nonnodes")
		...
		.on("tester", function () {
			assert.equal(
				this.nodeType,
				1,
				"Check node,textnode,comment on just does real nodes"
			);
		})
		.trigger("tester");
	...
	jQuery("<a href='#fail' class='test'>test</a>")
		...
		.appendTo("#qunit-fixture");
	assert.ok(
		jQuery("a.test").eq(0).triggerHandler("click") === false,
...
QUnit.test(
	"on(name, selector, false), off(name, selector, false)",
	function (assert) {
		assert.expect(3);
...
QUnit.test(
	"withinElement implemented with jQuery.contains()",
	function (assert) {
		assert.expect(1);
...
QUnit.test("trigger(type, [data], [fn])", function (assert) {
	...
	pass = true;
	try {
		jQuery("#qunit-fixture table")
			.eq(0)
			.on("test:test", function () {})
			.trigger("test:test");
	} catch (e) {
		pass = false;
	}
	assert.ok(
		pass,
		"Trigger on a table with a colon in the even type, see #3533"
	);
...
QUnit.test("submit event bubbles on copied forms (#11649)", function (assert) {
	...
	$formByHTML = jQuery(jQuery.parseHTML($fixture.html()))
		.filter("#testForm")
		.removeAttr("id");
...
QUnit.test(
	".trigger() bubbling on disconnected elements (#10489)",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"Delegated events with malformed selectors (gh-3071)",
	function (assert) {
		assert.expect(3);
...
QUnit.test(
	"Delegated events in forms (#10844; #11145; #8165; #11382, #11764)",
	function (assert) {
		assert.expect(5);
...
QUnit[/(ipad|iphone|ipod)/i.test(navigator.userAgent) ? "skip" : "test"](
	...
	function (assert) {
		...
		var iframe = jQuery(
			jQuery.parseHTML(
				"<iframe src='" + baseURL + "event/onbeforeunload.html'><iframe>"
			)
		);
...
QUnit.test("jQuery.Event properties", function (assert) {
	...
	var handler,
		$structure = jQuery(
			"<div id='ancestor'><p id='delegate'><span id='target'>shiny</span></p></div>"
		),
	...
	$target[0].onclick = function (e) {
		assert.strictEqual(
			e.currentTarget,
			this,
			"currentTarget at target (native handler)"
		);
	...
	$target[0].onclick = function (e) {
		assert.strictEqual(
			e.isTrigger,
			undefined,
			"native event at target (native handler)"
		);
	};
...
QUnit.test(".on()/.off()", function (assert) {
	...
	jQuery("#body").on(
		...
		function (e, data) {
			assert.equal(
				e.data,
				true,
				"on with with different this object, event data, and trigger data"
			);
			assert.equal(
				this.foo,
				"bar",
	...
	jQuery("#body").on("click", "span#liveSpan1", function (e) {
		...
		assert.equal(
			...
			"A",
			"Check the event.target within a on handler"
		);
	});
...
QUnit.test(
	"events with type matching an Object.prototype property (gh-3256)",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"events with type matching an Object.prototype property, cloned element (gh-3256)",
	function (assert) {
		assert.expect(1);
...
QUnit[jQuery.find.compile ? "test" : "skip"](
	"delegated event with delegateTarget-relative selector",
	function (assert) {
		assert.expect(3);
		var markup = jQuery(
...
QUnit.test(
	"delegated event with selector matching Object.prototype property (#13203)",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"ignore comment nodes in event delegation (gh-2055)",
	function (assert) {
		assert.expect(1);
		...
		var $foo = jQuery("#foo").on("DOMNodeInserted", "[id]", function () {
			...
			$comment = jQuery(document.createComment("comment")).appendTo(
				$foo.find("#sap")
			);
...
QUnit.test(
	"stopPropagation() stops directly-bound events on delegated target",
	function (assert) {
		assert.expect(1);
...
QUnit.test("off all bound delegated events", function (assert) {
	...
	assert.equal(
		clicks,
		2,
		"Make sure delegated and directly bound event occurred."
	);
	div.off("click");
});
...
QUnit.test("checkbox state (trac-3827)", function (assert) {
	...
	assert.equal(
		cb.checked,
		true,
		"native event - checkbox is initially checked"
	);
	cb.click();
	assert.equal(
		cb.checked,
		false,
		"native event - checkbox is no longer checked"
	);
	...
	assert.equal(
		cb.checked,
		true,
		"jQuery event - checkbox is initially checked"
	);
	jQuery(cb).trigger("click");
	assert.equal(
		cb.checked,
		false,
		"jQuery event - checkbox is no longer checked"
	);
	...
	assert.equal(
		cb.checked,
		false,
		"handlers only - checkbox is still unchecked"
	);
	...
	assert.equal(
		cb.checked,
		true,
		"jQuery event with data - checkbox is initially checked"
	);
	...
	assert.equal(
		cb.checked,
		false,
		"jQuery event with data - checkbox is no longer checked"
	);
});

QUnit.test(
	"event object properties on natively-triggered event",
	function (assert) {
		assert.expect(3);
...
QUnit.test("addProp extensions", function (assert) {
	...
	$fixture.on("click", function (event) {
		assert.equal(
			event.testProperty,
			42,
			"event.testProperty getter was invoked"
		);
	});
	fireNative($fixture[0], "click");
	$fixture.off("click");
...
QUnit.test(
	"drag/drop events copy mouse-related event properties (gh-1925, gh-2009)",
	function (assert) {
		assert.expect(4);
...
QUnit.test(
	"Namespace preserved when passed an Event (#12739)",
	function (assert) {
		assert.expect(4);
		...
		markup
			...
			.on("foo.bar2", function () {
				assert.ok(
					false,
					"foo.bar2 called on trigger " + triggered + " id " + this.id
				);
			});
...
QUnit.test("make sure events cloned correctly", function (assert) {
	...
	fixture
		.on("click change", function (event, result) {
...
QUnit.test(
	"String.prototype.namespace does not cause trigger() to throw (#13360)",
	function (assert) {
		assert.expect(1);
		var errored = false;
...
QUnit.test("Inline event result is returned (#13993)", function (assert) {
	...
	var result = jQuery("<p onclick='return 42'>hello</p>").triggerHandler(
		"click"
	);
...
QUnit.test(
	".off() removes the expando when there's no more data",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"jQuery.Event( src ) does not require a target property",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"preventDefault() on focusin does not throw exception",
	function (assert) {
		assert.expect(1);
...
QUnit.test("Donor event interference", function (assert) {
	...
	input.on("click", function (event) {
		...
		assert.ok(
			!event.isPropagationStopped(),
			"click event at input is still propagating"
		);
		assert.equal(
	...
	outer.on("click", function (event) {
		...
		assert.ok(
			!event.isPropagationStopped(),
			"click event at ancestor is still propagating"
		);
		assert.equal(
			typeof event.originalEvent,
			"object",
...
QUnit.test(
	...
	function (assert) {
		...
		assert.strictEqual(
			spy.prevent.called,
			false,
			"Native preventDefault not called"
		);
		assert.strictEqual(
			spy.stop.called,
			false,
			"Native stopPropagation not called"
		);
		assert.strictEqual(
			spy.immediate.called,
			false,
...
QUnit.test(
	"Event handling works with multiple async focus events (gh-4350)",
	function (assert) {
		assert.expect(3);
		...
		input
			.on("focus", function () {
				...
				assert.ok(
					true,
					"received focus event, expecting " + remaining + " more"
				);
				if (remaining > 0) {
					input.trigger("blur");
				} else {
...
QUnit.test(
	"native-backed events preserve trigger data (gh-1741, gh-4139)",
	function (assert) {
		assert.expect(17);
		...
		assert.strictEqual(
			targets[0].checked,
			false,
			"checkbox unchecked before click"
		);
		assert.strictEqual(
			targets[1].checked,
			false,
			"radio unchecked before click"
		);
...
(function () {
	...
	function makeTestFor3751(type, html) {
		var testName =
			"native-backed namespaced clicks are handled correctly (gh-3751) - " +
			type;
		QUnit.test(testName, function (assert) {
			...
			target
				...
				.on("click.notFired", function (event) {
					assert.ok(
						false,
						"namespaced event should not be received" +
							" by wrong-namespace listener at " +
