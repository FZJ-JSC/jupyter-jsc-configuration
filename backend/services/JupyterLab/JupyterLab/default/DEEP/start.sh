...
echo "Status Update: curl -X \"POST\" -H \"Authorization: token ${JUPYTERHUB_API_TOKEN}\" -H \"uuidcode: ${JUPYTER_JSC_STARTUUID}\" -H \"Content-Type: application/json\" --data '{\"progress\": 60, \"failed\": false, \"message\": \"\", \"html_message\": \"Preparing environment on '\"${HOSTNAMES}\"' ...\"}' http://${JUPYTER_JSC_REMOTENODE}:${JUPYTER_JSC_REMOTEPORT}/hub/api/${JUPYTERHUB_STATUS_URL}"
...
if [[ -n $JUPYTERJSC_USER_CMD ]]; then
    ...
    timeout 30d jupyterhub-singleuser --config ${DIR}/.config.py &
fi
child=$!
wait $child
