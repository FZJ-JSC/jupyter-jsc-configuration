...
class ProjectsORM(Base):
    ...
    def __repr__(self):
        return "<Project {}: {} {} {}>".format(
            self.id, self.user_id, self.system, self.name
        )
    ...
    def find_to_delete(cls, db, user_id, system, name):
        ...
        if system:
            if name:
                query = (
                    db.query(cls, ProjectSharesORM, User)
                    .filter(cls.user_id == user_id)
                    .filter(cls.system == system)
                    .filter(cls.name == name)
                    .filter(cls.id == ProjectSharesORM.project_id)
                    .filter(User.id == ProjectSharesORM.shared_with)
                    .all()
                )
...
class ProjectSharesORM(Base):
    ...
    project_id = Column(
        Integer, ForeignKey("projects.id", ondelete="CASCADE"), default=""
    )
    shared_with = Column(
        Integer, ForeignKey("users.id", ondelete="CASCADE"), default=""
    )

    def __repr__(self):
        return "<ProjectShare {}: {} - {}>".format(
            self.id, self.project_id, self.shared_with
        )
    ...
    def find_to_delete_shares(cls, db, owner_id, system, pname, user_id):
        if user_id:
            query = (
                db.query(cls, ProjectsORM)
                .filter(ProjectsORM.user_id == owner_id)
                .filter(ProjectsORM.system == system)
                .filter(ProjectsORM.name == pname)
                .filter(ProjectsORM.id == cls.project_id)
                .filter(cls.shared_with == user_id)
                .all()
            )
        else:
            query = (
    ...
    def find_to_delete_myself(cls, db, user_id, system, pname, owner_id):
        query = (
            db.query(cls, ProjectsORM)
            .filter(cls.shared_with == user_id)
            .filter(cls.project_id == ProjectsORM.id)
            .filter(ProjectsORM.system == system)
            .filter(ProjectsORM.name == pname)
            .filter(ProjectsORM.user_id == owner_id)
            .all()
