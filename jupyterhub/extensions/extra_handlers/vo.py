...
class VOHandler(BaseHandler):
    ...
    async def get(self):
        ...
        vo_active, vo_available = get_vos(auth_state, user.name, user.admin, auth_state["vo_active"])
        ...
        for vo_name in auth_state.get("vo_available", []):
            vo_details[vo_name] = vo_details_config.get("values", {}).get(vo_name, {}).get("description", "No description available")
...
class VOAPIHandler(APIHandler):
    ...
    async def post(self, group):
        ...
        if group in state.get("vo_available", []):
            ...
            self.log.debug("{} not part of list {}".format(group, state.get("vo_available", [])))
            self.set_status(403)
            return
        self.set_status(204)
        return
