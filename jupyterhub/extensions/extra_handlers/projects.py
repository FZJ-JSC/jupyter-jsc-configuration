...
from jupyterhub.utils import maybe_future, admin_only, new_token, token_authenticated
from jupyterhub import orm

from jupyterhub.metrics import RUNNING_SERVERS, SERVER_STOP_DURATION_SECONDS, ServerStopStatus
from jupyterhub.apihandlers.users import admin_or_self

from .projects_orm import ProjectsORM, ProjectSharesORM
...
class ProjectsAPIHandler(APIHandler):
    ...
    async def get(self, system="", name=""):
        ...
        self.log.debug("uuidcode={uuidcode} - List Projects for {owner} - {system} - {name}".format(uuidcode=uuidcode, owner=user.name, system=system, name=name))
    ...
    async def post(self, system, name):
        ...
        uuidcode = self.request.headers.get("uuidcode", "<no_uuidcode>")
        user = self.get_current_user_token()
        self.log.debug("uuidcode={uuidcode} - Create Projects for {owner} - {system} - {name}".format(uuidcode=uuidcode, owner=user.name, system=system, name=name))
        projects = ProjectsORM.find(self.db, user.orm_user.id, system, name)
        if projects:
            self.set_header('Content-Type', 'text/plain')
            username = user.name
            self.log.info(f"uuidcode={uuidcode} - Project {name} on {system} from {username} already exists")
    ...
    async def delete(self, system, name, owner=""):
        ...
        if len(owner) > 0 and owner != user.name:
            ...
            self.log.debug("uuidcode={uuidcode} - Delete myself ({myself}) from project {owner} - {system} - {name}".format(uuidcode=uuidcode, myself=user.name, owner=owner, system=system, name=name))
            owner_orm = self.find_user(owner)
            projects = ProjectShares.find_to_delete_myself(self.db, user.orm_user.id, system, name, owner_orm.id)
            if not projects:
                self.set_status(404)
                return
            ...
            self.log.debug("uuidcode={uuidcode} - Delete Project {owner} - {system} - {name}".format(uuidcode=uuidcode, owner=user.name, system=system, name=name))
            projects = ProjectsORM.find_to_delete(self.db, user.orm_user.id, system, name)
            if not projects:
                self.set_status(404)
                return
...
class ProjectSharesAPIHandler(APIHandler):
    ...
    async def get(self, system=""):
        ...
        uuidcode = self.request.headers.get("uuidcode", "<no_uuidcode>")
        user = self.get_current_user_token()
        self.log.debug("uuidcode={uuidcode} - List projects shared with {username}".format(uuidcode=uuidcode, username=user.name))
    ...
    async def post(self, system, project_name, username):
        ...
        if not projects:
            ...
            self.set_status(404)
            return
        pre_existing_share = ProjectSharesORM.find(self.db, user.orm_user.id, system, project_name, user_to_add.orm_user.id)
        if not pre_existing_share:
            share_orm = ProjectSharesORM(project_id = projects[0].id, shared_with = user_to_add.orm_user.id)
    ...
    async def delete(self, system, project_name, username=""):
        ...
        uuidcode = self.request.headers.get("uuidcode", "<no_uuidcode>")
        user = self.get_current_user_token()
        self.log.debug("uuidcode={uuidcode} - Delete sharing {owner} - {system} - {project_name} - {username}".format(uuidcode=uuidcode, owner=user.name, system=system, project_name=project_name, username=username))
        if username:
            ...
            user_id = ""
        to_delete_all = ProjectSharesORM.find_to_delete_shares(self.db, user.orm_user.id, system, project_name, user_id)
