...
class UNICORENotificationAPIHandler(APIHandler):
    def verify_unicore(self, uuidcode, kernelurl, auth, unicore_config):
        ...
        if system != "":
            ...
            raise Exception("Could not find any systems for the url: {}".format(kernelurl))
    ...
    async def post(self, username, server_name=''):
        ...
        try:
            ...
            self.log.exception("uuidcode={} - Could not verify token {} with public key for UNICORE/X".format(uuidcode, auth))
            self.set_status(401)
            return
        ...
        if jdata.get('status', '') == 'RUNNING':
            ...
            try:
                ...
                user_msg = err_msg
                for err_message_key, err_message_mapped in unicore_config.get("error_mapping", {}).items():
            ...
            self.log.error("uuidcode={} UX Handler: Unknown status. Insert reaction to this status: {}".format(uuidcode, jdata))
        return
