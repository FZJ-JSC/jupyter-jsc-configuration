...
if [[ ${ACTION} == ${START} ]]; then
        ...
        if [[ ! -n $PID ]]; then
                ssh -p 2222 -i /home/userlabs/.ssh/ed-25519 -oLogLevel=ERROR -oUserKnownHostsFile=/dev/null -oServerAliveInterval=60 -oServerAliveInterval=5 -oExitOnForwardFailure=yes -oStrictHostKeyChecking=no -L0.0.0.0:${PORT}:localhost:30800 tunnel@${HOST} -f -N
        fi
