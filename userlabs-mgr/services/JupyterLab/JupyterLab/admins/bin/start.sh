...
if [[ -f ${HOME}/JUST_HOMEs_readonly/judac/.jupyter/start_jupyter-jsc.sh ]]; then
    ...
    if [[ $JUPYTERHUB_USER == "t.kreuzer@fz-juelich.de" ]]; then
        module load JupyterProxy-XpraHTML5/.0.3.0-2020.2.6
    fi
...
if [[ -n $JUPYTERJSC_USER_CMD ]]; then
    ...
    jupyterhub-singleuser --config /opt/apps/bin/config.py >> /opt/apps/bin/jupyterlab.log 2>&1
fi
