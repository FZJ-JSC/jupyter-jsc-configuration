...
kubectl -n ${NAMESPACE} create configmap ${CONFIGMAP_NAME} --from-literal=JUPYTER_ENABLE_LAB=yes --from-literal=NB_UID=1094 --from-literal=JUPYTERHUB_STATUS_URL=${JUPYTERHUB_STATUS_URL} --from-literal=JUPYTERHUB_ACTIVITY_URL=${JUPYTERHUB_ACTIVITY_URL} --from-literal=JUPYTERHUB_CANCEL_URL=${JUPYTERHUB_CANCEL_URL} --from-literal=JUPYTERHUB_API_URL=${JUPYTERHUB_API_URL} --from-literal=JUPYTERHUB_CLIENT_ID=${JUPYTERHUB_CLIENT_ID} --from-literal=JUPYTERHUB_USER=${JUPYTERHUB_USER} --from-literal=JUPYTERHUB_SERVICE_PREFIX=${JUPYTERHUB_SERVICE_PREFIX} --from-literal=JUPYTERHUB_BASE_URL=${JUPYTERHUB_BASE_URL} --from-literal=PORT=${PORT} --from-literal=STARTUUIDCODE=${STARTUUIDCODE} --from-literal=SERVERNAMESHORT=${SERVERNAMESHORT} --from-literal=REMOTE_PORT=${REMOTE_PORT} --from-literal=REMOTE_NODE=${REMOTE_NODE} --from-literal=SERVICE_NAME=${SERVICE_NAME} >> ${JOB_PATH}/logs/start.log 2>&1
if [[ ! $? -eq 0 ]]; then
...
LOCAL_USERHOMES_PATH="/mnt/userdata/userdata/allsparknotebook"

if [[ ! -d ${LOCAL_USERHOMES_PATH}/${JUPYTERHUB_USER_ID} ]]; then
    echo "$(date) Create HOME for user." >> ${JOB_PATH}/logs/start.log 2>&1
    curl -X "POST" -H "Authorization: token ${JUPYTERHUB_API_TOKEN}" -H "uuidcode: ${STARTUUIDCODE}" -H "Content-Type: application/json" --data '{"progress": 65, "failed": false, "message": "", "html_message": "&nbsp;&nbsp;... create persistent directory for AllSpark JupyterLabs for '"${JUPYTERHUB_USER}"'"}' http://${REMOTE_NODE}:${REMOTE_PORT}${JUPYTERHUB_BASE_URL}hub/api/${JUPYTERHUB_STATUS_URL} >> ${JOB_PATH}/logs/start.log 2>&1
    cp -rp ${LOCAL_USERHOMES_PATH}/home-spark-3.1.2 ${LOCAL_USERHOMES_PATH}/${JUPYTERHUB_USER_ID}
    if [[ ! -d ${JOB_PATH}/motd/motd.d ]]; then
        mkdir -p ${JOB_PATH}/motd/motd.d >> ${JOB_PATH}/logs/start.log 2>&1
    fi
    if [[ -f ${LOCAL_USERHOMES_PATH}/skel/motd.d/newuser ]]; then
        cp -r ${LOCAL_USERHOMES_PATH}/skel/motd.d/newuser ${JOB_PATH}/motd/motd.d/newuser >> ${JOB_PATH}/logs/start.log 2>&1
    fi
    ...
    if [[ -f ${LOCAL_USERHOMES_PATH}/.${JUPYTERHUB_USER_ID}.quota.bytes ]]; then
        ...
        if [[ $USED_BYTES -gt $STORAGE_HARD_BYTES ]]; then
            ...
            if [[ -f ${LOCAL_USERHOMES_PATH}/skel/motd.d/softlimit ]]; then
                ...
                cp ${LOCAL_USERHOMES_PATH}/skel/motd.d/softlimit ${JOB_PATH}/motd/motd.d/softlimit >> ${JOB_PATH}/logs/start.log 2>&1
	        sed -i -e "s/_used_human_/${USED_HUMAN}/g" -e "s/_softlimit_/${STORAGE_USERDATA_SOFT}/g" -e "s/_hardlimit_/${STORAGE_USERDATA_HARD}/g" ${JOB_PATH}/motd/motd.d/softlimit >> ${JOB_PATH}/logs/start.log 2>&1
            fi 
...
if [[ -f ${LOCAL_USERHOMES_PATH}/skel/motd ]]; then
    cp ${LOCAL_USERHOMES_PATH}/skel/motd ${JOB_PATH}/motd/motd >> ${JOB_PATH}/logs/start.log 2>&1
    sed -i -e "s/_memory_/${RESOURCE_LIMIT_MEMORY}/g" ${JOB_PATH}/motd/motd >> ${JOB_PATH}/logs/start.log 2>&1
fi
...
sed -i -e "s/_resource_requests_cpu_/${RESOURCE_REQUESTS_CPU}/g" ${JOB_PATH}/yaml/userlab.yaml >> ${JOB_PATH}/logs/start.log 2>&1
