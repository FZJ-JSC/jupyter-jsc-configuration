...
kubectl -n ${NAMESPACE} create configmap ${CONFIGMAP_NAME} --from-literal=SYSTEM_USER=${SYSTEM_USER} --from-literal=NB_USER=${NB_USER} --from-literal=NB_UID=${NB_UID} --from-literal=NB_GID=${NB_GID} --from-literal=JUPYTERHUB_STATUS_URL=${JUPYTERHUB_STATUS_URL} --from-literal=JUPYTERHUB_ACTIVITY_URL=${JUPYTERHUB_ACTIVITY_URL} --from-literal=JUPYTERHUB_CANCEL_URL=${JUPYTERHUB_CANCEL_URL} --from-literal=JUPYTERHUB_API_URL=${JUPYTERHUB_API_URL} --from-literal=JUPYTERHUB_CLIENT_ID=${JUPYTERHUB_CLIENT_ID} --from-literal=JUPYTERHUB_USER=${JUPYTERHUB_USER} --from-literal=JUPYTERHUB_SERVICE_PREFIX=${JUPYTERHUB_SERVICE_PREFIX} --from-literal=JUPYTERHUB_BASE_URL=${JUPYTERHUB_BASE_URL} --from-literal=PORT=${PORT} --from-literal=STARTUUIDCODE=${STARTUUIDCODE} --from-literal=SERVERNAMESHORT=${SERVERNAMESHORT} --from-literal=REMOTE_PORT=${REMOTE_PORT} --from-literal=REMOTE_NODE=${REMOTE_NODE} --from-literal=SERVICE_NAME=${SERVICE_NAME} >> ${JOB_PATH}/logs/start.log 2>&1
if [[ ! $? -eq 0 ]]; then
...
LOCAL_USERHOMES_PATH="/mnt/userdata/userdata/aiidalab"
OLD_ID_USERHOMES_PATH="/mnt/userdata/userdata/aiidalab/old_by_id_to_name"
...
if [[ -d ${OLD_ID_USERHOMES_PATH}/${JUPYTERHUB_USER} && ! -d ${LOCAL_USERHOMES_PATH}/${JUPYTERHUB_USER_ID} ]]; then
    ...
    chown -R 1000:100 ${LOCAL_USERHOMES_PATH}/${JUPYTERHUB_USER_ID} >> ${JOB_PATH}/logs/start.log 2>&1
