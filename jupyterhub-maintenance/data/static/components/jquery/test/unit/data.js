...
QUnit.test("expando", function (assert) {
	...
	assert.equal(
		...
		true,
		"jQuery is exposing the expando"
	);
});
...
QUnit.test("jQuery.data & removeData, expected returns", function (assert) {
	...
	assert.equal(
		jQuery.data(elem, "hello"),
		"world",
		"jQuery.data( elem, key ) returns value"
	);
	assert.deepEqual(
		jQuery.data(elem, { goodnight: "moon" }),
		{ goodnight: "moon" },
		"jQuery.data( elem, obj ) returns obj"
	);
	assert.equal(
		jQuery.removeData(elem, "hello"),
		undefined,
...
QUnit.test("jQuery._data & _removeData, expected returns", function (assert) {
	...
	assert.equal(
		jQuery._data(elem, "hello"),
		"world",
		"jQuery._data( elem, key ) returns value"
	);
	assert.deepEqual(
		jQuery._data(elem, { goodnight: "moon" }),
		{ goodnight: "moon" },
		"jQuery._data( elem, obj ) returns obj"
	);
	assert.equal(
		jQuery._removeData(elem, "hello"),
		undefined,
...
function dataTests(elem, assert) {
	...
	assert.strictEqual(
		jQuery.hasData(elem),
		false,
		"jQuery.hasData agrees no data exists initially"
	);
	...
	assert.equal(
		typeof dataObj,
		"object",
		"Calling data with no args gives us a data object reference"
	);
	assert.strictEqual(
		jQuery.data(elem),
		dataObj,
	...
	assert.strictEqual(
		jQuery.hasData(elem),
		true,
		"jQuery.hasData agrees data exists when data exists"
	);
	...
	assert.equal(
		jQuery.data(elem, "foo"),
		"baz",
		"Data can be changed by jQuery.data"
	);
	assert.equal(
		dataObj["foo"],
		"baz",
	...
	assert.equal(
		jQuery.data(elem, "foo"),
		"baz",
		"Data is not unset by passing undefined to jQuery.data"
	);
	...
	assert.strictEqual(
		jQuery.data(elem, "foo"),
		null,
		"Setting null using jQuery.data works OK"
	);
	...
	assert.equal(
		jQuery.data(elem, "boom"),
		"bloz",
		"Extending the data object works"
	);
	...
	assert.equal(
		...
		"foo2",
		"Setting internal data works"
	);
	assert.equal(
		jQuery.data(elem, "foo"),
		"foo1",
		"Setting internal data does not override user data"
	);
	...
	assert.strictEqual(
		jQuery.data(elem, "foo"),
		undefined,
		"jQuery.removeData removes single properties"
	);
	...
	assert.equal(
		jQuery.data(elem, "foo"),
		"foo1",
		"(sanity check) Ensure data is set in user data object"
	);
	assert.equal(
		jQuery._data(elem, "foo"),
		"foo2",
...
QUnit.test(".data()", function (assert) {
	...
	assert.strictEqual(
		div.data("foo"),
		undefined,
		"Make sure that missing result is undefined"
	);
	...
	assert.strictEqual(
		div.data("foo"),
		undefined,
		"Make sure that missing result is still undefined"
	);
...
QUnit.test(
	"jQuery(Element).data(String, Object).data(String)",
	function (assert) {
		assert.expect(18);
		...
		assert.strictEqual(
			div.data("test", "success").data("test"),
			"success",
			"Data added"
		);
		assert.strictEqual(
			div.data("test", "overwritten").data("test"),
			"overwritten",
			"Data overwritten"
		);
		assert.strictEqual(
...
QUnit.test(
	"jQuery(plain Object).data(String, Object).data(String)",
	function (assert) {
		assert.expect(16);
		...
		assert.strictEqual(
			$obj.data("nothing"),
			undefined,
			"Non-existent data returns undefined"
		);
		assert.strictEqual(
			$obj.data("exists"),
			undefined,
			"Object properties are not returned as data"
		);
...
QUnit.test(
	".data(object) does not retain references. #13815",
	function (assert) {
		assert.expect(2);
...
QUnit.test("data-* attributes", function (assert) {
	...
	assert.equal(
		div.data("attr"),
		undefined,
		"Check for non-existing data-attr attribute"
	);
	...
	assert.equal(
		div.data("attr"),
		"exists",
		"Check for existing data-attr attribute"
	);
	...
	assert.equal(
		div.data("attr"),
		"exists",
		"Check that updates to data- don't update .data()"
	);
	...
	assert.equal(
		child.data("myobj"),
		"old data",
		"Value accessed from data-* attribute"
	);
	assert.equal(
		child.data("foo-42"),
		"boosh",
		"camelCasing does not affect numbers (#1751)"
	);
	...
	assert.equal(
		child.data("ignored"),
		"cache",
		"Cached data used before DOM data-* fallback"
	);
	...
	assert.equal(
		child.data("other"),
		"test",
		"Make sure value was pulled in properly from a .data()."
	);
	...
	assert.strictEqual(
		child.data("true"),
		true,
		"Primitive true read from attribute"
	);
	assert.strictEqual(
		child.data("false"),
		false,
		"Primitive false read from attribute"
	);
	assert.strictEqual(child.data("five"), 5, "Integer read from attribute");
	assert.strictEqual(
		child.data("point"),
		5.5,
		"Floating-point number read from attribute"
	);
	assert.strictEqual(
		child.data("pointe"),
		"5.5E3",
	...
	assert.strictEqual(
		child.data("badjson"),
		"{123}",
		"Bad JSON object read from attribute as string"
	);
	assert.strictEqual(
		child.data("badjson2"),
		"[abc]",
		"Bad JSON array read from attribute as string"
	);
	assert.strictEqual(
		child.data("notjson"),
		" {}",
	...
	assert.strictEqual(
		child.data("empty"),
		"",
		"Empty string read from attribute"
	);
	assert.strictEqual(
		child.data("space"),
		" ",
		"Whitespace string read from attribute"
	);
	assert.strictEqual(
		child.data("null"),
		null,
		"Primitive null read from attribute"
	);
	assert.strictEqual(
		child.data("string"),
		"test",
		"Typical string read from attribute"
	);
	assert.equal(
		i,
		2,
		"Correct number of JSON parse attempts when reading from attributes"
	);
	...
	function testData(index, elem) {
		switch (index) {
			...
			case 3:
				assert.equal(
					jQuery(elem).data("number"),
					true,
					"Check number property"
				);
				assert.deepEqual(
					jQuery(elem).data("stuff"),
					[2, 8],
					"Check stuff property"
				);
				break;
			default:
				assert.ok(
					false,
					["Assertion failed on index ", index, ", with data"].join("")
				);
		}
	}
...
QUnit.test(".data(Object)", function (assert) {
	...
	assert.equal(
		...
		"in",
		"Verify setting an object on an object extends the data object"
	);
	assert.equal(
		obj["test2"],
		undefined,
		"Verify setting an object on an object does not extend the object"
	);
...
QUnit.test("jQuery.removeData", function (assert) {
	...
	assert.ok(
		!jQuery.data(div, "test2"),
		"Make sure that the data property no longer exists."
	);
	assert.ok(
		!div[jQuery.expando],
	...
	assert.ok(
		!jQuery.data(div, "test3") || jQuery.data(div, "test4"),
		"Multiple delete with spaces."
	);
	...
	jQuery.removeData(div, "test3 test4");
	assert.ok(
		!jQuery.data(div, "test3 test4"),
		"Multiple delete with spaces deleted key with exact name"
	);
	...
	assert.equal(
		jQuery(obj).data("test"),
		"testing",
		"verify data on plain object"
	);
	jQuery.removeData(obj, "test");
	assert.equal(
		jQuery.data(obj, "test"),
		undefined,
		"Check removal of data on plain object"
	);

	jQuery.data(window, "BAD", true);
	jQuery.removeData(window, "BAD");
	assert.ok(
		!jQuery.data(window, "BAD"),
		"Make sure that the value was not still set."
	);
});
...
if (window.JSON && window.JSON.stringify) {
	QUnit.test("JSON serialization (#8108)", function (assert) {
		...
		assert.equal(
			JSON.stringify(obj),
			'{"foo":"bar"}',
			"Expando is hidden from JSON.stringify"
		);
	});
}

QUnit.test(
	".data should follow html5 specification regarding camel casing",
	function (assert) {
		assert.expect(12);
		...
		assert.equal(
			div.data()["bigALittleA"],
			"bouncing-b",
			"Verify single letter mixed data-* key"
		);
		...
		assert.equal(
			div.data("foo-bar"),
			"d",
			"Verify updated data-* key (foo-bar)"
		);
...
QUnit.test(
	".data should not miss preset data-* w/ hyphenated property names",
	function (assert) {
		assert.expect(2);

		var div = jQuery("<div></div>", { id: "hyphened" }).appendTo(
				"#qunit-fixture"
			),
			test = {
				camelBar: "camelBar",
		...
		jQuery.each(test, function (i, k) {
			assert.equal(
				div.data(k),
				k,
				"data with property '" + k + "' was correctly found"
			);
		});
	}
);
...
QUnit.test(
	"jQuery.data should not miss data-* w/ hyphenated property names #14047",
	function (assert) {
		assert.expect(1);
		...
		assert.equal(
			jQuery.data(div[0], "foo-bar"),
			"baz",
			"data with property 'foo-bar' was correctly found"
		);
	}
);

QUnit.test(
	".data should not miss attr() set data-* with hyphenated property names",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	".data always sets data with the camelCased key (gh-2257)",
	function (assert) {
		assert.expect(18);
		...
		jQuery.each(datas, function (key, val) {
			...
			assert.equal(
				allData[key],
				undefined,
				".data does not store with hyphenated keys"
			);
			assert.equal(
				allData[val.key],
				val.value,
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			allData["nested-Double"],
			"double",
			"Key with double hyphens is correctly camelCased"
		);
		assert.equal(
			allData["nested--Triple"],
			"triple",
			"Key with triple hyphens is correctly camelCased"
		);
	}
);

QUnit.test(
	".data supports interoperable hyphenated/camelCase get/set of properties with arbitrary non-null|NaN|undefined values",
	function (assert) {
		var div = jQuery("<div></div>", { id: "hyphened" }).appendTo(
				"#qunit-fixture"
			),
...
QUnit.test(
	".data supports interoperable removal of hyphenated/camelCase properties",
	function (assert) {
		var div = jQuery("<div></div>", { id: "hyphened" }).appendTo(
				"#qunit-fixture"
			),
...
QUnit.test(
	".removeData should not throw exceptions. (#10080)",
	function (assert) {
...
QUnit.test(
	".data only checks element attributes once. #8909",
	function (assert) {
		assert.expect(2);
		var testing = {
				test: "testing",
...
QUnit.test("data-* with JSON value can have newlines", function (assert) {
	...
	assert.equal(
		x.data("some").foo,
		"bar",
		"got a JSON data- attribute with spaces"
	);
	x.remove();
});

QUnit.test(
	".data doesn't throw when calling selection is empty. #13551",
	function (assert) {
		assert.expect(1);
...
QUnit.test("acceptData", function (assert) {
	...
	assert.equal(
		jQuery(document.documentElement).data("test", 42).data("test"),
		42,
		"documentElement"
	);
	assert.equal(jQuery({}).data("test", 42).data("test"), 42, "object");
	assert.equal(
		jQuery(document.createElement("embed")).data("test", 42).data("test"),
		42,
		"embed"
	);
	...
	assert.strictEqual(
		jQuery(document.createComment("")).data("test", 42).data("test"),
		undefined,
		"comment"
	);
	assert.strictEqual(
		jQuery(document.createTextNode("")).data("test", 42).data("test"),
		undefined,
		"text"
	);
	assert.strictEqual(
		jQuery(document.createDocumentFragment()).data("test", 42).data("test"),
		undefined,
		"documentFragment"
	);

	form = jQuery("#form").append(
		"<input id='nodeType'/><input id='nodeName'/>"
	)[0];
	assert.equal(
		jQuery(form).data("test", 42).data("test"),
		42,
...
QUnit.test(
	"Check proper data removal of non-element descendants nodes (#8335)",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"Check that the expando is removed when there's no more data",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"Check that the expando is removed when there's no more data on non-nodes",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	".data() returns a regular object (jQuery <4 only, gh-4665)",
	function (assert) {
		assert.expect(4);
