...
QUnit.test("jQuery()", function (assert) {
	...
	assert.equal(
		jQuery(window).length,
		1,
		"Correct number of elements generated for jQuery(window)"
	);
...
*/
	...
	assert.equal(
		code.parent().length,
		0,
		"Make sure that the generated HTML has no parent."
	);
	...
	assert.equal(
		div.length,
		4,
		"Correct number of elements generated for div hr code b"
	);
	assert.equal(
		div.parent().length,
		0,
	...
	assert.equal(
		jQuery([1, 2, 3]).get(1),
		2,
		"Test passing an array to the factory"
	);

	assert.equal(
		jQuery(document.body).get(0),
		jQuery("body").get(0),
		"Test passing an html node to the factory"
	);
	...
	if (jQuery.fn.attr) {
		assert.equal(
			elem[0].getAttribute("desired"),
			"very",
			"jQuery quick setter attr"
		);
	}
	...
	assert.equal(
		elem[0].defaultValue,
		"TEST",
		"Ensure cached nodes are cloned properly (Bug #6655)"
	);

	elem = jQuery("<input type='hidden'>", {});
	assert.strictEqual(
		elem[0].ownerDocument,
		document,
...
QUnit[jQuery.find.compile ? "test" : "skip"](
	...
	function (assert) {
		assert.expect(3);
		assert.deepEqual(
			jQuery("div p", "#qunit-fixture").get(),
			q("sndp", "en", "sap"),
			"Basic selector with string as context"
		);
		assert.deepEqual(
			jQuery("div p", q("qunit-fixture")[0]).get(),
			q("sndp", "en", "sap"),
			"Basic selector with element as context"
		);
		assert.deepEqual(
			jQuery("div p", jQuery("#qunit-fixture")).get(),
			q("sndp", "en", "sap"),
...
QUnit.test("globalEval", function (assert) {
	...
	assert.equal(
		...
		1,
		"Test variable assignments are global"
	);
	...
	assert.equal(
		window.globalEvalTest,
		2,
		"Test variable declarations are global"
	);
	...
	assert.equal(
		window.globalEvalTest,
		3,
		"Test context (this) is the window object"
	);
});
...
QUnit.test("globalEval with 'use strict'", function (assert) {
	...
	assert.equal(
		window.strictEvalTest,
		1,
		"Test variable declarations are global (strict mode)"
	);
});

QUnit.test(
	"globalEval execution after script injection (#7862)",
	function (assert) {
		assert.expect(1);
...
QUnit.test("isPlainObject", function (assert) {
	...
	assert.ok(
		jQuery.isPlainObject({ constructor: fn }),
		"plain object with constructor property"
	);
	assert.ok(
		jQuery.isPlainObject({ constructor: "foo" }),
	...
	assert.ok(
		!jQuery.isPlainObject(true) && !jQuery.isPlainObject(false),
		"boolean"
	);
	...
	assert.ok(
		!jQuery.isPlainObject(new fn()),
		"new fn with primitive constructor"
	);
	...
	// DOM Element
	assert.ok(
		!jQuery.isPlainObject(document.createElement("div")),
		"DOM Element"
	);
	...
	window.iframeDone = function (otherObject, detail) {
		...
		assert.ok(
			jQuery.isPlainObject(new otherObject()),
			"new otherObject" + (detail ? " - " + detail : "")
		);
		done();
	};

	try {
		iframe = jQuery("#qunit-fixture")[0].appendChild(
...
QUnit[typeof Symbol === "function" ? "test" : "skip"](
	...
	function (assert) {
		...
		assert.equal(
			jQuery.isPlainObject(Object(Symbol())),
			false,
...
QUnit["assign" in Object ? "test" : "skip"](
...
QUnit.test("isXMLDoc - embedded SVG", function (assert) {
	...
	assert.strictEqual(
		jQuery.isXMLDoc(htmlTree),
		false,
		"disconnected div element"
	);
	assert.strictEqual(
		jQuery.isXMLDoc(htmlTree.firstChild),
		true,
		"disconnected HTML-embedded SVG root element"
	);

	assert.strictEqual(
		jQuery.isXMLDoc(htmlTree.firstChild.firstChild),
		true,
...
QUnit.test("isXMLDoc - XML", function (assert) {
	...
	assert.ok(
		jQuery.isXMLDoc(xml.documentElement.firstChild),
		"XML child element"
	);
	...
	assert.ok(
		jQuery.isXMLDoc(svg.documentElement.firstChild),
		"SVG child element"
	);
...
QUnit.test("XSS via location.hash", function (assert) {
	...
	try {
		...
		jQuery(
			"#<img id='check9521' src='no-such-.gif' onerror='jQuery._check9521(false)'>"
...
QUnit.test("jQuery('html')", function (assert) {
	...
	assert.ok(
		jQuery["foo"],
		"Executing a script's contents in the right context"
	);

	// Test multi-line HTML
	div = jQuery("<div>\r\nsome text\n<p>some p</p>\nmore text\r\n</div>")[0];
	assert.equal(
		div.nodeName.toUpperCase(),
		"DIV",
		"Make sure we're getting a div."
	);
	...
	assert.ok(
		...
		"Create an input and set the type."
	);
	...
	assert.ok(
		!jQuery("<option>test</option>")[0].selected,
	...
	assert.equal(
		jQuery("element[attribute='<div></div>']").length,
		0,
	...
	if (jQuery.find.compile) {
		assert.equal(
			jQuery("element:not(<div></div>)").length,
			0,
...
QUnit.test("jQuery('html', context)", function (assert) {
	...
	assert.equal(
		$span.length,
		1,
		"verify a span created with a div context works, #1763"
	);
});

QUnit.test(
	"jQuery(selector, xml).text(str) - loaded via xml document",
	function (assert) {
		assert.expect(2);
...
QUnit.test("end()", function (assert) {
	...
	assert.equal(
		"Yahoo",
		jQuery("#yahoo").text(),
		"check for non-destructive behavior"
	);
});
...
QUnit.test("toArray()", function (assert) {
	assert.expect(1);
	assert.deepEqual(
		jQuery("#qunit-fixture p").toArray(),
		q("firstp", "ap", "sndp", "en", "sap", "first"),
...
QUnit.test("inArray()", function (assert) {
	...
	jQuery.each(tests, function (key, obj) {
		assert.equal(
			jQuery.inArray(obj.elem, selections[key]),
			obj.index,
	...
	jQuery.each(falseTests, function (key, elem) {
		assert.equal(
			!!~jQuery.inArray(elem, selections[key]),
			false,
...
QUnit.test("get(Number)", function (assert) {
	...
	assert.strictEqual(
		jQuery("#firstp").get(1),
		undefined,
		"Try get with index larger elements count"
	);
});
...
QUnit.test("jQuery.map", function (assert) {
	...
	assert.equal(
		result.join(""),
		"ab",
		"Array iteration does not include undefined/null results"
	);
	...
	assert.equal(
		result.join(""),
		"ab",
		"Object iteration does not include undefined/null results"
	);
	...
	if (Array.prototype.flat) {
		...
		assert.equal(
			result.length,
			300000,
			"Able to map 300000 records without any problems (#4320)"
		);
	} else {
...
QUnit.test("jQuery.merge()", function (assert) {
	...
	assert.deepEqual(
		jQuery.merge(
			[],
			document.getElementById("lengthtest").getElementsByTagName("input")
		),
...
QUnit.test("jQuery.grep()", function (assert) {
	...
	assert.deepEqual(
		jQuery.grep(new Array(4), searchCriterion),
		[],
		"Sparse array"
	);
...
QUnit.test("jQuery.grep(Array-like)", function (assert) {
	...
	assert.deepEqual(
		jQuery.grep({ length: 0 }, searchCriterion),
		[],
		"Empty array-like"
	);
...
QUnit.test("jQuery.extend(Object, Object)", function (assert) {
	...
	assert.equal(
		deep1["foo2"],
		document,
		"Make sure that a deep clone was not attempted on the document"
	);

	assert.ok(
		jQuery.extend(true, {}, nestedarray)["arr"] !== arr,
		"Deep extend of object must clone child array"
	);

	// #5991
	assert.ok(
		Array.isArray(jQuery.extend(true, { arr: {} }, nestedarray)["arr"]),
		"Cloned array have to be an Array"
	);
	assert.ok(
		jQuery.isPlainObject(jQuery.extend(true, { arr: arr }, { arr: {} })["arr"]),
	...
	assert.deepEqual(
		empty["foo"],
		optionsWithLength["foo"],
		"The length property must copy correctly"
	);
	...
	assert.ok(
		empty["foo"] && empty["foo"]["date"] === customObject,
...
QUnit.test(
	'jQuery.extend(Object, Object {created with "defineProperties"})',
	function (assert) {
		assert.expect(2);
		...
		assert.equal(
			accessorObj.enumerableProp,
			true,
			"Verify that getters are transferred"
		);
		assert.equal(
			accessorObj.nonenumerableProp,
			undefined,
			"Verify that non-enumerable getters are ignored"
		);
	}
);

QUnit.test(
	"jQuery.extend(true,{},{a:[], o:{}}); deep copy with array, followed by object",
	function (assert) {
		assert.expect(2);
		...
		assert.deepEqual(
			result,
			initial,
			"The [result] and [initial] have equal shape and values"
		);
		assert.ok(
			!Array.isArray(result.object),
			"result.object wasn't paved with an empty array"
		);
	}
);

QUnit.test(
	"jQuery.extend( true, ... ) Object.prototype pollution",
	function (assert) {
		assert.expect(1);
...
QUnit.test("jQuery.each(Object,Function)", function (assert) {
	...
	assert.equal(
		...
		"Iteration over document.styleSheets"
	);
});
...
QUnit.test(
	"JIT compilation does not interfere with length retrieval (gh-2145)",
	function (assert) {
		assert.expect(4);
		...
		jQuery.each({ 1: "1", 2: "2", 3: "3" }, function (index) {
			assert.equal(
				++i,
				index,
				"Iteration over object with solely " +
...
QUnit.test("jQuery.makeArray", function (assert) {
	...
	assert.equal(
		jQuery.makeArray(jQuery("html>*"))[0].nodeName.toUpperCase(),
		"HEAD",
		"Pass makeArray a jQuery object"
	);

	assert.equal(
		jQuery.makeArray(document.getElementsByName("PWD")).slice(0, 1)[0].name,
		"PWD",
	...
	assert.equal(
		...
		"12",
		"Pass makeArray an arguments array"
	);

	assert.equal(
		jQuery.makeArray([1, 2, 3]).join(""),
		"123",
		"Pass makeArray a real array"
	);

	assert.equal(
		jQuery.makeArray().length,
		0,
		"Pass nothing to makeArray and expect an empty array"
	);
	...
	assert.equal(
		jQuery.makeArray(true)[0].constructor,
		Boolean,
		"Pass makeArray a boolean"
	);

	assert.equal(
		jQuery.makeArray(document.createElement("div"))[0].nodeName.toUpperCase(),
		"DIV",
		"Pass makeArray a single node"
	);

	assert.equal(
		jQuery.makeArray({ length: 2, 0: "a", 1: "b" }).join(""),
		"ab",
		"Pass makeArray an array like map (with length)"
	);

	assert.ok(
		!!jQuery.makeArray(document.documentElement.childNodes).slice(0, 1)[0]
			.nodeName,
	...
	assert.equal(
		...
		1,
		"Pass makeArray a function"
	);

	//window, also has length
	assert.equal(
		jQuery.makeArray(window)[0],
		window,
		"Pass makeArray the window"
	);

	assert.equal(
		jQuery.makeArray(/a/)[0].constructor,
		RegExp,
		"Pass makeArray a regex"
	);

	// Some nodes inherit traits of nodelists
	assert.ok(
		jQuery.makeArray(document.getElementById("form")).length >= 13,
...
QUnit.test("jQuery.inArray", function (assert) {
	...
	assert.equal(
		jQuery.inArray(0, false),
		-1,
		"Search in 'false' as array returns -1 and doesn't throw exception"
	);

	assert.equal(
		jQuery.inArray(0, null),
		-1,
		"Search in 'null' as array returns -1 and doesn't throw exception"
	);

	assert.equal(
		jQuery.inArray(0, undefined),
		-1,
...
QUnit.test("jQuery.parseHTML", function (assert) {
	...
	assert.ok(
		Array.isArray(nodes),
		"parseHTML returns an array rather than a nodelist"
	);

	html = "<script>undefined()</script>";
	assert.equal(jQuery.parseHTML(html).length, 0, "Ignore scripts by default");
	assert.equal(
		jQuery.parseHTML(html, true)[0].nodeName.toLowerCase(),
		"script",
		"Preserve scripts when requested"
	);

	html += "<div></div>";
	assert.equal(
		jQuery.parseHTML(html)[0].nodeName.toLowerCase(),
		"div",
		"Preserve non-script nodes"
	);
	assert.equal(
		jQuery.parseHTML(html, true)[0].nodeName.toLowerCase(),
		"script",
		"Preserve script position"
	);

	assert.equal(
		jQuery.parseHTML("text")[0].nodeType,
		3,
		"Parsing text returns a text node"
	);
	assert.equal(
		jQuery.parseHTML("\t<div></div>")[0].nodeValue,
		"\t",
		"Preserve leading whitespace"
	);

	assert.equal(
		jQuery.parseHTML(" <div></div> ")[0].nodeType,
		3,
	...
	assert.equal(
		html[0].parentNode.nodeType,
		11,
		"parentNode should be documentFragment"
	);
	assert.equal(html[0].innerHTML, "test div", "Content should be preserved");

	assert.equal(
		jQuery.parseHTML("<span><span>").length,
		1,
		"Incorrect html-strings should not break anything"
	);
	assert.equal(
		jQuery.parseHTML("<td><td>")[1].parentNode.nodeType,
		11,
		"parentNode should be documentFragment for wrapMap (variable in manipulation module) elements too"
	);
	assert.ok(
		jQuery.parseHTML("<#if><tr><p>This is a test.</p></tr><#/if>") || true,
...
QUnit.test("jQuery.parseHTML(<a href>) - gh-2965", function (assert) {
	...
	assert.ok(
		/\/example\.html$/.test(href),
		"href is not lost after parsing anchor"
	);
});
...
if (jQuery.support.createHTMLDocument) {
	QUnit.test("jQuery.parseHTML", function (assert) {
		...
		window.setTimeout(function () {
			assert.equal(
				window.parseHTMLError,
				false,
				"onerror eventhandler has not been called."
			);
			done();
		}, 2000);
	});
...
QUnit.test("jQuery.parseXML", function (assert) {
	...
	try {
		...
		assert.strictEqual(
			tmp.childNodes[0].nodeValue,
			"well-formed",
			"<b> text is as expected"
		);
	} catch (e) {
...
QUnit[
	document.documentMode || /edge\//i.test(navigator.userAgent) ? "skip" : "test"
	...
	testIframe(
		...
		function (assert, jQuery, window, document, ready) {
			...
			assert.equal(
				true,
				ready,
				"document ready correctly fired when jQuery is loaded after DOMContentLoaded"
			);
		}
	);
}
...
QUnit[jQuery.Deferred ? "test" : "skip"](
	"jQuery.readyException (original)",
	function (assert) {
		assert.expect(1);
