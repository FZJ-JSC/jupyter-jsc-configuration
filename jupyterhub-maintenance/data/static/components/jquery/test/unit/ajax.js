...
(function () {
	...
	ajaxTest("jQuery.ajax() - success callbacks", 8, function (assert) {
		return {
			setup: addGlobalEvents(
				"ajaxStart ajaxStop ajaxSend ajaxComplete ajaxSuccess",
				assert
			),
			url: url("name.html"),
			beforeSend: function () {
	...
	ajaxTest(
		"jQuery.ajax() - success callbacks - (url, options) syntax",
		8,
		function (assert) {
			return {
				setup: addGlobalEvents(
	...
	ajaxTest(
		"jQuery.ajax() - execute js for crossOrigin when dataType option is provided",
		3,
		function (assert) {
			return {
				create: function (options) {
					...
					return jQuery.ajax(
						url("mock.php?action=script&header=ecma"),
						options
					);
				},
				success: function () {
	...
	ajaxTest(
		"jQuery.ajax() - custom attributes for script tag",
		4,
		function (assert) {
			return {
				create: function (options) {
					...
					assert.equal(
						jQuery("#jquery-ajax-test").attr("async"),
						"async",
						"attr value"
					);
					return xhr;
	...
	ajaxTest(
		"jQuery.ajax() - do not execute js (crossOrigin)",
		2,
		function (assert) {
			return {
				create: function (options) {
	...
	ajaxTest(
		"jQuery.ajax() - success callbacks (late binding)",
		8,
		function (assert) {
			return {
				setup: addGlobalEvents(
	...
	ajaxTest(
		"jQuery.ajax() - success callbacks (oncomplete binding)",
		8,
		function (assert) {
			return {
				setup: addGlobalEvents(
	...
	ajaxTest("jQuery.ajax() - error callbacks", 8, function (assert) {
		return {
			setup: addGlobalEvents(
				"ajaxStart ajaxStop ajaxSend ajaxComplete ajaxError",
				assert
			),
	...
	ajaxTest(
		"jQuery.ajax() - textStatus and errorThrown values",
		4,
		function (assert) {
			return [
				{
					...
					error: function (_, textStatus, errorThrown) {
						assert.strictEqual(
							textStatus,
							"abort",
							"textStatus is 'abort' for abort"
						);
						assert.strictEqual(
							errorThrown,
							"abort",
							"errorThrown is 'abort' for abort"
						);
					},
					afterSend: function (request) {
						request.abort();
				...
				},
				{
					...
					error: function (_, textStatus, errorThrown) {
						assert.strictEqual(
							textStatus,
							"mystatus",
							"textStatus is 'mystatus' for abort('mystatus')"
						);
						assert.strictEqual(
							errorThrown,
							"mystatus",
							"errorThrown is 'mystatus' for abort('mystatus')"
						);
					},
					afterSend: function (request) {
		...
		return {
			...
			error: function (xhr) {
				assert.strictEqual(
					xhr.responseText,
					"plain text message",
					"Test jqXHR.responseText is filled for HTTP errors"
				);
			},
		};
	});
	...
	QUnit.test(
		"jQuery.ajax() - retry with jQuery.ajax( this )",
		function (assert) {
			assert.expect(2);
			var previousUrl,
				firstTime = true,
	...
	ajaxTest("jQuery.ajax() - headers", 8, function (assert) {
		return {
			...
			url: url(
				"mock.php?action=headers&keys=siMPle|SometHing-elsE|OthEr|Nullable|undefined|Empty|ajax-send"
			),
			headers: {
				siMPle: "value",
			...
			success: function (data, _, xhr) {
				...
				assert.strictEqual(
					xhr.getResponseHeader("Sample-Header"),
					"Hello World",
					"Sample header received"
				);
				assert.ok(
					data.indexOf("undefined") < 0,
					"Undefined header value was not sent"
				);
				...
				assert.strictEqual(
					xhr.getResponseHeader("Sample-Header2"),
					"Hello World 2",
					"Second sample header received"
				);
				...
				if (isAndroid) {
					...
					assert.strictEqual(
						xhr.getResponseHeader("List-Header"),
						"Item 1, Item 2",
						"List header received"
					);
				}
				...
				if (isAndroid && QUnit.isSwarm) {
					...
					assert.ok(
						true,
						'BrowserStack\'s Android fails the "prototype collision (constructor)" test'
					);
				} else {
					assert.strictEqual(
						xhr.getResponseHeader("constructor"),
						"prototype collision (constructor)",
						"constructor header received"
					);
				}
				assert.strictEqual(
					xhr.getResponseHeader("__proto__"),
					null,
					"Undefined __proto__ header not received"
				);
			},
		};
	});
	...
	ajaxTest("jQuery.ajax() - protocol-less urls", 1, function (assert) {
		return {
			...
			beforeSend: function (xhr, settings) {
				assert.equal(
					settings.url,
					location.protocol + "//somedomain.com",
	...
	ajaxTest(
		"jQuery.ajax() - URL fragment component preservation",
		4,
		function (assert) {
			return [
				{
				...
				},
				{
					...
					beforeSend: function (xhr, settings) {
						assert.equal(
							settings.url,
							baseURL + "name.html?abc&test=123#foo",
				...
				},
				{
					...
					beforeSend: function (xhr, settings) {
						...
						assert.equal(
							url,
							baseURL + "name.html?abc&devo=hat&_=#brownies",
		...
		return [
			{
				...
				beforeSend: function (xhr, settings) {
					assert.equal(
						settings.url,
						"/?devo=hat&answer=42&quux=a%20space",
						"Simple case"
					);
					return false;
				},
			...
			},
			{
				...
				beforeSend: function (xhr, settings) {
					assert.equal(
						settings.url,
						"/?a=1&a=2&a=3&b%5B%5D=b1&b%5B%5D=b2",
						"Arrays"
					);
					return false;
				},
	...
	ajaxTest("jQuery.ajax() - cross-domain detection", 8, function (assert) {
			...
			return jQuery.extend(
				{
					...
					beforeSend: function (_, s) {
						assert.ok(
							crossDomainOrOptions === false ? !s.crossDomain : s.crossDomain,
							title
						);
						return false;
					},
		...
		return [
			...
			request("app:/path", "Adobe AIR app:/ URL detected as cross-domain"),
			request(
			...
			request("about:blank", "Test about:blank is detected as cross-domain"),
			request(
	...
	ajaxTest("jQuery.ajax() - abort", 9, function (assert) {
		return {
			setup: addGlobalEvents(
				"ajaxStart ajaxStop ajaxSend ajaxError ajaxComplete",
				assert
			),
			url: url("mock.php?action=wait&wait=5"),
			beforeSend: function () {
			...
			afterSend: function (xhr) {
				assert.strictEqual(
					xhr.readyState,
					1,
					"XHR readyState indicates successful dispatch"
				);
				xhr.abort();
				assert.strictEqual(
					xhr.readyState,
					0,
					"XHR readyState indicates successful abortion"
				);
			},
			error: true,
			complete: function () {
			...
			return {
				...
				error: function (xhr, msg) {
					assert.strictEqual(
						msg,
						"error",
						"Native abort triggers error callback"
					);
				},
				complete: function () {
					assert.ok(true, "complete");
			...
			return {
				...
				error: function (xhr, msg) {
					assert.strictEqual(
						msg,
						"error",
						"Native timeout triggers error callback"
					);
				},
				complete: function () {
					assert.ok(true, "complete");
		...
		function nocallback(msg) {
			return function () {
				assert.equal(
					typeof this.url,
					"string",
					"context is settings on callback " + msg
				);
			};
		}
		return {
	...
	ajaxTest(
		"#15118 - jQuery.ajax() - function without jQuery.event",
		1,
		function (assert) {
			var holder;
			return {
	...
	ajaxTest(
		"#15160 - jQuery.ajax() - request manually aborted in ajaxSend",
		3,
		function (assert) {
			return {
				setup: function () {
					...
					jQuery(document).on("ajaxError ajaxComplete", function (e, jqXHR) {
						assert.equal(
							jqXHR.statusText,
							"abort",
							"jqXHR.statusText equals abort on global ajaxComplete and ajaxError events"
						);
					});
				},
				url: url("name.html"),
	...
	ajaxTest(
		"jQuery.ajax() - context modification through ajaxSetup",
		3,
		function (assert) {
			var obj = {};
			return {
	...
	ajaxTest(
		"jQuery.ajax() - xml: non-namespace elements inside namespaced elements",
		3,
		function (assert) {
			return {
				...
				success: function (resp) {
					assert.equal(
						jQuery("properties", resp).length,
						1,
						"properties in responseXML"
					);
					assert.equal(
						jQuery("jsconf", resp).length,
						1,
						"jsconf in responseXML"
					);
					assert.equal(
						jQuery("thing", resp).length,
						2,
						"things in responseXML"
					);
				},
			};
		}
	...
	ajaxTest(
		"jQuery.ajax() - xml: non-namespace elements inside namespaced elements (over JSONP)",
		3,
		function (assert) {
			return {
		...
		return [
			{
				...
				success: function (data, status, xhr) {
					assert.ok(
						/Date/i.test(xhr.getAllResponseHeaders()),
	...
	ajaxTest(
		"jQuery.ajax() - beforeSend, cancel request manually",
		2,
		function (assert) {
			return {
				create: function () {
				...
				fail: function (_, reason) {
					assert.strictEqual(
						reason,
						"canceled",
						"canceled request must fail with 'canceled' status text"
					);
				},
			};
		}
		...
		return {
			...
			success: function (data) {
				...
				assert.strictEqual(
					window["testFoo"],
					"foo",
					"Check if script was evaluated for datatype html"
				);
				assert.strictEqual(
					window["testBar"],
					"bar",
					"Check if script src was evaluated for datatype html"
				);
			},
		};
	});
	...
	ajaxTest(
		"jQuery.ajax() - do execute scripts if JSONP from unsuccessful responses",
		1,
		function (assert) {
	...
	ajaxTest(
		"jQuery.ajax() - do not execute scripts from unsuccessful responses (gh-4250)",
		11,
		function (assert) {
			...
			return [
				...
				request("HTML reply with dataType", {
					dataType: "script",
					url: url("404.txt"),
				}),
				request("script reply", {
					url: url("mock.php?action=errorWithScript&withScriptContentType"),
				}),
				request("non-script reply", {
					url: url("mock.php?action=errorWithScript"),
				}),
				request("script reply with dataType", {
					dataType: "script",
					url: url("mock.php?action=errorWithScript&withScriptContentType"),
				}),
				request("non-script reply with dataType", {
					dataType: "script",
					url: url("mock.php?action=errorWithScript"),
				}),
				request("script reply with converter", {
					converters: failConverters,
					url: url("mock.php?action=errorWithScript&withScriptContentType"),
				}),
				request("non-script reply with converter", {
					converters: failConverters,
					url: url("mock.php?action=errorWithScript"),
				}),
				request("script reply with converter and dataType", {
					converters: failConverters,
					dataType: "script",
					url: url("mock.php?action=errorWithScript&withScriptContentType"),
				}),
				request("non-script reply with converter and dataType", {
					converters: failConverters,
					dataType: "script",
					url: url("mock.php?action=errorWithScript"),
				}),
				request("JSONP reply with dataType", {
					dataType: "jsonp",
					url: url("mock.php?action=errorWithScript"),
					beforeSend: function () {
						jQuery.globalEval = function (response) {
							assert.ok(
								/"status": 404, "msg": "Not Found"/.test(response),
	...
	ajaxTest(
		"jQuery.ajax() - synchronous request with callbacks",
		2,
		function (assert) {
			return {
				url: url("json_obj.js"),
	...
	QUnit.test(
		"jQuery.ajax(), jQuery.get[Script|JSON](), jQuery.post(), pass-through request object",
		function (assert) {
			assert.expect(8);
			...
			jQuery(document).one("ajaxStop", function () {
				...
				assert.equal(
					errorCount,
					0,
					"Check no ajax errors (status" + errorEx + ")"
				);
			...
			assert.ok(
				jQuery.getScript(url("mock.php?action=testbar"), success),
				"script"
			);
	...
	ajaxTest("jQuery.ajax() - cache", 28, function (assert) {
		...
		function request(url, title) {
			return {
				...
				beforeSend: function () {
					...
					// URL sanity check
					assert.equal(
						this.url.indexOf(rootUrl),
						0,
						"root url not mangled: " + this.url
					);
					assert.equal(
						/\&.*\?/.test(this.url),
						false,
						"parameter delimiters in order"
					);
					...
					while ((tmp = re.exec(this.url))) {
						assert.strictEqual(
							parameter,
							undefined,
							title + ": only one 'no-cache' parameter"
						);
						parameter = tmp[1];
		...
		return [
			request(rootUrl, "no query"),
			request(rootUrl + "?", "empty query"),
			request(rootUrl + "?pizza=true", "1 parameter"),
			request(rootUrl + "?_=tobereplaced555", "_= parameter"),
			request(rootUrl + "?pizza=true&_=tobereplaced555", "1 parameter and _="),
			request(rootUrl + "?_=tobereplaced555&tv=false", "_= and 1 parameter"),
			request(
	...
	jQuery.each(
		...
		function (crossDomain, label) {
			ajaxTest(
				"jQuery.ajax() - JSONP - Query String (?n)" + label,
				4,
				function (assert) {
					return [
						{
			...
			ajaxTest(
				"jQuery.ajax() - JSONP - Explicit callback param" + label,
				10,
				function (assert) {
					return {
						setup: function () {
						...
						requests: [
							...
							},
							{
								...
								success: function (data) {
									...
									xhr.fail(function () {
										assert.ok(
											true,
											"Ajax error JSON (GET, custom callback name to be cleaned up)"
										);
										assert.strictEqual(
											window["functionToCleanUp"],
											true,
							...
							},
							{
								...
								beforeSend: function () {
									assert.ok(
										/action=jsonp&callback=XXX&_=\d+$/.test(this.url),
			...
			ajaxTest(
				"jQuery.ajax() - JSONP - Callback in data" + label,
				2,
				function (assert) {
					return [
						{
	...
	ajaxTest(
		"jQuery.ajax() - script, Remote with scheme-less URL",
		2,
		function (assert) {
			return {
				setup: function () {
	...
	ajaxTest("jQuery.ajax() - malformed JSON", 2, function (assert) {
		return {
			...
			error: function (xhr, msg, detailedMsg) {
				...
				assert.ok(
					/(invalid|error|exception)/i.test(detailedMsg),
	...
	ajaxTest(
		"jQuery.ajax() - JSON by content-type disabled with options",
		6,
		function (assert) {
			return {
				url: url("mock.php?action=json"),
	...
	ajaxTest(
		"jQuery.ajax() - data option - empty bodies for non-GET requests",
		1,
		function (assert) {
			return {
	...
	ajaxTest(
		"jQuery.ajax() - data - x-www-form-urlencoded (gh-2658)",
		1,
		function (assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest(
		"jQuery.ajax() - don't escape %20 with contentType override (gh-4119)",
		1,
		function (assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest(
		"jQuery.ajax() - escape %20 with contentType override (gh-4119)",
		1,
		function (assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest(
		"jQuery.ajax() - override contentType with header (gh-4119)",
		1,
		function (assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest(
		"jQuery.ajax() - data - process string with GET",
		2,
		function (assert) {
			return {
				url: "bogus.html",
		...
		function (label, cache) {
			jQuery.each(
				...
				function (type, url) {
					...
					QUnit.test(
						"jQuery.ajax() - " + type + " support" + label,
						function (assert) {
							assert.expect(4);
							var done = assert.async();
							jQuery.ajax({
									...
									jQuery.ajax({
										...
										success: function (data, status, jqXHR) {
											assert.strictEqual(
												status,
												"notmodified",
												"Following status is 'notmodified'"
											);
											assert.strictEqual(
												jqXHR.status,
												304,
	...
	ajaxTest(
		"jQuery.ajax() - failing cross-domain (non-existing)",
		1,
		function (assert) {
			return {
				// see RFC 2606
	...
	QUnit.test("jQuery.ajax() - statusText", function (assert) {
		...
		var done = assert.async();
		jQuery
			.ajax(url("mock.php?action=status&code=200&text=Hello"))
			.done(function (_, statusText, jqXHR) {
				assert.strictEqual(
					statusText,
					"success",
					"callback status text ok for success"
				);
				assert.ok(
					jqXHR.statusText === "Hello" || jqXHR.statusText === "OK",
					"jqXHR status text ok for success (" + jqXHR.statusText + ")"
				);
				jQuery
					.ajax(url("mock.php?action=status&code=404&text=World"))
					.fail(function (jqXHR, statusText) {
						assert.strictEqual(
							statusText,
							"error",
							"callback status text ok for error"
						);
						done();
					});
			});
			...
			function (uri, isSuccess) {
				...
				jQuery.ajax(url(uri), {
					complete: function (jqXHR) {
						setTimeout(function () {
							jqXHR.statusCode(
								createStatusCodes("very late binding", isSuccess)
							);
							countComplete();
						}, 100);
					},
				...
				jQuery
					.ajax(url(uri), {
						...
						complete: function (jqXHR) {
							jqXHR.statusCode(
								createStatusCodes("all (on complete)", isSuccess)
							);
							setTimeout(function () {
								jqXHR.statusCode(
									createStatusCodes("all (very late binding)", isSuccess)
								);
								countComplete();
							}, 100);
						},
					...
					.statusCode(
						createStatusCodes("all (immediately with method)", isSuccess)
					);
				...
				jQuery.ajax(url(uri), {
					...
					complete: function () {
						assert.strictEqual(
							testString,
							"AB",
							"Test statusCode callbacks are ordered like " +
		...
		return [
			{
				...
				success: function () {
					...
					assert.strictEqual(
						this.dataTypes[0],
						"text",
						"response was retrieved as text"
					);
					assert.strictEqual(
						this.dataTypes[1],
						"myjson",
						"request expected myjson dataType"
					);
				},
			},
			{
	...
	ajaxTest(
		"jQuery.ajax() - empty json gets to error callback instead of success callback.",
		1,
		function (assert) {
			return {
				...
				error: function (_, __, error) {
					assert.equal(
						typeof error === "object",
						true,
						"Didn't get back error object for empty json response"
					);
				},
				dataType: "json",
			};
	...
	ajaxTest(
		"#2688 - jQuery.ajax() - beforeSend, cancel request",
		2,
		function (assert) {
			return {
				create: function () {
	...
	ajaxTest(
		"#2806 - jQuery.ajax() - data option - evaluate function values",
		1,
		function (assert) {
			return {
	...
	QUnit.test(
		"#7531 - jQuery.ajax() - Location object as url",
		function (assert) {
			assert.expect(1);
	...
	jQuery.each(
		...
		function (crossDomain, label) {
			ajaxTest(
				"#7578 - jQuery.ajax() - JSONP - default for cache option" + label,
				1,
				function (assert) {
					return {
						url: baseURL + "mock.php?action=jsonp",
						...
						beforeSend: function () {
							assert.strictEqual(
								this.cache,
								false,
								"cache must be false on JSON request"
							);
							return false;
						},
						error: true,
	...
	ajaxTest(
		"#8107 - jQuery.ajax() - multiple method signatures introduced in 1.5",
		4,
		function (assert) {
			return [
				{
	...
	jQuery.each(
		...
		function (crossDomain, label) {
			ajaxTest(
				"#8205 - jQuery.ajax() - JSONP - re-use callbacks name" + label,
				4,
				function (assert) {
					return {
						url: baseURL + "mock.php?action=jsonp",
	...
	QUnit.test(
		"#9887 - jQuery.ajax() - Context with circular references (#9887)",
		function (assert) {
			assert.expect(2);
	...
	jQuery.each(
		["as argument", "in settings object"],
		function (inSetting, title) {
			...
			ajaxTest(
				"#10093 - jQuery.ajax() - falsy url " + title,
				4,
				function (assert) {
					return [
	...
	ajaxTest("#11151 - jQuery.ajax() - parse error body", 2, function (assert) {
		return {
			...
			error: function (jqXHR) {
				assert.strictEqual(
					jqXHR.responseText,
					'{ "code": 40, "message": "Bad Request" }',
					"Error body properly set"
				);
				assert.deepEqual(
					jqXHR.responseJSON,
					{ code: 40, message: "Bad Request" },
					"Error body properly parsed"
				);
			},
		};
	});
	...
	ajaxTest(
		"#11426 - jQuery.ajax() - loading binary data shouldn't throw an exception in IE",
		1,
		function (assert) {
			return {
				url: url("1x1.jpg"),
	...
	if (
		...
		ajaxTest(
			"gh-2498 - jQuery.ajax() - binary data shouldn't throw an exception",
			2,
			function (assert) {
				return {
					url: url("1x1.jpg"),
					...
					success: function (data, s, jqxhr) {
						...
						assert.ok(
							jqxhr.response instanceof window.ArrayBuffer,
							"data in jQXHR"
						);
					},
				};
	...
	QUnit.test(
		"#11743 - jQuery.ajax() - script, throws exception",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
	...
	ajaxTest(
		"#13276 - jQuery.ajax() - compatibility between XML documents from ajax requests and parsed string",
		1,
		function (assert) {
			return {
				...
				success: function (ajaxXML) {
					var parsedXML = jQuery(
						jQuery.parseXML('<tab title="Added">blibli</tab>')
					).find("tab");
					...
					assert.strictEqual(
						ajaxXML.find("tab").length,
						3,
						"Parsed node was added properly"
					);
				},
			};
		}
	...
	ajaxTest(
		"#13292 - jQuery.ajax() - converter is bypassed for 204 requests",
		3,
		function (assert) {
			return {
	...
	ajaxTest(
		"#13922 - jQuery.ajax() - converter is bypassed for HEAD requests",
		3,
		function (assert) {
			return {
				url: baseURL + "mock.php?action=json",
	...
	ajaxTest(
		"#14683 - jQuery.ajax() - Exceptions thrown synchronously by xhr.send should be caught",
		4,
		function (assert) {
			return [
				{
	...
	ajaxTest(
		"gh-2587 - when content-type not xml, but looks like one",
		1,
		function (assert) {
			return {
	...
	ajaxTest(
		"gh-2587 - when content-type not json, but looks like one",
		1,
		function (assert) {
			return {
				url: url("mock.php?action=contentType"),
	...
	ajaxTest(
		"gh-2587 - when content-type not html, but looks like one",
		1,
		function (assert) {
			return {
				url: url("mock.php?action=contentType"),
	...
	ajaxTest(
		"gh-2587 - when content-type not javascript, but looks like one",
		1,
		function (assert) {
			return {
				url: url("mock.php?action=contentType"),
	...
	ajaxTest(
		"gh-2587 - when content-type not ecmascript, but looks like one",
		1,
		function (assert) {
			return {
				url: url("mock.php?action=contentType"),
		...
		return {
			...
			fail: function (_, reason) {
				assert.strictEqual(
					reason,
					"canceled",
					"Request aborted by the prefilter must fail with 'canceled' status text"
				);
			},
		};
	});
	...
	QUnit.test(
		"jQuery.ajaxSetup({ timeout: Number }) with localtimeout",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
			jQuery.ajaxSetup({
	...
	QUnit.test(
		"#11264 - jQuery.domManip() - no side effect because of ajaxSetup or global events",
		function (assert) {
			assert.expect(1);
			...
			jQuery("#qunit-fixture").append(
				"<script src='" + baseURL + "mock.php?action=script'></script>"
			);
	...
	QUnit.test(
		"jQuery.get( String, Hash, Function ) - parse xml and use text() on nodes",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
	...
	QUnit.test(
		"#8277 - jQuery.get( String, Function ) - data in ajaxSettings",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
			jQuery.ajaxSetup({
	...
	QUnit.test(
		"jQuery.getJSON( String, Hash, Function ) - JSON array",
		function (assert) {
			assert.expect(5);
			var done = assert.async();
			jQuery.getJSON(
	...
	QUnit.test(
		"jQuery.getJSON( String, Function ) - JSON object",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
	...
	QUnit.test(
		"jQuery.getJSON( String, Function ) - JSON object with absolute url to local content",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
			...
			if (!/^[a-z][a-z0-9+.-]*:/i.test(absoluteUrl)) {
				...
				if (absoluteUrl.charAt(0) === "/") {
					absoluteUrl =
						window.location.href.replace(/(:\/*[^/]*).*$/, "$1") + absoluteUrl;

					// A relative path replaces the last slash-separated path segment
				} else {
					absoluteUrl =
						window.location.href.replace(/[^/]*$/, "") + absoluteUrl;
				}
			}
	...
	QUnit.test(
		"jQuery.getScript( String, Function ) - with callback",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
	...
	QUnit.test(
		"jQuery.getScript( String, Function ) - no callback",
		function (assert) {
			assert.expect(1);
			Globals.register("testBar");
	...
	QUnit.test(
		"#8082 - jQuery.getScript( String, Function ) - source as responseText",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
	...
	QUnit.test("jQuery.getScript( Object ) - no callback", function (assert) {
		...
		Globals.register("testBar");
		jQuery
			.getScript({ url: url("mock.php?action=testbar") })
			.done(assert.async());
	});
	...
	QUnit.test("jQuery.fn.load( URL_SELECTOR )", function (assert) {
		...
		jQuery("#first").load(baseURL + "test3.html div.user", function () {
			assert.strictEqual(
				...
				2,
				"Verify that specific elements were injected"
			);
			done();
		});
	});
	...
	QUnit.test(
		"jQuery.fn.load( URL_SELECTOR with non-HTML whitespace(#3003) )",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
			jQuery("#first").load(
				baseURL + "test3.html   #whitespace\\\\xA0 ",
				function () {
					assert.strictEqual(
						jQuery(this).children("div").length,
						1,
	...
	QUnit.test(
		"jQuery.fn.load( String, Function ) - simple: inject text into DOM",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
			jQuery("#first").load(url("name.html"), function () {
				assert.ok(
					/^ERROR/.test(jQuery("#first").text()),
	...
	QUnit.test(
		"jQuery.fn.load( String, Function ) - check scripts",
		function (assert) {
			assert.expect(7);
			var done = assert.async();
			var verifyEvaluation = function () {
				assert.strictEqual(
					window["testBar"],
					"bar",
					"Check if script src was evaluated after load"
				);
				assert.strictEqual(
					jQuery("#ap").html(),
					"bar",
					"Check if script evaluation has modified DOM"
				);
				done();
			};
			...
			jQuery("#first").load(
				...
				function () {
					...
					assert.strictEqual(
						window["testFoo"],
						"foo",
						"Check if script was evaluated after load"
					);
	...
	QUnit.test(
		"jQuery.fn.load( String, Function ) - check file with only a script tag",
		function (assert) {
			assert.expect(3);
			var done = assert.async();
			Globals.register("testFoo");
	...
	QUnit.test(
		"jQuery.fn.load( String, Function ) - dataFilter in ajaxSettings",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
			jQuery.ajaxSetup({
			...
			jQuery("<div></div>").load(url("name.html"), function (responseText) {
				assert.strictEqual(
					jQuery(this).html(),
					"Hello World",
					"Test div was filled with filtered data"
				);
				assert.strictEqual(
					responseText,
					"Hello World",
					"Test callback receives filtered data"
				);
				done();
			});
		}
			...
			function () {
				...
				assert.ok(
					$node
						...
						.text()
						.match(/foo=3&bar=ok/),
					"Check if a string of data is passed correctly"
				);
				done();
			}
		);
	...
	QUnit.test(
		"jQuery.fn.load() - callbacks get the correct parameters",
		function (assert) {
			assert.expect(8);
			var completeArgs = {},
				done = assert.async();
						...
						function (options) {
							return jQuery.Deferred(function (defer) {
								jQuery("#foo").load(options.url, function () {
									var args = arguments;
									assert.strictEqual(
										completeArgs[options.url].length,
										args.length,
									...
									jQuery.each(completeArgs[options.url], function (i, value) {
										assert.strictEqual(
											args[i],
											value,
											"argument #" + i + " is the same (" + options.type + ")"
										);
									});
									defer.resolve();
								});
	...
	QUnit.test(
		"#2046 - jQuery.fn.load( String, Function ) with ajaxSetup on dataType json",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
			...
			jQuery(document).on("ajaxComplete", function (e, xml, s) {
				assert.strictEqual(
					s.dataType,
					"html",
					"Verify the load() dataType was html"
				);
	...
	QUnit.test(
		"#10524 - jQuery.fn.load() - data specified in ajaxSettings is merged in",
		function (assert) {
			assert.expect(1);
			var done = assert.async();
			...
			.when(
				jQuery.post(
					...
					function (xml) {
						jQuery("math", xml).each(function () {
							assert.strictEqual(
								jQuery("calculation", this).text(),
								"5-2",
								"Check for XML"
							);
							assert.strictEqual(
								jQuery("result", this).text(),
								"3",
								"Check for XML"
							);
						});
				...
				jQuery.ajax({
					...
					success: function (data) {
						assert.strictEqual(
							data,
							"test%5Blength%5D=7&test%5Bfoo%5D=bar",
	...
	QUnit.test(
		"jQuery.post( String, Hash, Function ) - simple with xml",
		function (assert) {
			assert.expect(4);
			var done = assert.async();
	...
	QUnit.test(
		"jQuery[get|post]( options ) - simple with xml",
		function (assert) {
			assert.expect(2);
			var done = assert.async();
	...
	QUnit.test("jQuery.active", function (assert) {
		...
		assert.ok(
			jQuery.active === 0,
			"ajax active counter should be zero: " + jQuery.active
		);
	});
})();
