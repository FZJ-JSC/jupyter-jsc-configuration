(function () {
	...
	QUnit.test("Animation( subject, props, opts ) - shape", function (assert) {
		...
		assert.notEqual(
			animation.props,
			props,
			".props is not the original however"
		);
		assert.deepEqual(
			animation.props,
			props,
			".props is a copy of the original"
		);
		...
		assert.equal(
			typeof animation.tweens[0].run,
			"function",
			"which has a .run function"
		);

		assert.equal(
			typeof animation.createTween,
			"function",
			".createTween is a function"
		);
		...
		assert.equal(
			jQuery.timers[0].elem,
			subject,
			"...with .elem as the subject"
		);
		assert.equal(
			jQuery.timers[0].anim,
			animation,
			"...with .anim as the animation"
		);
	...
	QUnit.test(
		"Animation.prefilter( fn ) - calls prefilter after defaultPrefilter",
		function (assert) {
			assert.expect(1);
			...
			jQuery.Animation({}, {}, {});
			assert.ok(
				prefilter.calledAfter(defaultSpy),
				"our prefilter called after"
			);
		}
	);

	QUnit.test(
		"Animation.prefilter( fn, true ) - calls prefilter before defaultPrefilter",
		function (assert) {
			assert.expect(1);
			...
			jQuery.Animation({}, {}, {});
			assert.ok(
				prefilter.calledBefore(defaultSpy),
				"our prefilter called before"
			);
		}
	);
	...
	QUnit.test("Animation.prefilter - prefilter return hooks", function (assert) {
		...
		var animation,
			...
			prefilter = this.sandbox.spy(function () {
				...
				assert.deepEqual(
					realAnimation.originalProperties,
					props,
					"originalProperties"
				);
		...
		assert.equal(
			jQuery.fx.timer.callCount,
			0,
			"Returning something never queues a timer"
		);
		assert.equal(
			animation,
			ourAnimation,
		...
		assert.equal(
			queueSpy.callCount,
			1,
			"Next function in queue called after complete"
		);

		assert.equal(
			prefilter.callCount,
			2,
			"Called prefilter again - animation #2"
		);
		assert.equal(
			ourAnimation.stop.callCount,
			0,
			".stop() on our animation hasn't been called"
		);
		...
		assert.ok(
			queueSpy.calledAfter(ourAnimation.stop),
			"After our animation was told to stop"
		);
		...
		assert.ok(
			ourAnimation.stop.calledWith(true),
			".stop(true) calls .stop(true)"
		);
		assert.ok(
			queueSpy.calledAfter(ourAnimation.stop),
	...
	QUnit.test(
		"Animation.tweener( fn ) - unshifts a * tweener",
		function (assert) {
			assert.expect(2);
	...
	QUnit.test(
		"Animation.tweener( 'prop', fn ) - unshifts a 'prop' tweener",
		function (assert) {
			assert.expect(4);
