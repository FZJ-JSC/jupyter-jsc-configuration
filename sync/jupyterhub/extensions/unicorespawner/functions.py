...
from .input_files import get_reservations, get_resources
...
async def options_form(spawner):
    ...
    if 'HDF-Cloud' in maintenance_list:
        ...
        hdf_to_remove = [k for k,v in hdf_cloud.items() if "vos" in v.keys() and vo_active not in v["vos"]]
    ...
    def regroup(x):
        ...
        if not sys:
            spawner.log.error(f"No system defined in {map_systems_path} for {groups_[1]}")
        partition = get_partition(groups_[1])
        if not partition:
            spawner.log.error(f"No system defined in {map_partitions_path} for {groups_[1]}")
        groups = [
                groups_[0],
    ...
    systems = sorted(
        {group[1] for group in user_hpc_list if group[1] not in maintenance_list and ("vos" not in unicore_config.get(group[1].upper(), {}).keys() or vo_active in unicore_config.get(group[1].upper())["vos"] ) }
    )
    ...
    def skip_resources(key, value):
        if key.startswith("resource_"):
            if formdata.get('system_input')[0] == 'hdfcloud':
                return True
            elif formdata.get('partition_input')[0] in ['LoginNode', 'LoginNodeVis', 'LoginBooster']:
                return True
            else:
