...
c.ConfigurableHTTPProxy.extra_spawn_routes_target = f'http://{my_pod_ip}.jupyterjsc.pod.cluster.local:8001'
import time
...
c.UnityOAuthenticator.extra_authorize_params_allowed_arguments = dict(uy_select_authn=["jupyterhdfaaiAuthn.hbp", "jupyterldapAuthn.password"])
...
c.JupyterHub.extra_handlers = [ (r'/api/tunneling/([^/]+)/([^/]+)/([^/]+)/([^/]+)/([^/]+)', setuptunnel.SetupTunnelAPIHandler),
                                (r'/api/vo/([^/]+)', vo.VOAPIHandler),
                                (r'/api/votoken/([^/]+)', vo.VOTokenAPIHandler),
                                (r'/api/unicorenotification/([^/]+)', unicorenotification.UNICORENotificationAPIHandler),
                                (r'/api/unicorenotification/([^/]+)/([^/]+)', unicorenotification.UNICORENotificationAPIHandler),
                                (r'/api/unicorenotification/([^/]+)/', unicorenotification.UNICORENotificationAPIHandler),
                                (r'/api/unicorenotification/([^/]+)/([^/]+)/', unicorenotification.UNICORENotificationAPIHandler),
