...
function moveBarToFull(progText, animationTime, setRed) {
  ...
  if ( pbleft_old == 0 || pbleft_old == "0deg") {
    ...
    delay_left = new Date().getTime() / 1000 - pbleft.style.getPropertyValue('--delay_start_left');
  }
  if ( pbright_old == 0 || pbright_old == "0deg" ) {
    pbright.style.setProperty('--delay_start_right', new Date().getTime() / 1000);
    delay_right = 0;
  } else {
    delay_right = new Date().getTime() / 1000 - pbright.style.getPropertyValue('--delay_start_right');
  }
  ...
  if (pbright_new == 180 && pbright_old != "180deg" && pbleft_new > 0) {
    ...
    $("#pbright").css('animation', "loading-rp "+duration_right+"s linear "+ delay_right+"s forwards");
    $("#pbleft").css('animation', "loading-lp "+duration_left+"s linear "+ delay_left+"s forwards");
  } else {
    //console.log("Right: " + animationTime + " - " +delay_right);
    $("#pbright").css('animation', "loading-rp "+ animationTime +"s linear "+ delay_right+"s forwards");
    if (pbleft_new != 0) {
      $("#pbleft").css('animation', "loading-lp "+ animationTime +"s linear "+ delay_left+"s forwards");
    }
  }
