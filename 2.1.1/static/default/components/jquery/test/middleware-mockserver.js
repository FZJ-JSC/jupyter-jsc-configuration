...
var mocks = {
	...
	wait: function (req, resp) {
		...
		setTimeout(function () {
			if (req.query.script) {
				...
				resp.end(
					'ERROR <script>QUnit.assert.ok( true, "mock executed" );</script>'
				);
			}
		}, wait);
	},
	...
	json: function (req, resp) {
		...
		if (req.query.array) {
			...
			resp.end(JSON.stringify({ data: { lang: "en", length: 25 } }));
		}
	},
	jsonp: function (req, resp, next) {
		...
		if (Array.isArray(req.query.callback)) {
			callback = Promise.resolve(
				req.query.callback[req.query.callback.length - 1]
			);
	...
	cspFrame: function (req, resp) {
		resp.writeHead(200, {
			...
			"Content-Security-Policy":
				"default-src 'self'; report-uri /base/test/data/mock.php?action=cspLog",
		});
	...
	cspNonce: function (req, resp) {
		...
		resp.writeHead(200, {
			"Content-Type": "text/html",
			"Content-Security-Policy":
				"script-src 'nonce-jquery+hardcoded+nonce'; report-uri /base/test/data/mock.php?action=cspLog",
		});
		var body = fs
			.readFileSync(__dirname + "/data/csp-nonce" + testParam + ".html")
			.toString();
		resp.end(body);
	},
