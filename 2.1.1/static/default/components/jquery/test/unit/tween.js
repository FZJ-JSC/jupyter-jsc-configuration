(function () {
	...
	QUnit.test(
		"jQuery.Tween - Default propHooks on plain objects",
		function (assert) {
			assert.expect(8);
			...
			assert.equal(
				defaultHook.get(fakeTween),
				undefined,
				"Can get missing property on object"
			);

			defaultHook.set(fakeTween);
			assert.equal(
				testObject.testMissing,
				10,
				"Sets missing value properly on plain object"
			);
			...
			assert.equal(
				testObject.opacity,
				10,
				"Correctly set opacity on plain object"
			);
			...
			defaultHook.set(fakeTween);
			assert.ok(
				stepSpy.calledWith(fakeTween),
				"Step function called with Tween"
			);
			assert.equal(
				testObject.test,
				0,
				"Because step didn't set, value is unchanged"
			);
		}
	);
	...
	QUnit.test("jQuery.Tween - Default propHooks on elements", function (assert) {
		...
		assert.ok(
			cssStub.calledWith(testElement, "height", ""),
			"Calls jQuery.css correctly"
		);
		...
		assert.equal(
			defaultHook.get(fakeTween),
			15,
			"Gets expected value not defined on style"
		);
		...
		assert.equal(
			defaultHook.get(fakeTween),
			10,
			"Can get missing property on element"
		);
		assert.ok(
			cssStub.calledWith(testElement, "testMissing", ""),
			"...using jQuery.css"
		);
		...
		defaultHook.set(fakeTween);
		assert.ok(
			styleStub.calledWith(testElement, "height", "10px"),
		...
		assert.equal(
			styleStub.callCount,
			0,
			"Did not call jQuery.style for non css property"
		);
		assert.equal(
			testElement.testMissing,
			10,
			"Instead, set value on element directly"
		);
		...
		assert.ok(
			styleStub.calledWith(testElement, "testMissing", "11px"),
	...
	QUnit.test("jQuery.Tween - Plain Object", function (assert) {
		...
		assert.ok(
			easingSpy.calledWith(
				0.1,
				0.1 * testOptions.duration,
				0,
				1,
	...
	QUnit.test(
		"jQuery.Tween - custom propHooks - advanced values",
		function (assert) {
			assert.expect(5);
			...
			assert.ok(
				propHook.get.calledWith(tween),
				"called propHook.get on create"
			);
