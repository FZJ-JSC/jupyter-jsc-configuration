...
QUnit.test("jQuery.param()", function (assert) {
	...
	assert.equal(
		...
		"handle nulls and undefineds properly"
	);
	...
	assert.equal(
		jQuery.param(params),
		"foo%5B%5D=baz&foo%5B%5D=42&foo%5B%5D=All%20your%20base%20are%20belong%20to%20us",
		"more array"
	);
	...
	assert.equal(
		decodeURIComponent(jQuery.param(params)),
	...
	assert.equal(
		jQuery.param(params, true),
		"a=0&a=1%2C2&a=3%2C4%2C5%2C6&a=%5Bobject%20Object%5D&a=17",
	...
	assert.equal(
		jQuery.param(params),
		"param1=",
		"Make sure that null params aren't traversed."
	);
	...
	assert.equal(
		jQuery.param(params),
		"test%5Blength%5D=3&test%5Bfoo%5D=bar",
	...
	assert.equal(
		jQuery.param(params),
		"test%5B%5D=1&test%5B%5D=2&test%5B%5D=",
	...
	assert.equal(
		jQuery.param(params),
		"",
		"jQuery.param( undefined ) === empty string"
	);
});

QUnit[jQuery.ajax ? "test" : "skip"](
	"jQuery.param() not affected by ajaxSettings",
	function (assert) {
		assert.expect(1);
...
QUnit.test("jQuery.param() Constructed prop values", function (assert) {
	...
	assert.equal(
		jQuery.param(params, false),
		"test=foo",
		"Do not mistake new String() for a plain object"
	);
	...
	assert.equal(
		jQuery.param(params, false),
		"test=5",
		"Do not mistake new Number() for a plain object"
	);
...
QUnit.test("serialize()", function (assert) {
	...
	assert.equal(
		jQuery("input,select,textarea,button", "#form").serialize(),
		"action=Test&radio2=on&check=on&hidden=&foo%5Bbar%5D=&name=name&search=search&email=dave%40jquery.com&number=43&select1=&select2=3&select3=1&select3=2&select5=3",
		"Check input serialization as query string"
	);

	assert.equal(
		jQuery("#testForm").serialize(),
		"T3=%3F%0D%0AZ&H1=x&H2=&PWD=&T1=&T2=YES&My%20Name=me&S1=abc&S3=YES&S4=",
		"Check form serialization as query string"
	);

	assert.equal(
		jQuery("input,select,textarea,button", "#testForm").serialize(),
		"T3=%3F%0D%0AZ&H1=x&H2=&PWD=&T1=&T2=YES&My%20Name=me&S1=abc&S3=YES&S4=",
		"Check input serialization as query string"
	);

	assert.equal(
		jQuery("#form, #testForm").serialize(),
	...
	assert.equal(
		jQuery(
			"#form, #testForm input, #testForm select, #testForm textarea, #testForm button"
		).serialize(),
		"action=Test&radio2=on&check=on&hidden=&foo%5Bbar%5D=&name=name&search=search&email=dave%40jquery.com&number=43&select1=&select2=3&select3=1&select3=2&select5=3&T3=%3F%0D%0AZ&H1=x&H2=&PWD=&T1=&T2=YES&My%20Name=me&S1=abc&S3=YES&S4=",
