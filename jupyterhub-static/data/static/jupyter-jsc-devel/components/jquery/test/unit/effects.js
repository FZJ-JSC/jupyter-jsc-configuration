(function () {
	...
	QUnit[jQuery.find.compile ? "test" : "skip"](
		...
		function (assert) {
			assert.expect(1);
			assert.equal(
				jQuery("#qunit-fixture:visible, #foo:visible").length,
				2,
	...
	QUnit.test("show()", function (assert) {
		...
		div.show().each(function () {
			assert.notEqual(
				this.style.display,
				"none",
				"don't change any <div> with display block"
			);
		});
	...
	supportjQuery.each(hideOptions, function (type, setup) {
		QUnit.test(
			"Persist correct display value - " + type + " hidden",
			function (assert) {
				assert.expect(3);

				jQuery(
					"<div id='show-tests'><span style='position:absolute;'>foo</span></div>"
				)
					.appendTo("#qunit-fixture")
					.find("*")
					.each(setup);
		...
		QUnit[
			...
			"Persist correct display value - " + type + " hidden, shadow child",
			function (assert) {
				assert.expect(3);
	...
	QUnit.test("animate block as inline width/height", function (assert) {
		...
		jQuery("#foo")
			.css({ display: "inline", width: "", height: "" })
				...
				},
				{
					...
					step: function () {
						if (jQuery(this).width() > 42) {
							assert.ok(
								false,
								"width was incorrectly augmented during animation"
							);
						}
					},
					complete: function () {
	...
	QUnit.test("animate table-row width/height", function (assert) {
		...
		var tr = jQuery("#table")
			...
			.html(
				"<tr style='height:42px;'><td style='padding:0;'><div style='width:20px;height:20px;'></div></td></tr>"
			)
			.find("tr");
		...
		tr.animate({ width: 10, height: 10 }, 100, function () {
			...
			assert.equal(
				this.offsetHeight,
				20,
				"height animated to shrink wrap point"
			);
		});
		this.clock.tick(100);
	});
	...
	QUnit.test("animate table-cell width/height", function (assert) {
		...
		var td = jQuery("#table")
			.attr({ cellspacing: 0, cellpadding: 0, border: 0 })
			.html(
				"<tr><td style='width:42px;height:42px;padding:0;'><div style='width:20px;height:20px;'></div></td></tr>"
			)
			.find("td");
	...
	QUnit.test("animate percentage(%) on width/height", function (assert) {
		...
		var $div = jQuery(
			"<div style='position:absolute;top:-999px;left:-999px;width:60px;height:60px;'><div style='width:50%;height:50%;'></div></div>"
		)
			.appendTo("#qunit-fixture")
			.children("div");
		...
		$div.animate({ width: "25%", height: "25%" }, 13, function () {
			...
			assert.equal(
				$this.css("width"),
				"15px",
				"Width was animated to 15px rather than 25px"
			);
			assert.equal(
				$this.css("height"),
				"15px",
				"Height was animated to 15px rather than 25px"
			);
		});
		this.clock.tick(20);
	});
	...
	QUnit.test(
		"animate resets overflow-x and overflow-y when finished",
		function (assert) {
			assert.expect(2);
			jQuery("#foo")
				.css({
...
*/
			...
			.animate(
				{ width: origWidth + 100 },
				{
					...
					complete: function () {
						...
						order.push(2);
						assert.equal(
							parseFloat(foo.css("width")),
							origWidth + 100,
							"Animation ended"
						);
						assert.equal(
							foo.queue("name").length,
							1,
							"Queue length of 'name' queue"
						);
					},
				}
			)
	...
	QUnit.test("animate duration 0", function (assert) {
		...
		assert.equal(
			jQuery.timers.length,
			0,
			"Make sure no animation was running from another test"
		);
		...
		assert.equal(
			jQuery.timers.length,
			0,
			"Make sure synchronic animations are not left on jQuery.timers"
		);
		...
		jQuery("#foo")
			...
			.animate({ "font-size": 20 }, 200, function () {
				assert.equal(
					this.style.fontSize,
					"20px",
					"The font-size property was animated."
				);
			});
	...
	QUnit.test(
		'animate non-element\'s zIndex without appending "px"',
		function (assert) {
			assert.expect(1);
			...
			jQuery(obj).animate({ zIndex: 200 }, 200, function () {
				assert.equal(
					obj.zIndex,
					200,
					'The custom property should be modified without appending "px".'
				);
			});
			this.clock.tick(200);
		}
	...
	QUnit.test("stop()", function (assert) {
		...
		$foo.hide().css("width", 200).animate({ width: "show" }, 1500);
		...
		assert.notEqual(
			parseFloat(nw),
			w,
			"Stop didn't reset the animation " + nw + " " + w + "px"
		);
		...
		$two.fadeTo(100, 0, function () {
			assert.equal(
				$two.css("opacity"),
				"0",
				"Stop does not interfere with animations on other elements (#6641)"
			);
	...
	QUnit.test("stop( queue, ..., ... ) - Stop single queues", function (assert) {
			...
			},
			{
				...
				complete: function () {
					assert.equal(
						parseFloat(foo.css("width")),
						400,
						"Animation completed for standard queue"
					);
					assert.equal(
						parseFloat(foo.css("height")),
						saved,
		...
		foo
			...
			.dequeue("height")
			.stop("height", false, true);

		assert.equal(
			parseFloat(foo.css("height")),
			400,
			"Height was stopped with gotoEnd"
		);
	...
	jQuery.each(
		{
			...
			"JS Auto": function (elem, prop) {
				jQuery(elem).css(prop, "").text("This is a long string of text.");
				return "";
			},
			...
			"JS 50": function (elem, prop) {
				jQuery(elem).css(prop, prop === "opacity" ? 0.5 : "50px");
					...
					QUnit.test(fn + " to " + tn, function (assert) {
						...
						jQuery.when(elem).done(function ($elem) {
							...
							if (t_w === "hide" || t_w === "show") {
								assert.ok(
									f_w === ""
										? elem.style.width === f_w
										: elem.style.width.indexOf(f_w) === 0,
							...
							if (t_h === "hide" || t_h === "show") {
								assert.ok(
									f_h === ""
										? elem.style.height === f_h
										: elem.style.height.indexOf(f_h) === 0,
							...
							if (t_o === "hide" || t_o === "show") {
								assert.equal(
									cur_o,
									f_o,
									"Opacity must be reset to " + f_o + ": " + cur_o
								);
							}
							...
							if (t_w === "hide") {
								assert.equal(
									elem.style.display,
									"none",
									"Hiding, display should be none: " + elem.style.display
								);
							}
							...
							if (t_o.constructor === Number) {
								assert.equal(
									cur_o,
									t_o,
									"Final opacity should be " + t_o + ": " + cur_o
								);

								assert.ok(
									jQuery.css(elem, "opacity") !== "" || cur_o === t_o,
									"Opacity should be explicitly set to " +
										t_o +
										", is instead: " +
										cur_o
								);
							...
							if (t_w.constructor === Number) {
								assert.equal(
									elem.style.width,
									t_w + "px",
									"Final width should be " + t_w + ": " + elem.style.width
								);

								cur_w = jQuery.css(elem, "width");

								assert.ok(
									elem.style.width !== "" || cur_w === t_w,
									"Width should be explicitly set to " +
										t_w +
										", is instead: " +
										cur_w
								);
							...
							if (t_h.constructor === Number) {
								...
								assert.ok(
									elem.style.height !== "" || cur_h === t_h,
									"Height should be explicitly set to " +
										t_h +
										", is instead: " +
										cur_h
								);
	...
	QUnit.test("Effects chaining", function (assert) {
		var remaining = 16,
			...
			getProps = function (el) {
				...
				jQuery.each(props, function (i, prop) {
					obj[prop] =
						(prop === "overflow" && el.style[prop]) || jQuery.css(el, prop);
				});
				return obj;
			};
		...
		setup(".fadeOut().fadeIn()", "#fadein div")
			.fadeOut("fast")
			.fadeIn("fast", check);
		setup(".fadeIn().fadeOut()", "#fadeout div")
			.fadeIn("fast")
			.fadeOut("fast", check);
		setup(".hide().show()", "#show div").hide("fast").show("fast", check);
		setup(".show().hide()", "#hide div").show("fast").hide("fast", check);
		setup(".show().hide(easing)", "#easehide div")
			.show("fast")
			.hide("fast", "linear", check);
		setup(".toggle().toggle() - in", "#togglein div")
			.toggle("fast")
			.toggle("fast", check);
		setup(".toggle().toggle() - out", "#toggleout div")
			.toggle("fast")
			.toggle("fast", check);
		setup(".toggle().toggle(easing) - out", "#easetoggleout div")
			.toggle("fast")
			.toggle("fast", "linear", check);
		setup(".slideDown().slideUp()", "#slidedown div")
			.slideDown("fast")
			.slideUp("fast", check);
		setup(".slideUp().slideDown()", "#slideup div")
			.slideUp("fast")
			.slideDown("fast", check);
		setup(".slideUp().slideDown(easing)", "#easeslideup div")
			.slideUp("fast")
			.slideDown("fast", "linear", check);
		setup(".slideToggle().slideToggle() - in", "#slidetogglein div")
			.slideToggle("fast")
			.slideToggle("fast", check);
		setup(".slideToggle().slideToggle() - out", "#slidetoggleout div")
			.slideToggle("fast")
			.slideToggle("fast", check);
		setup(".fadeToggle().fadeToggle() - in", "#fadetogglein div")
			.fadeToggle("fast")
			.fadeToggle("fast", check);
		setup(".fadeToggle().fadeToggle() - out", "#fadetoggleout div")
			.fadeToggle("fast")
			.fadeToggle("fast", check);
		setup(".fadeTo(0.5).fadeTo(1.0, easing)", "#fadeto div")
			.fadeTo("fast", 0.5)
			.fadeTo("fast", 1.0, "linear", check);
	...
	jQuery.makeTest = function (text) {
		...
		jQuery("<h4></h4>").text(text).appendTo("#fx-tests").after(elem);
	...
	QUnit.test(
		"jQuery.show('fast') doesn't clear radio buttons (bug #1095)",
		function (assert) {
			assert.expect(4);
			...
			$checkedtest.hide().show("fast", function () {
				...
				assert.ok(
					jQuery("input[type='checkbox']", $checkedtest)
						.first()
						.attr("checked"),
	...
	QUnit.test("interrupt toggle", function (assert) {
		...
		jQuery.each(
			{ slideToggle: "height", fadeToggle: "opacity", toggle: "width" },
			function (method, prop) {
				var $methodElems = $elems
					.filter("[id^='" + method.toLowerCase() + "']")
					.each(function () {
				...
				setTimeout(function () {
					$methodElems.stop().each(function () {
						assert.notEqual(
							jQuery(this).css(prop),
							jQuery.data(this, "startVal"),
							".stop() before completion of hiding ." +
								method +
								"() - #" +
								this.id
						);
					...
					$methodElems[method](shortDuration, function () {
						...
						setTimeout(function () {
							...
							assert.notEqual(
								$elem.css(prop),
								startVal,
								".stop() before completion of showing ." +
									method +
									"() - #" +
									id
							);
	...
	QUnit.test("animate with per-property easing", function (assert) {
		...
		jQuery(data).animate(props, 400, "_defaultTest", function () {
			...
			assert.equal(
				props.a[1],
				"_test1",
				"animate does not change original props (per-property easing would be lost)"
			);
			assert.equal(
				props.b[1],
				"_test2",
		...
		jQuery("#foo")
			.animate(propsBasic, 200, "animationScope", function () {
				...
				assert.equal(
					...
					"20px",
					"padding-left was animated"
				);
				assert.equal(
					this.style.paddingRight,
					"20px",
					"padding-right was animated"
				);
				assert.equal(
					this.style.paddingBottom,
					"30px",
					"padding-bottom was animated"
				);
				assert.equal(
					easeAnimation_count,
					4,
					"per-animation default easing called for each property"
				);
			...
			.animate(propsSpecial, 200, "animationScope", function () {
				assert.equal(
					...
					"1px",
					"padding-top was animated again"
				);
				assert.equal(
					this.style.paddingLeft,
					"2px",
					"padding-left was animated again"
				);
				assert.equal(
					this.style.paddingRight,
					"2px",
					"padding-right was animated again"
				);
				assert.equal(
					this.style.paddingBottom,
					"3px",
					"padding-bottom was animated again"
				);
				assert.equal(
					easeAnimation_count,
					0,
					"per-animation default easing not called"
				);
				assert.equal(
					easeProperty_count,
					4,
					"special easing called for each property"
				);
	...
	QUnit.test(
		"hide hidden elements, with animation (bug #7141)",
		function (assert) {
			assert.expect(4);

			var div = jQuery(
				"<div id='bug7141' style='display:none'></div>"
			).appendTo("#qunit-fixture");
			...
			div.hide(10, function () {
				assert.equal(
					div.css("display"),
					"none",
					"Element is hidden in .hide() callback"
				);
				div.show(11, function () {
					assert.equal(
						div.css("display"),
						"block",
						"Element is visible in .show() callback"
					);
				});
			});
			this.clock.tick(50);
			assert.equal(
				div.css("display"),
				"block",
				"Element is visible after animations"
			);
		}
	);
	...
	QUnit.test("animate unit-less properties (#4966)", function (assert) {
		...
		var div = jQuery(
			"<div style='z-index: 0; position: absolute;'></div>"
		).appendTo("#qunit-fixture");
	...
	QUnit.test(
		"animate properties missing px w/ opacity as last (#9074)",
		function (assert) {
			assert.expect(6);
			...
			var ml,
				l,
				div = jQuery(
					"<div style='position: absolute; margin-left: 0; left: 0px;'></div>"
				).appendTo("#qunit-fixture");
	...
	QUnit.test(
		"callbacks that throw exceptions will be removed (#5684)",
		function (assert) {
			assert.expect(2);
	...
	QUnit.test(
		"animate will scale margin properties individually",
		function (assert) {
			assert.expect(2);
			...
			assert.ok(
				foo.css("marginLeft") !== foo.css("marginRight"),
				"Sanity Check"
			);
			...
			assert.ok(
				foo.css("marginLeft") !== foo.css("marginRight"),
				"The margin properties are different"
			);
	...
	QUnit.test("Do not append px to 'fill-opacity' #9548", function (assert) {
		...
		$div
			.css("fill-opacity", 0)
			.animate({ "fill-opacity": 1.0 }, 0, function () {
				assert.equal(
					jQuery(this).css("fill-opacity"),
					1,
	...
	QUnit.test(
		"Animate Option: step: function( percent, tween )",
		function (assert) {
			assert.expect(1);
			...
			.animate(
				...
				function () {
					assert.equal(
						foo[0],
						this,
						"Complete callback after stop(true) `this` is element"
					);
				}
			)
			.stop(true, true);
	...
	QUnit.test(
		"User supplied callback called after show when fx off (#8892)",
		function (assert) {
			assert.expect(2);
			...
			foo.fadeIn(500, function () {
				assert.ok(
					supportjQuery(this).is(":visible"),
					"Element is visible in callback"
				);
				foo.fadeOut(500, function () {
					assert.ok(
						supportjQuery(this).is(":hidden"),
						"Element is hidden in callback"
					);
					jQuery.fx.off = false;
				});
			});
	...
	QUnit.test(
		"animate should set display for disconnected nodes",
		function (assert) {
			assert.expect(20);
			...
			assert.strictEqual(
				$divEmpty[0].parentNode,
				null,
				"Setup: element with null parentNode"
			);
			assert.strictEqual(
				($divTest[0].parentNode || {}).nodeType,
				11,
				"Setup: element under fragment"
			);

			assert.strictEqual(
				$divEmpty.show()[0].style.display,
				"",
				"set display with show() for element with null parentNode"
			);
			assert.strictEqual(
				$divTest.show()[0].style.display,
				"",
				"set display with show() for element under fragment"
			);
			assert.strictEqual(
				$divNone.show()[0].style.display,
				"",
				"show() should change display if it already set to none"
			);
			assert.strictEqual(
				$divInline.show()[0].style.display,
				"inline",
	...
	QUnit[jQuery.find.compile ? "test" : "skip"](
		"Animation callback should not show animated element as :animated (#7157)",
		function (assert) {
			assert.expect(1);
	...
	QUnit[jQuery.find.compile ? "test" : "skip"](
		"Initial step callback should show element as :animated (#14623)",
		function (assert) {
			assert.expect(1);
			...
			foo.animate(
				...
				},
				{
					...
					step: function () {
						assert.ok(
							foo.is(":animated"),
							"The element matches :animated inside step function"
						);
					},
				}
			);
	...
	QUnit.test(
		"hide called on element within hidden parent should set display to none (#10045)",
		function (assert) {
			assert.expect(3);
	...
	QUnit.test(
		"hide, fadeOut and slideUp called on element width height and width = 0 should set display to none",
		function (assert) {
			assert.expect(5);
	...
	QUnit.test(
		"hide should not leave hidden inline elements visible (#14848)",
		function (assert) {
			assert.expect(2);
	...
	QUnit.test(
		"animate does not change start value for non-px animation (#7109)",
		function (assert) {
			assert.expect(1);

			var parent = jQuery("<div><div></div></div>")
					.css({ width: 284, height: 1 })
					.appendTo("#qunit-fixture"),
				child = parent
					.children()
					.css({ fontSize: "98.6in", width: "0.01em", height: 1 }),
			...
			child
				...
				.queue(function (next) {
					...
					assert.ok(
						ratio > 0.9 && ratio < 1.1,
						"Starting width was close enough (" +
							computed[0] +
							" approximates " +
							actual +
							")"
	...
	QUnit.test(
		"non-px animation handles non-numeric start (#11971)",
		function (assert) {
			assert.expect(2);
				...
				{ backgroundPositionX: "42%" },
				{
					...
					progress: function (anim, percent) {
						...
						if (parseFloat(initial)) {
							assert.equal(
								jQuery.style(this, "backgroundPositionX"),
								initial,
					...
					progress: function (anim, p, ms) {
						if (!step++) {
							...
							assert.strictEqual(
								p,
								0,
								"first progress callback: progress ratio"
							);
							assert.strictEqual(
								ms,
								dur,
							...
							assert.strictEqual(
								p,
								1,
								"last progress callback: progress ratio"
							);
	...
	QUnit.test(
		"Animate properly sets overflow hidden when animating width/height (#12117)",
		function (assert) {
			assert.expect(8);
	...
	QUnit.test(
		"Each tick of the timer loop uses a fresh time (#12837)",
		function (assert) {
			var lastVal,
				tmp = jQuery({
					test: 0,
			...
			tmp.animate(
				...
				},
				{
					step: function (p, fx) {
						assert.ok(
							fx.now !== lastVal,
							"Current value is not the last value: " + lastVal + " - " + fx.now
						);
						lastVal = fx.now;
					},
				}
	...
	QUnit.test(
		"Animations with 0 duration don't ease (#12273)",
		function (assert) {
			assert.expect(1);
					...
					step: function (p, fx) {
						if (fx.pos > 0.1 && fx.prop === check && !tested) {
							...
							assert.equal(
								fx.start,
								stopped,
								check + " starts at " + stopped + " where it stopped"
							);
							assert.equal(
								fx.end,
								original,
								check + " ending value is " + original
							);
							element.stop();
						}
					},
	...
	QUnit.test(".finish() completes all queued animations", function (assert) {
		...
		jQuery.each(animations, function (prop, value) {
			assert.equal(
				parseFloat(div.css(prop)),
				value,
				prop + " finished at correct value"
			);
		});
		...
		if (jQuery.find.compile) {
			...
			assert.ok(
				"skip",
				":animated selector not supported with selector-native"
			);
		}
	...
	QUnit.test(
		'.finish( "custom" ) - custom queue animations',
		function (assert) {
			var animations = {
					top: 100,
					left: 100,
	...
	QUnit.test(
		".finish() calls finish of custom queue functions",
		function (assert) {
	...
	QUnit.test(
		".finish() is applied correctly when multiple elements were animated (#13937)",
		function (assert) {
			assert.expect(3);
	...
	QUnit.test("slideDown() after stop() (#13483)", function (assert) {
		...
		var ul = jQuery(
				"<ul style='height: 100px; display: block;'></ul>"
			).appendTo("#qunit-fixture"),
	...
	QUnit.test(
		"Respect display value on inline elements (#14824)",
		function (assert) {
			assert.expect(2);
	...
	QUnit.test(
		"Display value is correct for disconnected nodes (trac-13310)",
		function (assert) {
			assert.expect(3);
	...
	QUnit.test("Show/hide/toggle and display: inline", function (assert) {
		...
		jQuery(
			"<span></span><div style='display:inline' title='inline div'></div>"
		).each(function () {
			var completed,
				interrupted,
				N = 100,
			...
			completed = jQuery.map(
				[
					$el
						.clone()
						.data({ call: "hide", done: "none" })
						.appendTo(fixture)
						.hide(N),
					$el
						.clone()
						.data({ call: "toggle", done: "none" })
						.appendTo(fixture)
						.toggle(N),
					$el
						.clone()
						.data({ call: "hide+show", done: "inline" })
						.appendTo(fixture)
						.hide()
						.show(N),
					$el
						.clone()
						.data({ call: "hide+toggle", done: "inline" })
						.appendTo(fixture)
						.hide()
						.toggle(N),
				],
			...
			interrupted = jQuery.map(
				[
					...
					$el
						.clone()
						.data({ call: "hide+show+stop" })
						.appendTo(fixture)
						.hide()
						.show(N),
					$el
						.clone()
						.data({ call: "hide+toggle+stop" })
						.appendTo(fixture)
						.hide()
						.toggle(N),
				],
	...
	QUnit.test(
		"jQuery.speed( speed, easing, complete ) - with easing function",
		function (assert) {
			testEasing(
				assert,
				10,
	...
	QUnit.test(
		"jQuery.speed( options ) - with easing function",
		function (assert) {
			testEasing(assert, {
				duration: 10,
				easing: function () {
	...
	QUnit.test("jQuery.speed() - durations", function (assert) {
		...
		assert.equal(
			get(100),
			0,
			"jQuery.speed defaults duration to zero if fx is off"
		);
		jQuery.fx.off = false;
	});
})();
