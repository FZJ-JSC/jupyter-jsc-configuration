...
module.exports = function (Release, files, complete) {
	...
	const extras = ["src", "LICENSE.txt", "AUTHORS.txt", "package.json"];
	...
	function clone() {
		...
		Release.exec(
			`git clone ${distRemote} ${Release.dir.dist}`,
			"Error cloning repo."
		);
	...
	async function copy() {
		...
		const readme = await fs.readFile(
			`${Release.dir.repo}/build/fixtures/README.md`,
			"utf8"
		);
		const rmIgnore = [...files, "node_modules"].map(
			(file) => `${Release.dir.dist}/${file}`
		);
		...
		const { blogPostLink } = await inquirer.prompt([
			{
				...
				name: "blogPostLink",
				message:
					"Enter URL of the blog post announcing the jQuery release...\n",
			},
		]);
