...
module.exports = function (grunt) {
	...
	function convert(name, path, contents) {
		...
		if (/.\/var\//.test(path.replace(process.cwd(), ""))) {
			...
			contents = contents
				.replace(
					/\/\*\s*ExcludeStart\s*\*\/[\w\W]*?\/\*\s*ExcludeEnd\s*\*\//gi,
					""
				)
		...
		// AMD Name
		if (
			(amdName = grunt.option("amd")) != null &&
			/^exports\/amd$/.test(name)
		) {
			if (amdName) {
	...
	grunt.registerMultiTask(
		...
		function () {
			...
			if (!optIn) {
				...
				config.rawText.jquery =
					"define([" + (included.length ? included.join(",") : "") + "]);";
			}
		...
		var args = this.args,
			modules = args.length
				? args[0]
						...
						.reduce(
							(acc, elem) =>
								acc.concat(elem === "slim" ? ["-ajax", "-effects"] : [elem]),
							[]
						)
