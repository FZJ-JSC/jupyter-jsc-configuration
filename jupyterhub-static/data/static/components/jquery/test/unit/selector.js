...
QUnit.test("id", function (assert) {
	...
	assert.t("ID selector with existing ID descendant", "#firstp #simon1", [
		"simon1",
	]);
	assert.t("ID selector with non-existent descendant", "#firstp #foobar", []);
	assert.t("ID selector using UTF8", "#台北Táiběi", ["台北Táiběi"]);
	assert.t("Multiple ID selectors using UTF8", "#台北Táiběi, #台北", [
		"台北Táiběi",
		"台北",
	]);
	...
	assert.t("Descendant escaped ID", "div #test\\.foo\\[5\\]bar", [
		"test.foo[5]bar",
	]);
	assert.t("Child escaped ID", "form > #foo\\:bar", ["foo:bar"]);
	assert.t("Child escaped ID", "form > #test\\.foo\\[5\\]bar", [
		"test.foo[5]bar",
	]);
	...
	assert.t("ID Selector contains backslash", "#backslash\\\\foo", [
		"backslash\\foo",
	]);
...
QUnit.test("class", function (assert) {
	...
	assert.deepEqual(
		jQuery(".blog", document.getElementsByTagName("p")).get(),
		q("mark", "simon"),
	...
	assert.deepEqual(
		jQuery(".blog", jQuery("p")).get(),
		q("mark", "simon"),
		"Finding elements with a context."
	);
	assert.deepEqual(
		jQuery("p").find(".blog").get(),
		q("mark", "simon"),
		"Finding elements with a context."
	);
});
...
QUnit.test("name", function (assert) {
	...
	assert.t("Name selector with single quotes", "input[name='action']", [
		"text1",
	]);
	assert.t("Name selector with double quotes", 'input[name="action"]', [
		"text1",
	]);

	assert.t("Name selector for grouped input", "input[name='types[]']", [
		"types_all",
		"types_anime",
		"types_movie",
	]);
...
QUnit.test("selectors with comma", function (assert) {
	...
	var fixture = jQuery(
		"<div><h2><span></span></h2><div><p><span></span></p><p></p></div></div>"
	);

	assert.equal(
		fixture.find("h2, div p").filter("p").length,
		2,
		"has to find two <p>"
	);
	assert.equal(
		fixture.find("h2, div p").filter("h2").length,
		1,
		"has to find one <h2>"
	);
	assert.equal(
...
QUnit.test("child and adjacent", function (assert) {
	...
	assert.selectInFixture("All Grandchildren", "p > * > *", [
		"anchor1",
		"anchor2",
	]);
	...
	assert.t("Element Preceded By", "#siblingfirst ~ em", [
		"siblingnext",
		"siblingthird",
	]);
	assert.t(
	...
	if (jQuery.find.compile) {
		...
		assert.equal(
			jQuery("#listWithTabIndex li:eq(2) ~ li").length,
			1,
	...
	assert.t("Multiple combinators selects all levels", "#siblingTest em *", [
		"siblingchild",
		"siblinggrandchild",
	...
	assert.equal(
		jQuery("#listWithTabIndex").length,
		1,
		"Parent div for next test is found via ID (#8310)"
	);
	assert.equal(
		jQuery("#__sizzle__").length,
		0,
...
QUnit.test("attributes", function (assert) {
	...
	assert.t("Find elements with a tabindex attribute", "[tabindex]", [
		"listWithTabIndex",
	...
	assert.t("Attribute Exists (case-insensitive)", "#qunit-fixture a[TITLE]", [
		"google",
	]);
	...
	assert.t("Boolean attribute exists", "#select2 option[selected]", [
		"option2d",
	]);
	assert.t("Boolean attribute equals", "#select2 option[selected='selected']", [
		"option2d",
	]);
	...
	assert.t("Attribute Equals", "#qunit-fixture a[ rel = 'bookmark' ]", [
		"simon1",
	]);
	assert.t("Attribute Equals Number", "#qunit-fixture option[value='1']", [
		"option1b",
		"option2b",
		"option3b",
		"option4b",
	...
	assert.t("Attribute Equals Number", "#qunit-fixture li[tabIndex='-1']", [
		"foodWithNegativeTabIndex",
	]);
	...
	assert.t("Attribute Begins With", "a[href ^= 'http://www']", [
		"google",
		"yahoo",
	]);
	...
	if (jQuery.find.compile) {
		assert.t("Empty values", "#select1 option[value!='']", [
			"option1b",
			"option1c",
			"option1d",
		]);
		assert.t("Attribute Is Not Equal", "#ap a[hreflang!='en']", [
			"google",
			"groups",
			"anchor1",
		]);
		assert.t("Select options via :selected", "#select1 option:selected", [
			"option1a",
		]);
		assert.t("Select options via :selected", "#select2 option:selected", [
			"option2d",
		]);
		assert.t("Select options via :selected", "#select3 option:selected", [
			"option3b",
			"option3c",
		]);
		assert.t(
	...
	assert.t("Underscores don't need escaping", "input[id=types_all]", [
		"types_all",
	]);

	assert.t("input[type=text]", "#form input[type=text]", [
		"text1",
		"text2",
		"hidden2",
		"name",
	...
	assert.ok(
		withScript.find("#moretests script[src]").has("script"),
	...
	assert.t('Object.prototype property "constructor"', "[constructor='foo']", [
		"foo",
	]);
	assert.t('Gecko Object.prototype property "watch"', "[watch='bar']", ["foo"]);

	assert.t("Value attribute is retrieved correctly", "input[value=Test]", [
		"text1",
		"text2",
	]);
	...
	if (jQuery.find.compile) {
		...
		assert.ok(
			jQuery(
				"<select value='12600'><option value='option' selected='selected'></option><option value=''></option></select>"
			)
				.prop("value", "option")
		...
		assert.ok(
			jQuery("<input type='text' value='12600'/>")
				.prop("value", "option")
				.is(":input[value='12600']"),
...
QUnit.test("disconnected nodes", function (assert) {
	...
	assert.equal(
		$div.is("div"),
		true,
		"Make sure .is('nodeName') works on disconnected nodes."
	);
});
...
testIframe(
	...
	function (assert, jQuery, window, document) {
		...
		jQuery.expandedEach(
			["draggable", "contenteditable", "aria-disabled"],
			function (i, val) {
		...
		t(
			"tabindex selector does not retrieve all elements in IE6/7 (#8473)",
			"form, [tabindex]",
			["form1", "text1"]
		);
		t(
			"Improperly named form elements do not interfere with form selections (#9570)",
			"form[name='formName']",
			["form1"]
		);
	}
...
QUnit.test("jQuery.contains", function (assert) {
	...
	assert.ok(
		!jQuery.contains(container, document.documentElement),
		"documentElement (negative)"
	);
	assert.ok(
		!jQuery.contains(container, null),
		"Passing null does not throw an error"
	);
	assert.ok(
		jQuery.contains(document, document.documentElement),
		"documentElement (positive)"
	);
	assert.ok(
		jQuery.contains(document, element),
		"document container (positive)"
	);
	assert.ok(
		!jQuery.contains(document, detached),
		"document container (negative)"
	);
});
...
QUnit.test("jQuery.uniqueSort", function (assert) {
	...
	for (i = 0; i < 12; i++) {
		...
		detached2.appendChild(document.createElement("li")).id =
			"detachedChild" + i;
	}
	...
	jQuery.each(tests, function (label, test) {
		...
		assert.deepEqual(
			jQuery.uniqueSort(test.input).slice(0, length),
			test.expected,
			label + " (array)"
		);
		assert.deepEqual(
			jQuery.uniqueSort(new Arrayish(test.input)).slice(0, length),
			test.expected,
			label + " (quasi-array)"
		);
	});
...
testIframe(
	...
	function (assert, jQuery, window, document) {
		...
		assert.deepEqual(
			$cached(".test a").get(),
			[document.getElementById("collision")],
			"Select collision anchor with first sizzle"
		);
		assert.equal(
			jQuery(".evil a").length,
			0,
			"Select nothing with second sizzle"
		);
		assert.equal(
			jQuery(".evil a").length,
			0,
			"Select nothing again with second sizzle"
		);
	}
);

QUnit.test(
	"Iframe dispatch should not affect jQuery (#13936)",
	function (assert) {
		assert.expect(1);
		var loaded = false,
			thrown = false,
		...
		jQuery(iframe).on("load", function () {
			...
			if (loaded) {
				assert.strictEqual(
					thrown,
					false,
					"No error thrown from post-reload jQuery call"
				);
...
QUnit.test(
	"Ensure escapeSelector exists (escape tests in Sizzle)",
	function (assert) {
		assert.expect(1);

		assert.equal(
			jQuery.escapeSelector("#foo.bar"),
			"\\#foo\\.bar",
			"escapeSelector present"
		);
	}
);
