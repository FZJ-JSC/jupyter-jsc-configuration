if (jQuery.css) {
	...
	QUnit.test("css(String|Hash)", function (assert) {
		...
		assert.equal(
			jQuery("#qunit-fixture").css("display"),
			"block",
		...
		assert.equal(
			div2.find("input").css("height"),
			"20px",
			"Height on hidden input."
		);
		assert.equal(
			div2.find("textarea").css("height"),
			"20px",
			"Height on hidden textarea."
		);
		assert.equal(
			div2.find("div").css("height"),
			"20px",
			"Height on hidden div."
		);
		...
		assert.equal(
			...
			0,
			"Test negative width set to 0"
		);
		assert.equal(
			parseFloat(jQuery("#nothiddendiv").css("height")),
			0,
			"Test negative height set to 0"
		);

		assert.equal(
			jQuery("<div style='display: none;'></div>").css("display"),
			"none",
		...
		assert.equal(
			jQuery("#empty").css("opacity"),
			"0",
			"Assert opacity is accessible"
		);
		jQuery("#empty").css({ opacity: "1" });
		assert.equal(
			jQuery("#empty").css("opacity"),
			"1",
			"Assert opacity is taken from style attribute when set"
		);
		...
		assert.equal(
			parseInt(div.css("fontSize"), 10),
			16,
			"Verify fontSize px set."
		);
		assert.equal(
			parseInt(div.css("font-size"), 10),
			16,
			"Verify fontSize px set."
		);
		assert.equal(
			parseInt(child.css("fontSize"), 10),
			16,
			"Verify fontSize px set."
		);
		assert.equal(
			parseInt(child.css("font-size"), 10),
			16,
			"Verify fontSize px set."
		);
		...
		assert.equal(
			...
			"100%",
			"Make sure the height is being set correctly."
		);
		...
		assert.strictEqual(
			child.css("x-fake"),
			undefined,
			"Make sure undefined is returned from css(nonexistent)."
		);
	...
	QUnit.test("css() explicit and relative values", function (assert) {
		...
		assert.equal(
			$elem.css("width"),
			"1px",
			"Initial css set or width/height works (hash)"
		);
		assert.equal(
			$elem.css("paddingLeft"),
			"1px",
			"Initial css set of paddingLeft works (hash)"
		);
		assert.equal(
			$elem.css("opacity"),
			"1",
			"Initial css set of opacity works (hash)"
		);
	...
	QUnit.test(
		"css() mismatched relative values with bounded styles (gh-2144)",
		function (assert) {
			assert.expect(1);
			...
			$el.css("right", "-=25em");
			assert.equal(
				Math.round(parseFloat($el.css("right"))),
				100,
	...
	QUnit.test("css(String, Object)", function (assert) {
		...
		assert.equal(
			j.css("overflow"),
			"visible",
			"Check node,textnode,comment css works"
		);
		assert.equal(
			jQuery("#t2037 .hidden").css("display"),
			"none",
			"Make sure browser thinks it is hidden"
		);
	...
	QUnit.test("css(String, Object) with negative values", function (assert) {
		...
		assert.equal(
			jQuery("#nothiddendiv").css("margin-top"),
			"-10px",
			"Ensure negative top margins work."
		);
		assert.equal(
			jQuery("#nothiddendiv").css("margin-left"),
			"-10px",
			"Ensure negative left margins work."
		);
		...
		assert.equal(
			jQuery("#nothiddendiv").css("top"),
			"-20px",
			"Ensure negative top values work."
		);
		assert.equal(
			jQuery("#nothiddendiv").css("left"),
			"-20px",
			"Ensure negative left values work."
		);
	});
	...
	QUnit.test("css(Array)", function (assert) {
		...
		assert.deepEqual(
			elem.css(expectedMany).css(["overflow", "width"]),
			expectedMany,
			"Getting multiple element array"
		);
		assert.deepEqual(
			elem.css(expectedSingle).css(["width"]),
			expectedSingle,
	...
	QUnit.test("css(String, Function)", function (assert) {
		...
		jQuery(
			"<div id='cssFunctionTest'><div class='cssFunction'></div>" +
		...
		jQuery("#cssFunctionTest div").each(function () {
			...
			assert.equal(
				computedSize,
				expectedSize,
				"Div #" + index + " should be " + expectedSize
			);
			index++;
		});
	...
	QUnit.test(
		"css(Object) where values are Functions with incoming values",
		function (assert) {
			assert.expect(3);
	...
	if (jQuery.fn.show && jQuery.fn.hide) {
		QUnit.test("show()", function (assert) {
			...
			assert.equal(
				jQuery.css(hiddendiv[0], "display"),
				"none",
		...
		QUnit.test("show/hide detached nodes", function (assert) {
			...
			assert.equal(
				div.css("display"),
				"none",
				"hide() updates inline style of a detached div"
			);
			...
			div = jQuery("<div><div style='display: none'></div></div>").children(
				"div"
			);
			div.show().appendTo("#qunit-fixture");
			assert.equal(
		...
		QUnit[
			document.body.attachShadow && document.body.getRootNode ? "test" : "skip"
		...
		QUnit.test("hide hidden elements (bug #7141)", function (assert) {
			...
			var div = jQuery("<div style='display:none'></div>").appendTo(
				"#qunit-fixture"
			);
			assert.equal(div.css("display"), "none", "Element is hidden by default");
			div.hide();
			assert.ok(
				!jQuery._data(div, "olddisplay"),
				"olddisplay is undefined after hiding an already-hidden element"
			);
			div.show();
		...
		QUnit.test(
			"show() after hide() should always set display to initial value (#14750)",
			function (assert) {
				assert.expect(1);
				...
				assert.equal(
					div.css("display"),
					"list-item",
					"should get last set display value"
				);
			}
		);
		...
		QUnit.test("show/hide 3.0, default display", function (assert) {
			...
			var i,
				$elems = jQuery("<div></div>")
					.appendTo("#qunit-fixture")
					.html(
						"<div data-expected-display='block'></div>" +
		...
		QUnit.test("show/hide 3.0, cascade display", function (assert) {
			...
			var i,
				$elems = jQuery("<div></div>")
					.appendTo("#qunit-fixture")
					.html(
						"<span class='block'></span><div class='inline'></div><div class='list-item'></div>"
					)
					.children();
		...
		QUnit.test("show/hide 3.0, inline display", function (assert) {
			...
			var i,
				$elems = jQuery("<div></div>")
					.appendTo("#qunit-fixture")
					.html(
						"<span data-expected-display='block' style='display:block'></span>" +
		...
		QUnit.test("show/hide 3.0, cascade hidden", function (assert) {
			...
			var i,
				$elems = jQuery("<div></div>")
					.appendTo("#qunit-fixture")
					.html(
						"<div class='hidden' data-expected-display='block'></div>" +
		...
		QUnit.test("show/hide 3.0, inline hidden", function (assert) {
			...
			var i,
				$elems = jQuery("<div></div>")
					.appendTo("#qunit-fixture")
					.html(
						"<span data-expected-display='inline' style='display:none'></span>" +
	...
	QUnit[jQuery.find.compile && jQuery.fn.toggle ? "test" : "skip"](
		"toggle()",
		function (assert) {
			assert.expect(9);
			var div,
			...
			div = jQuery("<div style='display:none'><div></div></div>").appendTo(
				"#qunit-fixture"
			);
			x = div.find("div");
	...
	QUnit[jQuery.find.compile && jQuery.fn.toggle ? "test" : "skip"](
		...
		function (assert) {
			...
			var detached = jQuery("<p><a></a><p>").find("*").addBack(),
				hiddenDetached = jQuery("<p><a></a></p>")
					.find("*")
					.addBack()
					.css("display", "none"),
				cascadeHiddenDetached = jQuery("<p><a></a></p>")
					.find("*")
					.addBack()
					.addClass("hidden");
			...
			assert.equal(
				detached[1].style.display,
				"none",
				"element in detached tree"
			);

			hiddenDetached.toggle();
			hiddenDetached.appendTo("#qunit-fixture");
			assert.equal(
				hiddenDetached[0].style.display,
				"",
				"detached, hidden element"
			);
			assert.equal(
				hiddenDetached[1].style.display,
				"",
			...
			assert.equal(
				cascadeHiddenDetached[0].style.display,
				"none",
				"detached, cascade-hidden element"
			);
			assert.equal(
				cascadeHiddenDetached[1].style.display,
				"none",
	...
	QUnit.test(
		"jQuery.css(elem, 'height') doesn't clear radio buttons (bug #1095)",
		function (assert) {
			assert.expect(4);
			...
			assert.ok(
				jQuery("input[type='radio']", $checkedtest).first().attr("checked"),
				"Check first radio still checked."
			);
			assert.ok(
				!jQuery("input[type='radio']", $checkedtest).last().attr("checked"),
				"Check last radio still NOT checked."
			);
			assert.ok(
				jQuery("input[type='checkbox']", $checkedtest).first().attr("checked"),
				"Check first checkbox still checked."
			);
			assert.ok(
				!jQuery("input[type='checkbox']", $checkedtest).last().attr("checked"),
	...
	QUnit.test(
		"internal ref to elem.runtimeStyle (bug #7608)",
		function (assert) {
			assert.expect(1);
			var result = true;
	...
	QUnit.test(
		"box model properties incorrectly returning % instead of px, see #10639 and #12088",
		function (assert) {
			assert.expect(2);
			...
			var container = jQuery("<div></div>")
					...
					.appendTo("#qunit-fixture"),
				el = jQuery("<div></div>")
					.css({ width: "50%", marginRight: "50%" })
					.appendTo(container),
				el2 = jQuery("<div></div>")
					.css({ width: "50%", minWidth: "300px", marginLeft: "25%" })
					.appendTo(container);

			assert.equal(
				el.css("marginRight"),
				"200px",
				"css('marginRight') returning % instead of px, see #10639"
			);
			assert.equal(
				el2.css("marginLeft"),
				"100px",
				"css('marginLeft') returning incorrect pixel value, see #12088"
			);
		}
	);
	...
	QUnit.test("jQuery.cssProps behavior, (bug #8402)", function (assert) {
		...
		assert.equal(
			div.css("top"),
			"10px",
			"the fixed property is used when accessing the computed style"
		);
		div.css("top", "100px");
		assert.equal(
			div[0].style.left,
			"100px",
			"the fixed property is used when setting the style"
		);
	...
	QUnit.test("widows & orphans #8936", function (assert) {
		...
		assert.equal(
			$p.css("widows") || jQuery.style($p[0], "widows"),
			3,
			"widows correctly set to 3"
		);
		assert.equal(
			$p.css("orphans") || jQuery.style($p[0], "orphans"),
			3,
	...
	QUnit.test(
		"can't get css for disconnected in IE<9, see #10254 and #8388",
		function (assert) {
			assert.expect(2);
			var span, div;

			span = jQuery("<span></span>").css(
				"background-image",
				"url(" + baseURL + "1x1.jpg)"
			);
			assert.notEqual(
				...
				null,
				"can't get background-image in IE<9, see #10254"
			);
	...
	QUnit.test(
		"Ensure styles are retrieving from parsed html on document fragments",
		function (assert) {
			assert.expect(1);
			...
			assert.equal(
				$span.css("font-size"),
				"14px",
				"Font-size retrievable on parsed HTML node"
			);
		}
	);

	QUnit.test(
		"can't get background-position in IE<9, see #10796",
		function (assert) {
	...
	if (jQuery.fn.offset) {
		QUnit.test(
			"percentage properties for left and top should be transformed to pixels, see #9505",
			function (assert) {
				assert.expect(2);
				var parent = jQuery(
					...
					div = jQuery(
						"<div style='position: absolute; width: 20px; height: 20px; top:50%; left:50%'></div>"
					).appendTo(parent);

				assert.equal(
					div.css("top"),
					"100px",
					"position properties not transformed to pixels, see #9505"
				);
				assert.equal(
					div.css("left"),
					"100px",
	...
	QUnit.test("Do not append px (#9548, #12990, #2792)", function (assert) {
		...
		assert.equal(
			$div.css("fill-opacity"),
			1,
			"Do not append px to 'fill-opacity'"
		);

		$div.css("column-count", 1);
		if ($div.css("column-count") !== undefined) {
			assert.equal(
				$div.css("column-count"),
				1,
				"Do not append px to 'column-count'"
			);
		} else {
		...
		if ($div.css("animation-iteration-count") !== undefined) {
			...
			assert.equal(
				$div.css("animation-iteration-count"),
				2,
				"Do not append px to 'animation-iteration-count'"
			);
		} else {
	...
	QUnit[jQuery("<div/>")[0].style.gridArea === "" ? "test" : "skip"](
		"Do not append px to CSS Grid-related properties (gh-4007)",
		function (assert) {
			assert.expect(12);
	...
	QUnit.test(
		"css('width') and css('height') should respect box-sizing, see #11004",
		function (assert) {
			assert.expect(4);
	...
	QUnit.test(
		"table rows width/height should be unaffected by inline styles",
		function (assert) {
			assert.expect(2);
			...
			assert.ok(
				parseInt(tr.css("width")) > 10,
				"tr width unaffected by inline style"
			);
			assert.ok(
				parseInt(tr.css("height")) > 10,
				"tr height unaffected by inline style"
			);
		}
	);
	...
	testIframe(
		...
		function (assert, jQuery, window, document, widthBeforeSet, widthAfterSet) {
			...
			assert.strictEqual(
				widthAfterSet,
				"100px",
				"elem.css('width', val) works correctly with browser zoom"
			);
		}
	);
	...
	(function () {
		...
		supportsFractionalGBCR =
			div.getBoundingClientRect().width.toFixed(1) === "3.3";
		qunitFixture.removeChild(div);

		QUnit.test(
			"css('width') and css('height') should return fractional values for nodes in the document",
			function (assert) {
				if (!supportsFractionalGBCR) {
					assert.expect(1);
					assert.ok(
						true,
						"This browser doesn't support fractional values in getBoundingClientRect()"
					);
					return;
				}
				...
				var el = jQuery("<div class='test-div'></div>").appendTo(
					"#qunit-fixture"
				);
				jQuery(
					"<style>.test-div { width: 33.3px; height: 88.8px; }</style>"
				).appendTo("#qunit-fixture");

				assert.equal(
					Number(el.css("width").replace(/px$/, "")).toFixed(1),
					"33.3",
					"css('width') should return fractional values"
				);
				assert.equal(
					Number(el.css("height").replace(/px$/, "")).toFixed(1),
					"88.8",
		...
		QUnit.test(
			"css('width') and css('height') should return fractional values for disconnected nodes",
			function (assert) {
				if (!supportsFractionalGBCR) {
					assert.expect(1);
					assert.ok(
	...
	QUnit.test(
		"certain css values of 'normal' should be convertable to a number, see #8627",
		function (assert) {
			assert.expect(3);

			var el = jQuery(
				"<div style='letter-spacing:normal;font-weight:normal;'>test</div>"
			).appendTo("#qunit-fixture");

			assert.ok(
				!isNaN(parseFloat(el.css("letterSpacing"))),
				"css('letterSpacing') not convertable to number, see #8627"
			);
			assert.ok(
				!isNaN(parseFloat(el.css("fontWeight"))),
				"css('fontWeight') not convertable to number, see #8627"
			);
			assert.equal(
				typeof el.css("fontWeight"),
				"string",
	...
	if (document.documentMode === 9) {
		QUnit.test(
			".css('filter') returns a string in IE9, see #12537",
			function (assert) {
				assert.expect(1);

				assert.equal(
					jQuery(
						"<div style='-ms-filter:\"progid:DXImageTransform.Microsoft.gradient(startColorstr=#FFFFFF, endColorstr=#ECECEC)\";'></div>"
					).css("filter"),
	...
	QUnit.test("cssHooks - expand", function (assert) {
		...
		jQuery.each(properties, function (property, keys) {
			...
			assert.deepEqual(
				result,
				expected,
				property + " expands properly with a number"
			);
			...
			assert.deepEqual(
				result,
				expected,
				property + " expands properly with '10px'"
			);
			...
			assert.deepEqual(
				result,
				expected,
				property + " expands properly with '10px 20px'"
			);
			...
			assert.deepEqual(
				result,
				expected,
				property + " expands properly with '10px 20px 30px'"
			);
			...
			assert.deepEqual(
				result,
				expected,
				property + " expands properly with '10px 20px 30px 40px'"
			);
		});
	});

	QUnit.test(
		"css opacity consistency across browsers (#12685)",
		function (assert) {
			assert.expect(3);
			...
			jQuery("<style>.opacity_t12685 { opacity: 0.1; }</style>").appendTo(
				fixture
			);
			...
			assert.equal(
				Math.round(el.css("opacity") * 100),
				10,
				"opacity from style sheet"
			);
			el.css("opacity", 0.3);
	...
	QUnit[jQuery.find.compile ? "test" : "skip"](
		...
		function (assert) {
			...
			assert.ok(
				jQuery("#nothiddendiv").is(":visible"),
			...
			assert.ok(
				jQuery("#siblingspan").is(":visible"),
				"Span with no content is visible"
			);
			$div = jQuery("<div><span></span></div>").appendTo("#qunit-fixture");
			assert.equal(
				$div.find(":visible").length,
				1,
				"Span with no content is visible"
			);
			...
			assert.ok(
				$div.is(":visible"),
				"Div with width and height of 0 is still visible (gh-2227)"
			);
			...
			assert.equal(
				jQuery("#table td:visible").length,
				1,
			...
			assert.equal(
				jQuery("#table td:visible").length,
				0,
				"hidden cell children not perceived as visible (#4512)"
			);

			assert.t("Is Visible", "#qunit-fixture div:visible:lt(2)", [
				"foo",
				"nothiddendiv",
			]);
			...
			assert.ok(
				$a.is(":visible"),
				"Anchor tag with flow content is visible (gh-2227)"
			);
		}
	);

	QUnit.test(
		"Keep the last style if the new one isn't recognized by the browser (#14836)",
		function (assert) {
			assert.expect(1);

			var el = jQuery("<div></div>")
				.css("position", "absolute")
				.css("position", "fake value");
			assert.equal(
				el.css("position"),
				"absolute",
				"The old style is kept when setting an unrecognized value"
			);
		}
	);
	...
	QUnit[/\bedge\/16\./i.test(navigator.userAgent) ? "skip" : "test"](
		...
		function (assert) {
			...
			assert.equal(
				el.css("position"),
				"absolute",
				"The old style is kept when setting to a space"
			);
		}
	);
		...
		function (assert) {
			...
			var styles = [
				...
				},
				{
					...
					expected: [
						"none",
						'url("http://static.jquery.com/files/rocker/images/logo_jquery_215x53.gif")',
					],
				},
				{
			...
			jQuery.each(styles, function (index, style) {
				...
				if (source.style[style.name] === undefined) {
					assert.ok(
						true,
						style.name + ": style isn't supported and therefore not an issue"
					);
					assert.ok(true);
				...
				window.setTimeout(function () {
					assert.notEqual(
						$clone.css(style.name),
						style.value[0],
						"Cloned css was changed"
					);

					assert.ok(
						jQuery.inArray($source.css(style.name) !== -1, style.value),
						"Clearing clone.css() doesn't affect source.css(): " +
							style.name +
							"; result: " +
							$source.css(style.name) +
							"; expected: " +
							style.value.join(",")
					);

					assert.ok(
						jQuery.inArray($children.css(style.name) !== -1, style.value),
						"Clearing clonedChildren.css() doesn't affect children.css(): " +
							style.name +
							"; result: " +
							$children.css(style.name) +
							"; expected: " +
							style.value.join(",")
					);
				}, 100);
	...
	(function () {
		...
		if (exist) {
			QUnit.test(
				'Don\'t append px to CSS "order" value (#14049)',
				function (assert) {
					assert.expect(1);
	...
	QUnit.test(
		"Do not throw on frame elements from css method (#15098)",
		function (assert) {
			assert.expect(1);
	...
	(function () {
		...
		QUnit.test(
			"Don't default to a cached previously used wrong prefixed name (gh-2015)",
			function (assert) {
				...
				if (appearanceName) {
					assert.equal(
						elemStyle[appearanceName],
						"none",
						"setting properly-prefixed appearance"
					);
				}
				if (transformName) {
					assert.equal(
						elemStyle[transformName],
						transformVal,
		...
		QUnit.test(
			"Don't detect fake set properties on a node when caching the prefixed version",
			function (assert) {
				assert.expect(1);
				...
				assert.equal(
					style.MozFakeProperty,
					"old value",
					"Fake prefixed property is not cached"
				);
			}
		);
	})();
	...
	(function () {
		...
		QUnit[supportsCssVars ? "test" : "skip"](
			"css(--customProperty)",
			function (assert) {
				...
				assert.equal(
					div.css("--color"),
					"blue",
					"Modified CSS custom property using string"
				);
				...
				assert.equal(
					div.css("--color"),
					"red",
					"Modified CSS custom property using object"
				);
				...
				if (!oldSafari) {
					assert.equal(
						$elem.css("--prop2"),
						" val2",
						"Preceding whitespace maintained"
					);
					assert.equal(
						$elem.css("--prop3"),
						"val3 ",
						"Following whitespace maintained"
					);
				}
		...
		QUnit[supportsCssVars ? "test" : "skip"](
			"Don't append px to CSS vars",
			function (assert) {
				assert.expect(3);

				var $div = jQuery("<div>").appendTo("#qunit-fixture");

				$div.css("--a", 3).css("--line-height", 4).css("--lineHeight", 5);
