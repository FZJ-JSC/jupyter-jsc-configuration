...
(function () {
			...
			createDeferred(function (defer) {
				...
				jQuery.each(promise, function (key) {
					if (typeof promise[key] !== "function") {
						assert.ok(
							false,
							key + " is a function (" + typeof promise[key] + ")"
						);
					}
				...
				createDeferred(function (defer) {
					...
					defer.progress(function (value) {
						assert.strictEqual(
							value,
							checked,
							"Progress: right value (" + value + ") received"
						);
					});
					...
					assert.strictEqual(
						defer.state(),
						"pending",
						"pending after notification"
					);
					defer[change]();
					assert.notStrictEqual(
						defer.state(),
						"pending",
						"not pending after " + change
					);
					defer.notify();
				});
			});
		...
		jQuery
			...
			.done(function (value) {
				assert.strictEqual(
					value,
					undefined,
					"then done callback can return undefined/null"
				);
				done.pop().call();
			});
	});
	...
	QUnit[typeof Symbol === "function" && Symbol.toStringTag ? "test" : "skip"](
		...
		function (assert) {
			...
			function faker() {
				assert.ok(
					true,
					"handler with non-'Function' @@toStringTag gets invoked"
				);
			}
		...
		function (assert) {
			...
			window.console.warn = function () {
				...
				if (/chrome/i.test(navigator.userAgent)) {
					oldChromium =
						parseInt(navigator.userAgent.match(/chrome\/(\d+)/i)[1], 10) < 42;
				}
				if (oldChromium) {
					assert.ok(
						/(?:barf|undefined)/.test(msg),
						"Message (weak assertion): " + msg
					);
				} else {
	...
	QUnit[window.console ? "test" : "skip"](
		...
		function (assert) {
			...
			window.console.warn = function () {
				...
				if (oldChromium) {
					assert.ok(
						/(?:cough_up_hairball|undefined)/.test(msg),
						"Function mentioned (weak assertion): " + msg
					);
				} else {
					assert.ok(
						/cough_up_hairball/.test(msg),
	...
	QUnit.test(
		"jQuery.Deferred.then - progress and thenables",
		function (assert) {
			assert.expect(2);
	...
	QUnit.test("jQuery.Deferred - notify and resolve", function (assert) {
		...
		var notifiedResolved = jQuery
				.Deferred()
				.notify("foo") /*xxx .resolve( "bar" )*/,
			done = jQuery.map(new Array(3), function () {
				return assert.async();
			});
		...
		notifiedResolved
			...
			.progress(function (v) {
				...
				assert.strictEqual(
					v,
					"quux",
					"deferred replaced then'd progress value"
				);
				done.pop().call();
			});
	});
	...
	QUnit.test(
		"jQuery.Deferred - resolved to a notifying deferred",
		function (assert) {
			assert.expect(2);
			...
			deferred.resolve(
				jQuery.Deferred(function (notifyingDeferred) {
	...
	QUnit.test(
		"jQuery.when(nonThenable) - like Promise.resolve",
		function (assert) {
			"use strict";

			assert.expect(44);

			var // Support: Android 4.0 only
			...
			jQuery
				...
				.done(function (resolveValue) {
					assert.strictEqual(
						arguments.length,
						0,
						"Resolved .done with no arguments"
					);
					assert.strictEqual(
						this,
						defaultContext,
				...
				.then(function (resolveValue) {
					assert.strictEqual(
						arguments.length,
						0,
						"Resolved .then with no arguments"
					);
					assert.strictEqual(
						this,
						defaultContext,
	...
	QUnit.test("jQuery.when(thenable) - like Promise.resolve", function (assert) {
		...
		var slice = [].slice,
			...
			inputs = {
				...
				multiDeferredWith: jQuery
					.Deferred()
					.resolveWith(sentinel, ["foo", "bar"]),
				...
				multiRejectedDeferredWith: jQuery
					.Deferred()
					.rejectWith(sentinel, ["baz", "quux"]),
			},
			contexts = {
			...
			numCases =
				Object.keys(willSucceed).length + Object.keys(willError).length,
			// Support: Android 4.0 only
			// Strict mode functions invoked without .call/.apply get global-object context
			defaultContext = function getDefaultContext() {
				return this;
		...
		jQuery.each(
			...
			function (label, args) {
				...
				assert.ok(
					...
					"Thenable returned from " + label
				);
				assert.strictEqual(
					result.resolve,
					undefined,
					"Non-deferred returned from " + label
				);
				assert.strictEqual(
					result.promise(),
					result,
				...
				jQuery.each(args, function (i, arg) {
					assert.notStrictEqual(
						result,
						arg,
						"Returns distinct from arg " + i + " of " + label
					);
					if (arg.promise) {
						assert.notStrictEqual(
							result,
	...
	QUnit.test(
		"jQuery.when - notify does not affect resolved",
		function (assert) {
			assert.expect(3);
	...
	QUnit.test(
		"jQuery.when(...) - opportunistically synchronous",
		function (assert) {
			assert.expect(5);
			...
			jQuery
				.when(resolved)
				.done(validate("jQuery.when(alreadyFulfilled)"))
				.always(done);
			jQuery
				.when(rejected)
				.fail(validate("jQuery.when(alreadyRejected)"))
				.always(done);
			jQuery
