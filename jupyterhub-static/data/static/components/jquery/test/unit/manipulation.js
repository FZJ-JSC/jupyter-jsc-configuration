...
QUnit.test("text()", function (assert) {
	...
	assert.equal(
		jQuery(document.createTextNode("foo")).text(),
		"foo",
	...
	assert.notEqual(
		...
		"",
		"Retrieving text for the document retrieves all text (#10724)."
	);
	...
	assert.equal(
		jQuery(frag).text(),
		"foo",
		"Document Fragment Text node was retrieved from .text()."
	);
	...
	assert.equal(
		$newLineTest.text(),
		"test\ntesty",
		"text() does not remove new lines (#11153)"
	);
...
QUnit.test("text(undefined)", function (assert) {
	...
	assert.equal(
		jQuery("#foo").text("<div").text(undefined)[0].innerHTML,
		"&lt;div",
...
function testText(valueObj, assert) {
	...
	assert.equal(
		jQuery("#foo").text(val)[0].innerHTML.replace(/>/g, "&gt;"),
		"&lt;div&gt;&lt;b&gt;Hello&lt;/b&gt; cruel world!&lt;/div&gt;",
		"Check escaped text"
	);
	...
	assert.equal(
		$multipleElements.eq(0).text(),
		expected,
		"text() updates multiple elements (#11809)"
	);
	assert.equal(
		$multipleElements.eq(1).text(),
		expected,
		"text() updates multiple elements (#11809)"
	);
...
function testAppendForObject(valueObj, isFragment, assert) {
	...
	assert.equal(
		$base
			.clone()
			.append(valueObj(first.cloneNode(true)))
			.text(),
		text + "Try them out:",
	...
	assert.equal(
		$base
			.clone()
			.append(valueObj([first.cloneNode(true), yahoo.cloneNode(true)]))
			.text(),
	...
	assert.equal(
		$base
			.clone()
			.append(valueObj(jQuery("#yahoo, #first").clone()))
			.text(),
	...
	assert.equal(
		$base.clone().append(valueObj(5)).text(),
		text + "5",
	...
	assert.equal(
		$base
			.clone()
			.append(
				valueObj([jQuery("#first").clone(), jQuery("#yahoo, #google").clone()])
			)
			.text(),
	...
	assert.equal(
		$base.clone().append(valueObj(" text with spaces ")).text(),
		text + " text with spaces ",
		"Check for appending text with spaces" + type
	);

	assert.equal(
		$base.clone().append(valueObj([])).text(),
		text,
	...
	assert.equal(
		$base
			.clone()
			.append(valueObj(document.getElementsByTagName("foo")))
			.text(),
		text,
	...
	assert.equal(
		$base
			.clone()
			.append(valueObj(document.getElementById("form").cloneNode(true)))
...
function testAppend(valueObj, assert) {
	...
	assert.equal(
		jQuery("#select3")
			.append(valueObj("<option value='appendTest'>Append Test</option>"))
	...
	jQuery("#qunit-fixture form").append(
		valueObj("<input name='radiotest' type='radio' checked='checked' />")
	);
	jQuery("#qunit-fixture form input[name=radiotest]")
		.each(function () {
	...
	jQuery("#qunit-fixture form input[name=radiotest2]")
		.each(function () {
			assert.ok(
				jQuery(this).is(":checked"),
				"Append alternately formatted checked radio"
			);
		})
		.remove();

	jQuery("#qunit-fixture form").append(
		valueObj("<input name='radiotest3' type='radio' checked />")
	);
	jQuery("#qunit-fixture form input[name=radiotest3]")
		.each(function () {
			assert.ok(
				jQuery(this).is(":checked"),
				"Append HTML5-formatted checked radio"
			);
		})
		.remove();

	jQuery("#qunit-fixture form").append(
		valueObj("<input type='radio' checked='checked' name='radiotest4' />")
	);
	jQuery("#qunit-fixture form input[name=radiotest4]")
		.each(function () {
			assert.ok(
				jQuery(this).is(":checked"),
				"Append with name attribute after checked attribute"
			);
		})
		.remove();
	...
	try {
		if (iframeDoc && iframeDoc.body) {
			assert.equal(
				jQuery(iframeDoc.body).append(
					valueObj("<div id='success'>test</div>")
				)[0].lastChild.id,
				"success",
				message
			);
	...
	jQuery("<fieldset></fieldset>")
		.appendTo("#form")
		.append(valueObj("<legend id='legend'>test</legend>"));
	assert.t("Append legend", "#legend", ["legend"]);

	$map = jQuery("<map></map>").append(
		valueObj(
			"<area id='map01' shape='rect' coords='50,50,150,150' href='http://www.jquery.com/' alt='jQuery'>"
		)
	);

	assert.equal($map[0].childNodes.length, 1, "The area was inserted.");
	assert.equal(
		$map[0].firstChild.nodeName.toLowerCase(),
		"area",
		"The area was inserted."
	);

	jQuery("#select1").append(valueObj("<OPTION>Test</OPTION>"));
	assert.equal(
		jQuery("#select1 option:last-child").text(),
		"Test",
		"Appending OPTION (all caps)"
	);

	jQuery("#select1").append(
		valueObj("<optgroup label='optgroup'><option>optgroup</option></optgroup>")
	);
	assert.equal(
		jQuery("#select1 optgroup").attr("label"),
		"optgroup",
		"Label attribute in newly inserted optgroup is correct"
	);
	assert.equal(
		jQuery("#select1 option").last().text(),
		"optgroup",
		"Appending optgroup"
	);
	...
	jQuery.each(
		"thead tbody tfoot colgroup caption tr th td".split(" "),
		function (i, name) {
			$table.append(valueObj("<" + name + "/>"));
			assert.equal($table.find(name).length, 1, "Append " + name);
			assert.ok(
				jQuery.parseHTML("<" + name + "/>").length,
				name + " wrapped correctly"
			);
		}
	);
	...
	jQuery("#form")
		...
		.append(
			valueObj("<select id='appendSelect2'><option>Test</option></select>")
		);
	assert.t("Append Select", "#appendSelect1, #appendSelect2", [
		"appendSelect1",
		"appendSelect2",
	]);
	...
	assert.equal(
		"Two nodes",
		jQuery("<div></div>").append("Two", " nodes").text(),
		"Appending two text nodes (#4011)"
	);
	assert.equal(
		jQuery("<div></div>").append("1", "", 3).text(),
		"13",
	...
	assert.equal(
		jQuery("#nonnodes").length,
		1,
		"Check node,textnode,comment append moved leaving just the div"
	);
	assert.equal(
		d.contents().length,
		3,
	...
	d.remove();
	assert.equal(
		jQuery("#nonnodes").contents().length,
		3,
	...
	assert.equal(
		...
		true,
		"Reappending radios uphold which radio is checked"
	);
	assert.equal(
		$radioUnchecked[0].checked,
		false,
		"Reappending radios uphold not being checked"
	);

	assert.equal(
		jQuery("<div></div>").append(valueObj("option<area></area>"))[0].childNodes
			.length,
		2,
...
QUnit.test("append(Function) returns Element", function (assert) {
	assert.expect(2);
	var expected =
			'This link has class="blog": Simon Willison\'s WeblogTry them out:',
		old = jQuery("#sap").html();
...
QUnit.test("append(Function) returns Number", function (assert) {
	...
	assert.ok(
		jQuery("#sap")[0].innerHTML.match(/5$/),
...
QUnit.test("XML DOM manipulation (#9960)", function (assert) {
	...
	assert.deepEqual(
		jQuery("state", xml2).get(),
		scxml1.find("state").get(),
		"replaceWith"
	);
});
...
QUnit.test("append HTML5 sectioning elements (Bug #6485)", function (assert) {
	...
	assert.equal(
		article.get(0).style.fontSize,
		"10px",
		"HTML5 elements are styleable"
	);
	assert.equal(
		aside.length,
		1,
		"HTML5 elements do not collapse their children"
	);
});

if (jQuery.css) {
	QUnit.test(
		"HTML5 Elements inherit styles from style rules (Bug #10501)",
		function (assert) {
			assert.expect(1);

			jQuery("#qunit-fixture").append("<article id='article'></article>");
			jQuery("#article").append(
				"<section>This section should have a pink background.</section>"
			);
...
QUnit.test("html(String) with HTML5 (Bug #6485)", function (assert) {
	...
	jQuery("#qunit-fixture").html(
		"<article><section><aside>HTML5 elements</aside></section></article>"
	);
	assert.equal(
		jQuery("#qunit-fixture").children().children().length,
		1,
		"Make sure HTML5 article elements can hold children. innerHTML shortcut path"
	);
	assert.equal(
		jQuery("#qunit-fixture").children().children().children().length,
		1,
...
QUnit.test(
	"html(String) tag-hyphenated elements (Bug #1987)",
	function (assert) {
		assert.expect(27);
		...
		jQuery.each(
			...
			function (i, name) {
				...
				assert.ok(
					j[0].nodeName === name.toUpperCase() + "-D",
					"Hyphenated node name"
				);
				assert.ok(
					j[1].nodeName === name.toUpperCase() + "-D",
					"Hyphenated node name"
				);
			}
		);
		...
		var j = jQuery(
			"<tr-multiple-hyphens><td-with-hyphen>text</td-with-hyphen></tr-multiple-hyphens>"
		);
		assert.ok(
			j[0].nodeName === "TR-MULTIPLE-HYPHENS",
			"Tags with multiple hyphens"
		);
		assert.ok(
			j.children()[0].nodeName === "TD-WITH-HYPHEN",
			"Tags with multiple hyphens"
		);
		assert.equal(
			j.children().text(),
			"text",
			"Tags with multiple hyphens behave normally"
		);
	}
);

QUnit.test(
	"Tag name processing respects the HTML Standard (gh-2005)",
	function (assert) {
		assert.expect(240);
		...
		function assertSpecialCharsSupport(method, characters) {
			...
			if (
				...
				assert.ok(
					true,
					"This Chromium version upper-cases µ incorrectly; skip test"
				);
				assert.ok(
					true,
					"This Chromium version upper-cases µ incorrectly; skip test"
...
QUnit.test("IE8 serialization bug", function (assert) {
	...
	assert.equal(
		wrapper.children("article").length,
		1,
		"HTML5 elements are insertable with .html()"
	);

	wrapper.html("<div></div><link></link>");
	assert.equal(
		wrapper.children("link").length,
		1,
		"Link elements are insertable with .html()"
	);
});
...
QUnit.test("html() object element #10324", function (assert) {
	...
	assert.equal(
		clone.html(),
		object.html(),
		"html() returns correct innerhtml of cloned object elements"
	);
});
...
QUnit.test("append(xml)", function (assert) {
	...
	assert.ok(
		jQuery(xml1).append(xml2),
		"Append an xml element to another without raising an exception."
	);
});
...
QUnit.test("appendTo(String)", function (assert) {
	...
	assert.equal(
		jQuery("<option value='appendTest'>Append Test</option>")
			.appendTo("#select3")
			.parent()
			.find("option:last-child")
			.attr("value"),
	...
	jQuery("<strong>test</strong>");
	jQuery([
		jQuery("<strong>test</strong>")[0],
		jQuery("<strong>test</strong>")[0],
	]).appendTo("#first");
	assert.equal(
		jQuery("#first").children().length,
		l,
		"Make sure the elements were inserted."
	);
	assert.equal(
		jQuery("#first").children().last()[0].nodeName.toLowerCase(),
		"strong",
...
QUnit.test("appendTo(jQuery)", function (assert) {
	...
	assert.ok(
		jQuery(document.createElement("script")).appendTo("body").length,
	...
	assert.ok(
		jQuery("#qunit-fixture div").last().hasClass("test"),
		"appendTo element was modified after the insertion"
	);
	assert.ok(
		jQuery("#moretests div").last().hasClass("test"),
		"appendTo element was modified after the insertion"
	);
	...
	assert.equal(
		div.children().length,
		1,
		"Make sure the right number of children were inserted."
	);
	...
	assert.equal(
		jQuery("#qunit-fixture div").length,
		num,
		"Make sure all the removed divs were inserted."
	);
});
...
QUnit.test("prepend(String)", function (assert) {
	...
	assert.equal(
		jQuery("#select3")
			.prepend("<option value='prependTest'>Prepend Test</option>")
...
QUnit.test("prepend(Element)", function (assert) {
	...
	var expected;
	expected =
		'Try them out:This link has class="blog": Simon Willison\'s Weblog';
...
QUnit.test("prepend(Array<jQuery>)", function (assert) {
	...
	var expected;
	expected =
		'Try them out:GoogleYahooThis link has class="blog": Simon Willison\'s Weblog';
	jQuery("#sap").prepend([jQuery("#first"), jQuery("#yahoo, #google")]);
	assert.equal(
		jQuery("#sap").text(),
		expected,
		"Check for prepending of array of jQuery objects"
	);
});

QUnit.test(
	"prepend(Function) with incoming value -- String",
	function (assert) {
		assert.expect(4);
...
QUnit.test(
	"prepend(Function) with incoming value -- Element",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"prepend(Function) with incoming value -- Array<Element>",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"prepend(Function) with incoming value -- jQuery",
	function (assert) {
		assert.expect(2);
...
QUnit.test("prependTo(String)", function (assert) {
	...
	assert.equal(
		jQuery("<option value='prependTest'>Prepend Test</option>")
			.prependTo("#select3")
			.parent()
			.find("option:first-child")
			.attr("value"),
...
QUnit.test("prependTo(Array<jQuery>)", function (assert) {
	...
	jQuery(
		"<select id='prependSelect2'><option>Test</option></select>"
	).prependTo("#form");

	assert.t("Prepend Select", "#prependSelect2, #prependSelect1", [
		"prependSelect2",
		"prependSelect1",
	]);
});
...
QUnit.test("before(Element)", function (assert) {
	...
	jQuery("#yahoo").before(
		manipulationBareObj(document.getElementById("first"))
	);
...
QUnit.test("before(Array<jQuery>)", function (assert) {
	...
	jQuery("#yahoo").before(
		manipulationBareObj([jQuery("#first"), jQuery("#mark, #google")])
	);
	assert.equal(
		jQuery("#en").text(),
		expected,
		"Insert array of jQuery objects before"
	);
});
...
QUnit.test("before(Function) -- Returns Element", function (assert) {
	...
	expected = "This is a normal link: Try them out:Yahoo";
	jQuery("#yahoo").before(
		manipulationFunctionReturningObj(document.getElementById("first"))
	);
	assert.equal(jQuery("#en").text(), expected, "Insert element before");
});
...
QUnit.test("before(Function) -- Returns jQuery", function (assert) {
	...
	expected = "This is a normal link: diveintomarkTry them out:Yahoo";
	jQuery("#yahoo").before(
		manipulationFunctionReturningObj(jQuery("#mark, #first"))
	);
	assert.equal(jQuery("#en").text(), expected, "Insert jQuery before");
});
...
QUnit.test("before(no-op)", function (assert) {
	...
	assert.equal(
		set[0].nodeName.toLowerCase(),
		"div",
		"Insert before a disconnected node should be a no-op"
	);
	assert.equal(
		set.length,
		1,
		"Insert the element before the disconnected node. should be a no-op"
	);
});
...
QUnit.test(".before() and .after() disconnected node", function (assert) {
	...
	assert.equal(
		jQuery("<input type='checkbox'/>").before("<div></div>").length,
		1,
		"before() on disconnected node is no-op"
	);
	assert.equal(
		jQuery("<input type='checkbox'/>").after("<div></div>").length,
		1,
...
QUnit.test(
	"insert with .before() on disconnected node last",
	function (assert) {
		assert.expect(1);
		...
		assert.equal(
			jQuery("#en").text(),
			expectedBefore,
			"Insert String before with disconnected node last"
		);
	}
);

QUnit.test(
	"insert with .before() on disconnected node first",
	function (assert) {
		assert.expect(1);
		...
		assert.equal(
			jQuery("#en").text(),
			expectedBefore,
			"Insert String before with disconnected node first"
		);
	}
);
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			jQuery("#en").text(),
			expectedAfter,
			"Insert String after with disconnected node last"
		);
	}
);
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			jQuery("#en").text(),
			expectedAfter,
			"Insert String after with disconnected node first"
		);
	}
);
...
QUnit.test(".after(jQuery)", function (assert) {
	...
	assert.equal(
		jQuery("#en").text(),
		expected,
		"Insert array of jQuery objects after"
	);
});
...
QUnit.test(".after(Function) returns Array<Element>", function (assert) {
	...
	var expected = "This is a normal link: YahooTry them out:diveintomark",
		val = manipulationFunctionReturningObj;
	jQuery("#yahoo").after(
		val([document.getElementById("first"), document.getElementById("mark")])
	);
	assert.equal(
		jQuery("#en").text(),
		expected,
...
QUnit.test(".after(disconnected node)", function (assert) {
	...
	assert.equal(
		set[0].nodeName.toLowerCase(),
		"div",
		"Insert after a disconnected node should be a no-op"
	);
	assert.equal(
		set.length,
		1,
		"Insert the element after the disconnected node should be a no-op"
	);
});
...
function testReplaceWith(val, assert) {
	...
	assert.deepEqual(
		jQuery("#yahoo, #baz").get(),
		q("yahoo", "baz"),
		"Replace element with multiple arguments (#13722)"
	);
	assert.strictEqual(
		jQuery("#yahoo")[0].nextSibling,
		jQuery("#baz")[0].previousSibling,
		"Argument order preserved"
	);
	assert.deepEqual(
		jQuery("#bar").get(),
		[],
		"Verify that original element is gone, after multiple arguments"
	);

	jQuery("#google").replaceWith(
		val([document.getElementById("first"), document.getElementById("mark")])
	);
	assert.deepEqual(
		jQuery("#mark, #first").get(),
		q("first", "mark"),
	...
	assert.deepEqual(
		jQuery("#mark, #first").get(),
		q("first", "mark"),
		"Replace element with jQuery collection"
	);
	assert.ok(
		!jQuery("#groups")[0],
		"Verify that original element is gone, after jQuery collection"
	);

	jQuery("#mark, #first").replaceWith(
		val("<span class='replacement'></span><span class='replacement'></span>")
	);
	assert.equal(
		jQuery("#qunit-fixture .replacement").length,
		4,
		"Replace multiple elements (#12449)"
	);
	assert.deepEqual(
		jQuery("#mark, #first").get(),
		[],
	...
	$div.replaceWith($div);
	assert.deepEqual(
		jQuery(".pathological", "#qunit-fixture").get(),
		$div.get(),
		"Self-replacement"
	);
	$div.replaceWith(child);
	assert.deepEqual(
		jQuery("#qunit-fixture").children().first().get(),
		child.get(),
	...
	nonExistent = jQuery("#does-not-exist").replaceWith(
		val("<b>should not throw an error</b>")
	);
	...
	$div.replaceWith(
		val(
			"<div class='replacewith'></div><script>" +
	...
	assert.equal(
		jQuery("#qunit-fixture").find("div[id=replaceWith]").length,
		1,
...
QUnit.test(
	"replaceWith(String|Element|Array<Element>|jQuery)",
	function (assert) {
...
QUnit.test("replaceWith(string) for more than one element", function (assert) {
	...
	assert.equal(
		jQuery("#foo p").length,
		3,
		"ensuring that test data has not changed"
	);

	jQuery("#foo p").replaceWith("<span>bar</span>");
	assert.equal(
		jQuery("#foo span").length,
		3,
		"verify that all the three original element have been replaced"
	);
	assert.equal(
		jQuery("#foo p").length,
		0,
...
QUnit.test(
	"Empty replaceWith (trac-13401; trac-13596; gh-2204)",
	function (assert) {
		assert.expect(25);
		...
		jQuery.each(tests, function (label, input) {
			...
			assert.strictEqual(
				$el.html(),
				"",
				"replaceWith(function returning " + label + ")"
			);
			$el
				.html("<i></i>")
				.children()
			...
			assert.strictEqual(
				$el.html(),
				"",
				"replaceWith(other function returning " + label + ")"
			);
			$el
				.html("<p></p>")
				.children()
...
QUnit.test("jQuery.clone() (#8017)", function (assert) {
	...
	assert.ok(
		jQuery.clone && typeof jQuery.clone === "function",
...
QUnit.test("append to multiple elements (#8070)", function (assert) {
	...
	var selects = jQuery(
		"<select class='test8070'></select><select class='test8070'></select>"
	).appendTo("#qunit-fixture");
...
QUnit.test("table manipulation", function (assert) {
	...
	var table = jQuery("<table style='font-size:16px'></table>")
			.appendTo("#qunit-fixture")
			.empty(),
...
QUnit.test("clone()", function (assert) {
	...
	assert.equal(
		jQuery("#first").append(jQuery("#yahoo").clone()).text(),
		"Try them out:Yahoo",
		"Check for clone"
	);
	assert.equal(
	...
	jQuery.each(
		"table thead tbody tfoot tr td div button ul ol li select option textarea iframe".split(
			" "
		),
		function (i, nodeName) {
			assert.equal(
	...
	assert.equal(
		jQuery("<input type='checkbox' />").clone()[0].nodeName.toLowerCase(),
		"input",
	...
	assert.equal(
		jQuery("#nonnodes").contents().clone().length,
		3,
	...
	div = jQuery("<div></div>").html(
		"<embed height='355' width='425' src='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'></embed>"
	);
	...
	div = jQuery("<div></div>").html(
		"<object height='355' width='425' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000'>  <param name='movie' value='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='wmode' value='transparent'> </object>"
	);
	...
	(function () {
		...
		clone.find("param").each(function (index, param) {
			params[
				param.attributes.name.nodeValue.toLowerCase()
	...
	div = jQuery("<div></div>").html(
		"<object height='355' width='425' type='application/x-shockwave-flash' data='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='movie' value='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='wmode' value='transparent'> </object>"
	);
	...
	assert.equal(
		clone.data("a"),
		false,
		"Ensure cloned element data object was correctly modified"
	);
	assert.equal(
		div.data("a"),
		true,
		"Ensure cloned element data object is copied, not referenced"
	);
	...
	assert.equal(
		jQuery(form).clone().children().length,
		1,
		"Make sure we just get the form back."
	);
...
QUnit.test("clone(script type=non-javascript) (#11359)", function (assert) {
	...
	var src = jQuery(
			"<script type='text/filler'>Lorem ipsum dolor sit amet</script><q><script type='text/filler'>consectetur adipiscing elit</script></q>"
		),
		dest = src.clone();
	...
	assert.ok(
		/^\s*<scr.pt\s+type=['"]?text\/filler['"]?\s*>consectetur adipiscing elit<\/scr.pt>\s*$/i.test(
			dest.last().html()
		),
		"Cloning preserves nested script text"
	);
...
QUnit.test("clone(form element) (Bug #3879, #6655)", function (assert) {
	...
	element = jQuery(
		"<select><option>Foo</option><option value='selected' selected>Bar</option></select>"
	);
	...
	assert.equal(
		element
			...
			.val(),
		"selected",
		"Selected option cloned correctly"
	);

	element = jQuery("<input type='checkbox' value='foo'>").attr(
		"checked",
		"checked"
	);
	clone = element.clone();
	...
	assert.equal(
		clone.is(":checked"),
		element.is(":checked"),
		"Checked input cloned correctly"
	);
	assert.equal(
		clone[0].defaultValue,
		"foo",
		"Checked input defaultValue cloned correctly"
	);
	...
	assert.equal(
		clone[0].defaultValue,
		"foo",
		"Text input defaultValue cloned correctly"
	);
	...
	assert.equal(
		clone[0].defaultValue,
		"foo",
		"Textarea defaultValue cloned correctly"
	);
});
...
QUnit.test("clone() on XML nodes", function (assert) {
	...
	assert.equal(
		origTab.text(),
		"origval",
		"Check original XML node was correctly set"
	);
	assert.equal(
		cloneTab.text(),
		"cloneval",
		"Check cloned XML node was correctly set"
	);
});
...
QUnit.test("clone() on local XML nodes with html5 nodename", function (assert) {
	...
	assert.equal(
		$meter[0].nodeName,
		"meter",
		"Check if nodeName was not changed due to cloning"
	);
	assert.equal(
		$meter[0].nodeType,
		1,
		"Check if nodeType is not changed due to cloning"
	);
});
...
QUnit.test("html(undefined)", function (assert) {
	...
	assert.equal(
		jQuery("#foo").html("<i>test</i>").html(undefined).html().toLowerCase(),
		"<i>test</i>",
...
QUnit.test("html() on empty set", function (assert) {
	...
	assert.strictEqual(
		jQuery().html(),
		undefined,
		".html() returns undefined for empty sets (#11962)"
	);
});

function childNodeNames(node) {
	return jQuery
		.map(node.childNodes, function (child) {
...
function testHtml(valueObj, assert) {
	...
	div.html(
		valueObj(
			"<div id='parent_1'><div id='child_1'></div></div><div id='parent_2'></div>"
		)
	);
	...
	expected = [];
	tmp = jQuery("<map></map>")
		.html(valueObj("<area alt='area'></area>"))
		.each(function () {
			expected.push("AREA");
	...
	assert.equal(
		div.html(valueObj(Infinity)).html(),
		"Infinity",
		"Setting Infinity as html"
	);
	assert.equal(div.html(valueObj(NaN)).html(), "", "Setting NaN as html");
	assert.equal(
		div.html(valueObj(1e2)).html(),
		"100",
		"Setting exponential number notation as html"
	);
	...
	tmp = "&lt;div&gt;hello1&lt;/div&gt;";
	assert.equal(
		div.html(valueObj(tmp)).html().replace(/>/g, "&gt;"),
		tmp,
		"Escaped html"
	);
	tmp = "x" + tmp;
	assert.equal(
		div.html(valueObj(tmp)).html().replace(/>/g, "&gt;"),
		tmp,
		"Escaped html, leading x"
	);
	tmp = " " + tmp.slice(1);
	assert.equal(
		div.html(valueObj(tmp)).html().replace(/>/g, "&gt;"),
		tmp,
		"Escaped html, leading space"
	);
	...
	assert.deepEqual(
		actual,
		expected,
		"Set containing element, text node, comment"
	);
	...
	expected = [];
	fixture
		.html(valueObj("<style>.foobar{color:green;}</style>"))
		.each(function () {
			expected.push("STYLE");
			actual.push(childNodeNames(this));
		});
	...
	fixture.html(
		valueObj(
			"<script type='text/javascript'>QUnit.assert.ok( true, 'Injection of identical script' );</script>"
		)
	);
	fixture.html(
		valueObj(
	...
	fixture.html(
		valueObj(
			"foo <form><script type='text/javascript'>QUnit.assert.ok( true, 'Injection of identical script (#975)' );</script></form>"
		)
	);
...
QUnit[
	...
	!/iphone os 10_/i.test(navigator.userAgent) ? "test" : "skip"
...
QUnit.test(
	"html(Function) with incoming value -- direct selection",
	function (assert) {
		assert.expect(4);
...
QUnit.test(
	"html(Function) with incoming value -- jQuery.contents()",
	function (assert) {
		assert.expect(14);
...
QUnit.test(
	"clone()/html() don't expose jQuery/Sizzle expandos (#12858)",
	function (assert) {
		assert.expect(2);

		var $content = jQuery("<div><b><i>text</i></b></div>").appendTo(
				"#qunit-fixture"
			),
...
QUnit.test("remove() with filters", function (assert) {
	...
	assert.equal(
		jQuery("#ap").children().length,
		0,
		"Check multi-filtered remove"
	);
	...
	// Handle the case where no comment is in the document
	assert.ok(
		jQuery("#nonnodes").contents().length >= 2,
		"Check node,textnode,comment remove works"
	);
...
QUnit.test("empty()", function (assert) {
	...
	assert.equal(
		jQuery("#ap").children().empty().text().length,
		0,
...
QUnit.test("jQuery.cleanData", function (assert) {
	...
	function getDiv() {
		var div = jQuery("<div class='outer'><div class='inner'></div></div>")
			.on("click", function () {
		...
		div[0].firstChild.detachEvent = div[0].firstChild.removeEventListener = function (
			t
		) {
...
QUnit.test(
	"jQuery.cleanData eliminates all private data (gh-2127)",
	function (assert) {
		assert.expect(3);
		...
		assert.ok(
			!jQuery.isEmptyObject(jQuery._data(div[0])),
			"Ensure some private data exists"
		);

		div.remove();

		assert.ok(
			!jQuery.hasData(div[0]),
			"Removed element hasData should return false"
		);
...
QUnit.test("jQuery.cleanData eliminates all public data", function (assert) {
	...
	div.data("some", "data");
	assert.ok(
		!jQuery.isEmptyObject(jQuery.data(div[0])),
		"Ensure some public data exists"
	);
...
QUnit.test("domManip plain-text caching (trac-6779)", function (assert) {
	...
	assert.equal(
		$f.text(),
		bad.join(""),
		"Cached strings that match Object properties"
	);
	$f.remove();
});

QUnit.test(
	"domManip executes scripts containing html comments or CDATA (trac-9221)",
	function (assert) {
		assert.expect(3);
...
testIframe(
	...
	function (assert, framejQuery, frameWindow, frameDocument) {
		...
		return new Promise(function (resolve) {
			...
			jQuery(frameDocument.body).append(
				"<script src='" +
					url("manipulation/set-global-scripttest.js") +
					"'></script>"
			);
			assert.ok(!window.scriptTest, "script executed in iframe context");
			assert.ok(frameWindow.scriptTest, "script executed in iframe context");
...
QUnit.test(
	"jQuery.clone - no exceptions for object elements #9587",
	function (assert) {
		assert.expect(1);
...
QUnit.test("Cloned, detached HTML5 elems (#10667,10670)", function (assert) {
	...
	assert.equal(
		$clone[0].nodeName,
		"SECTION",
		"detached clone nodeName matches 'SECTION'"
	);
	...
	assert.equal(
		$clone.attr("class"),
		$section.attr("class"),
		"clone and element have same class attribute"
	);
	assert.equal(
		$clone.attr("title"),
		$section.attr("title"),
		"clone and element have same title attribute"
	);
...
QUnit.test(
	...
	function (assert) {
		...
		assert.ok(
			div && div.jquery,
			"Created nodes safely, guarded against exceptions on safeChildNodes[ -1 ]"
		);
	}
);

QUnit.test(
	"Ensure oldIE creates a new set on appendTo (#8894)",
	function (assert) {
		assert.expect(5);
		...
		assert.strictEqual(
			jQuery("<div></div>")
				...
				.hasClass("test"),
			false,
			"Check jQuery.fn.appendTo after jQuery.clone"
		);
		assert.strictEqual(
			jQuery("<div></div>")
				.find("p")
				...
				.hasClass("test"),
			false,
			"Check jQuery.fn.appendTo after jQuery.fn.find"
		);
		assert.strictEqual(
			jQuery("<div></div>")
				.text("test")
				...
				.hasClass("test"),
			false,
			"Check jQuery.fn.appendTo after jQuery.fn.text"
		);
		assert.strictEqual(
			jQuery("<bdi></bdi>")
				.clone()
				...
				.hasClass("test"),
			false,
			"Check jQuery.fn.appendTo after clone html5 element"
		);
		assert.strictEqual(
			jQuery("<p></p>").appendTo("<div></div>").end().length,
			jQuery("<p>test</p>").appendTo("<div></div>").end().length,
...
QUnit.test("html() - script exceptions bubble (#11743)", function (assert) {
	...
	window.onerror = function () {
		...
		if (jQuery.ajax) {
			...
			jQuery("#qunit-fixture").html(
				"<script src='" + baseURL + "badcall.js'></script>"
			);
...
QUnit.test("checked state is cloned with clone()", function (assert) {
	...
	elem.checked = false;
	assert.equal(
		jQuery(elem).clone().attr("id", "clone")[0].checked,
		false,
...
QUnit.test(
	"manipulate mixed jQuery and text (#12384, #12346)",
	function (assert) {
		assert.expect(2);

		var div = jQuery("<div>a</div>").append(
				"&nbsp;",
				jQuery("<span>b</span>"),
				"&nbsp;",
				jQuery("<span>c</span>")
		...
		assert.equal(
			div.text(),
			"a" + nbsp + "b" + nbsp + "c",
			"Appending mixed jQuery with text nodes"
		);
...
QUnit.test("script evaluation (#11795)", function (assert) {
	...
	assert.deepEqual(
		scriptsOut.get(),
		fixture.find("script").get(),
		"All script tags remain."
	);

	objGlobal.ok = notOk;
	scriptsOut = scriptsOut.add(scriptsOut.clone()).appendTo(fixture.find("div"));
	assert.deepEqual(
		fixture.find("div script").get(),
		scriptsOut.get(),
		"Scripts cloned without reevaluation"
	);
	fixture.append(scriptsOut.detach());
	assert.deepEqual(
		fixture.children("script").get(),
		scriptsOut.get(),
		"Scripts detached without reevaluation"
	);
	objGlobal.ok = isOk;
	...
	if (jQuery.ajax) {
		...
		jQuery("#qunit-fixture").append(
			"<script src='" + url("mock.php?action=testbar") + "'></script>"
		);
...
QUnit[jQuery.ajax ? "test" : "skip"](
	...
	function (assert) {
		...
		jQuery("#qunit-fixture").append(
			"<script src='1'></script><script src='2'></script>"
		);
		...
		jQuery("#qunit-fixture").append(
			"<script src='3'></script><script src='4'></script>"
		);
...
QUnit.test("jQuery.htmlPrefilter (gh-1747)", function (assert) {
	...
	var expectedArgument,
		...
		poison =
			"<script>jQuery.htmlPrefilter.assert.ok( false, 'script not executed' );</script>";
	...
	jQuery.htmlPrefilter = function (html) {
		...
		return htmlPrefilter
			...
			.replace(/<(script|del)(?=[\s>])[\w\W]*?<\/\1\s*>/gi, "");
	};
	...
	assert.equal(
		invocations,
		3,
		"htmlPrefilter invoked for all DOM manipulations"
	);
...
QUnit.test(
	"insertAfter, insertBefore, etc do not work when destination is original element. Element is removed (#4087)",
	function (assert) {
		assert.expect(10);
		...
		jQuery.each(
			...
			function (index, name) {
				...
				jQuery("#test4087-complex div")[name](
					"#test4087-complex li:last-child div:last-child"
				);
				assert.equal(
					jQuery("#test4087-complex li:last-child div").length,
					name === "replaceAll" ? 1 : 2,
				...
				assert.equal(
					jQuery(".test4087-1").length,
					1,
					name + " a node to itself, simple case."
				);
...
QUnit.test(
	"Index for function argument should be received (#13094)",
	function (assert) {
		assert.expect(2);
...
QUnit.test(
	"Make sure jQuery.fn.remove can work on elements in documentFragment",
	function (assert) {
		assert.expect(1);
		...
		assert.equal(
			...
			0,
			"div element was removed from documentFragment"
		);
	}
);

QUnit.test(
	"Make sure specific elements with content created correctly (#13232)",
	function (assert) {
		assert.expect(20);
		...
		jQuery.each(elems, function (name, value) {
			...
			assert.ok(
				jQuery
					.parseHTML("<" + name + ">" + value + "</" + name + ">")[0]
					.nodeName.toLowerCase() === name,
...
QUnit.test(
	"Validate creation of multiple quantities of certain elements (#13818)",
	function (assert) {
		assert.expect(22);
		...
		jQuery.each(tags, function (index, tag) {
			jQuery("<" + tag + "></" + tag + "><" + tag + "></" + tag + ">").each(
				function () {
					assert.ok(
						this.nodeName.toLowerCase() === tag,
						tag + " elements created correctly"
					);
				}
			);
		});
...
QUnit.test(
	"Make sure tr element will be appended to tbody element of table when present",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"Make sure tr elements will be appended to tbody element of table when present",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"Make sure tfoot element will not be appended to tbody element of table when present",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"Make sure document fragment will be appended to tbody element of table when present",
	function (assert) {
		assert.expect(1);
...
QUnit.test(
	"Make sure tr is not appended to the wrong tbody (gh-3439)",
	function (assert) {
		assert.expect(1);
		...
		var htmlOut,
			...
			table = supportjQuery("<table></table>")
				.html(htmlIn)
...
QUnit.test(
	"Make sure tags with single-character names are found (gh-4124)",
	function (assert) {
		assert.expect(1);
...
// The AJAX module is needed for jQuery._evalUrl.
QUnit[jQuery.ajax ? "test" : "skip"](
	"Insert script with data-URI (gh-1887)",
	function (assert) {
		assert.expect(1);
		...
		jQuery(fixture).append(
			"<script src=\"data:text/javascript,testFoo = 'foo';\"></script>"
		);
...
QUnit.test(
	"Ignore content from unsuccessful responses (gh-4126)",
	function (assert) {
		assert.expect(1);
		...
		jQuery.globalEval = function (code) {
			assert.ok(
				false,
				"no attempt to evaluate code from an unsuccessful response"
			);
		};
		...
		try {
			...
			assert.ok(
				true,
				"no error thrown from embedding script with unsuccessful-response src"
			);
		} catch (e) {
			throw e;
		} finally {
...
testIframe(
	...
	QUnit[
		/\bedge\/|iphone os [789]|android 4\./i.test(navigator.userAgent)
			? "skip"
			: "test"
	]
);
...
QUnit.test("Sanitized HTML doesn't get unsanitized", function (assert) {
	...
	function test(htmlString) {
		...
		setTimeout(function () {
			assert.ok(
				window.xss.withArgs(currCounter).notCalled,
