...
(function () {
	...
	QUnit.test("queue() with other types", function (assert) {
		...
		$div.promise("foo").done(function () {
			assert.equal(
				counter,
				0,
				"Deferred for collection with no queue is automatically resolved"
			);
		});
		...
		assert.equal(
			$div.queue("foo", undefined).queue("foo").length,
			4,
	...
	QUnit.test(
		"queue(name) passes in the next item in the queue as a parameter",
		function (assert) {
			assert.expect(2);
			...
			div
				...
				.queue("bar", function () {
					assert.equal(
						++counter,
						3,
						"Other queues are not triggered by next()"
					);
				});
	...
	QUnit.test(
		"queue() passes in the next item in the queue as a parameter to fx queues",
		function (assert) {
	...
	QUnit.test(
		"jQuery.queue should return array while manipulating the queue",
		function (assert) {
			assert.expect(1);
			...
			assert.ok(
				Array.isArray(jQuery.queue(div, "fx", jQuery.noop)),
		...
		function (assert) {
			...
			foo.promise("queue").done(function () {
				assert.strictEqual(
					test,
					undefined,
					"called immediately when queue was already empty"
				);
			});
			test = 1;
			foo
	...
	if (jQuery.fn.animate) {
		QUnit.test(
			'fn.promise( "queue" ) - waits for animation to complete before resolving',
			function (assert) {
				assert.expect(2);
				var done = assert.async();
				var foo = jQuery("#foo"),
	...
	QUnit.test(".promise(obj)", function (assert) {
		...
		assert.ok(
			typeof promise.promise === "function",
			".promise(type, obj) returns a promise"
		);
		assert.strictEqual(promise, obj, ".promise(type, obj) returns obj");
	});

	QUnit[jQuery.fn.stop ? "test" : "skip"](
		"delay() can be stopped",
		function (assert) {
			var done = assert.async();
			assert.expect(3);
			var storage = {};
			jQuery({})
				...
				.queue("alternate", function () {
					...
					assert.ok(
						true,
						"The function was dequeued immediately, the delay was stopped"
					);
				})
				.dequeue("alternate")
