...
QUnit[jQuery.fn.delegate ? "test" : "skip"](
	"delegate/undelegate",
	function (assert) {
		assert.expect(2);
...
QUnit[jQuery.fn.hover ? "test" : "skip"](
...
QUnit[jQuery.fn.click ? "test" : "skip"](
	"trigger() shortcuts",
	function (assert) {
		assert.expect(5);
		...
		var counter,
			clickCounter,
			elem = jQuery("<li><a href='#'>Change location</a></li>").prependTo(
				"#firstUL"
			);
		elem
			.find("a")
...
QUnit[jQuery.fn.click ? "test" : "skip"]("Event aliases", function (assert) {
	...
	var $elem = jQuery("<div></div>").appendTo("#qunit-fixture"),
		aliases = (
			...
			"select submit keydown keypress keyup contextmenu"
		).split(" ");
...
QUnit[jQuery.parseJSON ? "test" : "skip"](
	"jQuery.parseJSON",
	function (assert) {
		assert.expect(20);
		...
		assert.deepEqual(
			jQuery.parseJSON(
				'[ "string", -4.2, 2.7180e0, 3.14E-1, {}, [], true, false, null ]'
			),
		...
		assert.throws(
			...
			null,
			"Single-quoted string raises an error"
		);
		...
		assert.throws(
			...
			null,
			"Leading-zero number raises an error or is parsed as decimal"
		);
		assert.throws(
			function () {
			...
			null,
			"Unquoted property raises an error"
		);
		assert.throws(
			function () {
				jQuery.parseJSON("{'a':1}");
			...
			null,
			"Single-quoted property raises an error"
		);
		assert.throws(
			function () {
				jQuery.parseJSON("[,]");
			...
			null,
			"Array element elision raises an error"
		);
		assert.throws(
			function () {
				jQuery.parseJSON("{},[]");
			...
			null,
			"Comma expression raises an error"
		);
		assert.throws(
			function () {
				jQuery.parseJSON("[]\n,{}");
			...
			null,
			"Newline-containing comma expression raises an error"
		);
		assert.throws(
			function () {
				jQuery.parseJSON('""\n""');
			...
			null,
			"Automatic semicolon insertion raises an error"
		);
...
QUnit[jQuery.isArray ? "test" : "skip"]("jQuery.isArray", function (assert) {
	...
	assert.strictEqual(
		jQuery.isArray,
		Array.isArray,
		"Array.isArray equals jQuery.isArray"
	);
});
...
QUnit[jQuery.nodeName ? "test" : "skip"]("jQuery.nodeName", function (assert) {
	...
	assert.strictEqual(
		typeof jQuery.nodeName,
		"function",
		"jQuery.nodeName is a function"
	);
...
QUnit[jQuery.isFunction ? "test" : "skip"]("isFunction", function (assert) {
	...
	assert.notOk(
		jQuery.isFunction(Object.create(fn)),
...
QUnit[jQuery.isFunction ? "test" : "skip"](
	"isFunction(cross-realm function)",
	function (assert) {
		assert.expect(1);
		...
		window.iframeDone = function (fn, detail) {
			...
			assert.ok(
				...
				"cross-realm function" + (detail ? " - " + detail : "")
			);
			done();
		};
...
supportjQuery.each(
	...
	function (subclass, source) {
		...
		QUnit[jQuery.isFunction && fn ? "test" : "skip"](
			"isFunction(" + subclass + ")",
			function (assert) {
				assert.expect(1);
...
QUnit[
	jQuery.isFunction && typeof Symbol === "function" && Symbol.toStringTag
		? "test"
		: "skip"
]("isFunction(custom @@toStringTag)", function (assert) {
	assert.expect(2);
...
QUnit[jQuery.isWindow ? "test" : "skip"]("jQuery.isWindow", function (assert) {
	...
	assert.ok(
		jQuery.isWindow(document.getElementsByTagName("iframe")[0].contentWindow),
...
QUnit[jQuery.camelCase ? "test" : "skip"](
	"jQuery.camelCase()",
	function (assert) {
		var tests = {
			"foo-bar": "fooBar",
...
QUnit[jQuery.proxy ? "test" : "skip"]("jQuery.proxy", function (assert) {
	...
	test3 = function (a, b) {
		assert.equal(
			b,
			"normal",
			"Ensure arguments can be pre-applied and passed as usual."
		);
	};
