...
QUnit.test("attributes", function (assert) {
	...
	assert.strictEqual(
		a.hasClass("abc"),
		true,
		".(add|remove|has)Class, class present"
	);
	assert.strictEqual(
		a.hasClass("def"),
		false,
		".(add|remove|has)Class, class missing"
	);
...
QUnit.test("core", function (assert) {
	...
	assert.deepEqual(
		jQuery.merge([1, 2], ["a", "b"]),
		[1, 2, "a", "b"],
		"jQuery.merge"
	);

	assert.deepEqual(
		jQuery.grep([1, 2, 3], function (value) {
			return value % 2 !== 0;
		}),
		[1, 3],
		"jQuery.grep"
	...
	assert.deepEqual(
		jQuery.makeArray({ 0: "a", 1: "b", 2: "c", length: 3 }),
		["a", "b", "c"],
		"jQuery.makeArray"
	);

	assert.strictEqual(
		jQuery.parseHTML("<div></div><span></span>").length,
		2,
		"jQuery.parseHTML"
	);
});
...
QUnit.test("manipulation", function (assert) {
	...
	assert.strictEqual(
		elem1.prepend(elem2)[0].childNodes[0],
		elem2[0],
		".prepend"
	);
...
QUnit[/jsdom\//.test(navigator.userAgent) ? "skip" : "test"](
	"offset",
	function (assert) {
		assert.expect(3);
		...
		var parent = jQuery(
			...
			elem = jQuery("<div style='position:absolute;top:5px;'></div>").appendTo(
				parent
			);
...
QUnit.test("selector", function (assert) {
	...
	var elem = jQuery(
		"<div><span class='a'></span><span class='b'><a></a></span></div>"
	).appendTo("#qunit-fixture");

	assert.strictEqual(elem.find(".a a").length, 0, ".find - no result");
	assert.strictEqual(
		elem.find("span.b a")[0].nodeName,
		"A",
		".find - one result"
	);
});
