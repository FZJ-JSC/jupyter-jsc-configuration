...
(function () {
			...
			jQuery.each(filters, function (filterLabel) {
				jQuery.each(
					...
					function (flagsTypes, flags) {
						QUnit.test(
							"jQuery.Callbacks( " + showFlags(flags) + " ) - " + filterLabel,
							function (assert) {
								assert.expect(29);
								...
								assert.strictEqual(
									output,
									"X",
									"Adding a callback after disabling"
								);
								cblist.fire("A");
								...
								assert.strictEqual(
									output,
									"XAB",
									"Basic binding and firing (arguments)"
								);
								...
								cblist.add(function () {
									...
									assert.strictEqual(
										arguments.length,
										0,
										"fireWith with no arguments (no arguments)"
									);
								});
								cblist.fireWith();
								...
								assert.strictEqual(
									output,
									"XA",
									"Locking doesn't abort execution (gh-1990)"
								);
								...
								assert.strictEqual(
									output,
									results.shift(),
									"Multiple fire (first new callback)"
								);
								output = "X";
								cblist.fire("B");
								assert.strictEqual(
								...
								assert.strictEqual(
									output,
									results.shift(),
									"Multiple fire (second new callback)"
								);
								...
								assert.strictEqual(
									output,
									results.shift(),
									"Adding a callback after one returned false"
								);
	...
	QUnit.test(
		"jQuery.Callbacks( options ) - options are copied",
		function (assert) {
			assert.expect(1);
	...
	QUnit.test(
		"jQuery.Callbacks.fireWith - arguments are copied",
		function (assert) {
			assert.expect(1);
	...
	QUnit.test(
		"jQuery.Callbacks.remove - should remove all instances",
		function (assert) {
			assert.expect(1);
	...
	QUnit.test("jQuery.Callbacks.has", function (assert) {
		...
		assert.strictEqual(
			cb.has(),
			true,
			"No arguments to .has() returns whether callback function(s) are attached or not"
		);
		assert.strictEqual(
			cb.has(getA),
			true,
			"Check if a specific callback function is in the Callbacks list"
		);
		...
		assert.strictEqual(
			cb.has(getB),
			false,
			"Remove a specific callback function and make sure its no longer there"
		);
		assert.strictEqual(
			cb.has(getA),
			true,
		...
		assert.strictEqual(
			cb.has(),
			false,
			"Empty list and make sure there are no callback function(s)"
		);
		assert.strictEqual(
			cb.has(getA),
			false,
			"Check for a specific function in an empty() list"
		);
		...
		cb.add(getA, getB, function () {
			assert.strictEqual(
				cb.has(),
				true,
				"Check if list has callback function(s) from within a callback function"
			);
			assert.strictEqual(
				cb.has(getA),
				true,
				"Check if list has a specific callback from within a callback function"
			);
		}).fire();
		...
		assert.strictEqual(
			cb.has(),
			true,
			"Callbacks list has callback function(s) after firing"
		);
		...
		assert.strictEqual(
			cb.has(),
			false,
			"disabled() list has no callback functions (returns false)"
		);
		assert.strictEqual(
			cb.has(getA),
			false,
			"Check for a specific function in a disabled() list"
		);
		...
		assert.strictEqual(
			cb.has(),
			true,
			"Check if unique list has callback function(s) attached"
		);
		cb.lock();
		assert.strictEqual(
			cb.has(),
			false,
			"locked() list is empty and returns false"
		);
	});

	QUnit.test(
		"jQuery.Callbacks() - adding a string doesn't cause a stack overflow",
		function (assert) {
			assert.expect(1);
	...
	QUnit.test(
		"jQuery.Callbacks() - disabled callback doesn't fire (gh-1790)",
		function (assert) {
			assert.expect(1);
	...
	QUnit.test(
		"jQuery.Callbacks() - list with memory stays locked (gh-3469)",
		function (assert) {
			assert.expect(3);
